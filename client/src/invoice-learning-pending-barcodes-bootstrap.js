...
if(window.location.pathname.replace(/\/+$/,'')===PATH){
  ...
  const openModal=item=>{
    ...
    m.innerHTML=`<div class="pb-box"><div class="pb-titlebar"><span>Νέο είδος / Διόρθωση προϊόντος</span><button class="pb-close">×</button></div><div class="pb-body"><div class="pb-hint">Διόρθωσε από εδώ όλα τα στοιχεία του προϊόντος. Το Learning θα κρατήσει τις αλλαγές για τον συγκεκριμένο προμηθευτή.</div><div class="pb-search"><b>Βρες πρώτα το σωστό προϊόν / barcode</b><div class="pb-search-row"><input data-q value="${esc(item.description)}"><button class="pb-open" data-master>🔎 Master Catalog</button><button class="pb-find" data-google>🌐 Google / Barcode</button></div><div class="pb-results" data-results></div></div><div class="pb-form">
      ...
      <label>Γραμμωτός κώδικας<input data-f="barcode" value="${esc(item.barcode||'')}" placeholder="Σκάναρε ή γράψε barcode"></label>
      ...
      <label>Κωδικός προμηθευτή<input data-f="supplierItemCode" value="${esc(item.supplierItemCode||'')}"></label>
  ...
  const render=()=>{
    ...
    let host=document.querySelector('[data-pending-barcodes]');if(!host){host=document.createElement('section');host.className='pb-card';host.dataset.pendingBarcodes='1';host.hidden=true;const history=[...main.querySelectorAll('.card')].find(x=>x.textContent.includes('Εκπαιδευμένα τιμολόγια'));if(history)main.insertBefore(host,history);else main.appendChild(host)}
    const items=collect(),pending=items.filter(x=>!x.barcode);
    host.innerHTML=`<div class="pb-head"><div><h2>3. Προϊόντα / Εκκρεμή Barcodes</h2><div>Άνοιξε το προϊόν και διόρθωσε από ένα σημείο barcode, περιγραφή, κατηγορία, ΦΠΑ, μονάδες, τιμές και stock.</div></div><span class="pb-badge">${pending.length} χωρίς barcode</span></div><div style="overflow:auto"><table class="pb-table"><thead><tr><th>Προμηθευτής</th><th>Κωδικός προμηθευτή</th><th>Προϊόν</th><th>Γραμμωτός κώδικας</th><th></th></tr></thead><tbody>${items.length?items.map((x,i)=>`<tr data-i="${i}"><td>${esc(x.supplierName)}${x.central?'<br><span class="pb-central">ΚΕΝΤΡΙΚΗ ΕΚΜΑΘΗΣΗ</span>':''}</td><td>${esc(x.supplierItemCode||'—')}</td><td><b>${esc(x.description)}</b></td><td>${esc(x.barcode||'Εκκρεμεί')}</td><td><button class="pb-open">Άνοιγμα / Διόρθωση προϊόντος</button></td></tr>`).join(''):'<tr><td colspan="5">Δεν υπάρχουν ακόμη προϊόντα εκμάθησης.</td></tr>'}</tbody></table></div>`;
    if(!host.dataset.pendingToggle){host.dataset.pendingToggle='1';document.querySelector('[data-jump="pending"]')?.addEventListener('click',()=>{const compact=document.querySelector('#pending');if(compact)compact.hidden=true;host.hidden=false;host.scrollIntoView({behavior:'smooth',block:'start'})})}host.querySelectorAll('tbody tr[data-i]').forEach(row=>{const item=items[Number(row.dataset.i)];row.querySelector('.pb-open').onclick=()=>openModal(item)});return true;
  };
