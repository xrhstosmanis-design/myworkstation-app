...
const isPlatformSuper=()=>{
  if(!/^\/platform-admin(?:\/|$)/.test(window.location.pathname))return false;
  const platformUser=readStored("platformUser"),legacyUser=readStored("user");
  const u=platformUser||legacyUser||{};
  return u.isSuperAdmin===true||u.platformRole==="SUPER_ADMIN"||u.role==="SUPER_ADMIN";
};
...
function install(){
  ...
  document.querySelectorAll(".master-catalog-modal").forEach(modal=>{
    ...
    const quickSections=[...modal.querySelectorAll(".master-section")];
    const quick=quickSections.find(section=>String(section.textContent||"").includes("Γρήγορος έλεγχος καταλόγου"));
    const host=quick?.querySelector("form")||quick?.querySelector(".master-search")||quick||modal.querySelector(".master-maintenance")||modal.querySelector(".master-catalog-head");
    if(!host)return;
