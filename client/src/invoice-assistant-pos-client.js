...
export async function openPosInvoiceAssistant(orderId,order,onComplete){
  ...
  overlay.innerHTML=`<section role="dialog" aria-modal="true" aria-label="Βοηθός τιμολογίου POS" style="width:min(1800px,100%);height:min(96vh,1100px);background:#f7fafc;border-radius:15px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 20px 60px #0005">
    ...
    <div style="display:grid;grid-template-columns:minmax(280px,42%) minmax(350px,1fr);min-height:0;flex:1">
      ...
      <div style="min-height:0;overflow:auto;padding:16px"><p style="margin:0 0 9px">Δες τις φωτογραφίες, πες τι θέλεις να διορθωθεί και έλεγξε τις προτάσεις πριν τις περάσεις στο πρόχειρο.</p>
        ...
        <details style="margin-top:14px;padding:10px;background:#eaf5f1;border-radius:9px"><summary style="cursor:pointer;font-weight:800">Κανόνας συσκευασίας προμηθευτή</summary><p>Αποθήκευση μετά από έλεγχο του κωδικού και της συσκευασίας. Ο κανόνας ισχύει για τον προμηθευτή σε όλα τα καταστήματα και μπορεί να αποθηκευτεί από ιδιοκτήτη ή ελεγκτή τιμολογίων.</p><div style="display:flex;flex-wrap:wrap;gap:8px"><label>ΑΦΜ προμηθευτή<br><input data-rule-tax maxlength="9" inputmode="numeric" style="width:120px"></label><label>Κωδικός προϊόντος<br><input data-rule-code style="width:150px"></label><label>Τεμάχια ανά πακέτο<br><input data-rule-factor type="number" min="1" step="1" style="width:100px"></label><button data-save-rule type="button">Αποθήκευση κανόνα</button></div><p data-rule-status role="status"></p></details>
  ...
  overlay.querySelector("[data-save-rule]").onclick=async()=>{
    ...
    if(!/^\d{9}$/.test(tax)||!code||!Number.isInteger(factor)||factor<1){ruleStatus.textContent="Συμπλήρωσε έγκυρο ΑΦΜ, κωδικό προϊόντος και θετικό ακέραιο πλήθος τεμαχίων.";return}
    const button=overlay.querySelector("[data-save-rule]");button.disabled=true;
    try{const result=await api("/api/platform/invoice-learning/supplier-profile/stock-rules",{method:"PUT",body:JSON.stringify({supplierTaxId:tax,supplierName:order.supplierName||"",rules:[{supplierItemCode:code,invoiceUnit:"PACKAGE",stockUnit:"PIECE",factor}]})});ruleStatus.textContent=`Ο κανόνας αποθηκεύτηκε κεντρικά (έκδοση ${result.profileVersion}). Εφάρμοσε τη μετατροπή στην αντίστοιχη γραμμή του πίνακα, αν ισχύει στο τυπωμένο τιμολόγιο.`}
