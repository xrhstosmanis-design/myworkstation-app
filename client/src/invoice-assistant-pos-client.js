...
export async function openPosInvoiceAssistant(orderId,order,onComplete){
  ...
  overlay.querySelector("[data-save-rule]").onclick=async()=>{
    ...
    try{
      const matches=activePrintedRows.filter(row=>String(row.supplierCode||"").trim().toUpperCase()===code.toUpperCase());
      if(matches.length!==1||!activePagesComplete||activePrintedTotal===null)throw new Error("Έλεγξε πρώτα ολόκληρο το τιμολόγιο και επίλεξε έναν μοναδικό τυπωμένο κωδικό.");
