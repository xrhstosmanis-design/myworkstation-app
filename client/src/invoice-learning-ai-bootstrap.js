...
if(labPath){
  let selectedFiles=[],selectedPages=[],running=false;
  ...
  const renderSelectedPages=()=>{let host=document.querySelector('[data-learning-pages]');if(!host){host=document.createElement('div');host.dataset.learningPages='1';host.style.cssText='display:grid;gap:5px;margin-top:9px';document.querySelector('.uploadBtns')?.after(host)}host.hidden=!selectedFiles.length;host.innerHTML=selectedFiles.map((file,index)=>`<div style="padding:7px 9px;border:1px solid #b7d8cc;border-radius:7px;background:#f5fbf8"><b>Σελίδα ${index+1}/${selectedFiles.length}</b> · ${String(file.name||'φωτογραφία').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]))}</div>`).join('')};
  const acceptFiles=async files=>{if(running)return;const incoming=Array.from(files||[]).filter(Boolean),status=document.querySelector('#status');if(!incoming.length)return;if(incoming.length>5){alert('Μπορείς να επιλέξεις έως 5 σελίδες του ίδιου τιμολογίου.');return}if(incoming.some(file=>file.type==='application/pdf')&&incoming.length>1){alert('Επίλεξε είτε ένα PDF είτε έως 5 φωτογραφίες του ίδιου τιμολογίου.');return}selectedFiles=[];selectedPages=[];if(status)status.textContent=`Προετοιμασία ${incoming.length} ${incoming.length===1?'σελίδας':'σελίδων'} και έλεγχος ευκρίνειας…`;try{for(const file of incoming){const prepared=await prepareDocumentFile(file,{strict:true,maxSide:3000,enhance:true});selectedFiles.push(prepared.file);selectedPages.push({filename:prepared.file.name,mimeType:prepared.file.type||'image/jpeg',fileData:await readFile(prepared.file),imageQuality:prepared.quality})}renderSelectedPages();if(status)status.textContent=`✓ ${selectedFiles.length}/${selectedFiles.length} ${selectedFiles.length===1?'σελίδα είναι έτοιμη':'σελίδες είναι έτοιμες'}. Ξεκινά η ενιαία ανάγνωση…`;setTimeout(()=>runAi(document.querySelector('[data-ai-recheck]')),0)}catch(error){selectedFiles=[];selectedPages=[];renderSelectedPages();if(status)status.textContent=`⚠ ${error?.message||'Μία σελίδα δεν πέρασε τον έλεγχο ποιότητας.'}`;alert(error?.message||'Μία σελίδα δεν πέρασε τον έλεγχο ποιότητας.')}};
  const acceptFile=async file=>acceptFiles(file?[file]:[]);
  const capture=async e=>acceptFiles(e.target.files);
  ...
  const readStableInvoice=async()=>{
    ...
    for(let attempt=1;attempt<=3;attempt++){
      ...
      try{
        ...
        if(best?.completeness?.complete)return {...best,stableRead:false,stabilityIncomplete:true,stabilityWarning:error?.message||'Ο επαναληπτικός έλεγχος δεν ολοκληρώθηκε.'};
        throw error;
      }
      if(!response.ok){
        const providerState=data.azureState?` Azure: ${data.azureState}.`:'',error=new Error(`${data.error||`AI σφάλμα ${response.status}`}${providerState}`);
        if(best?.completeness?.complete)return {...best,stableRead:false,stabilityIncomplete:true,stabilityWarning:error.message};
        throw error;
      }
      best=data;
  ...
  const runAi=async button=>{
    if(running)return;
    if(!selectedPages.length){alert('Επίλεξε ξανά το PDF ή τις φωτογραφίες ώστε το AI να διαβάσει ολόκληρο το πρωτότυπο παραστατικό.');return}
    ...
    try{const data=await readStableInvoice();applyResult(data);const source=providerLabel(data);if(status)status.textContent=data.requiresManualCompletion?`⚠ ${source} έδωσε μερικό αποτέλεσμα. Διόρθωσε ή πρόσθεσε γραμμές και μετά πάτησε «Αποθήκευση Πρόχειρου».`:`${source} ολοκληρώθηκε: ${(data.productLines||[]).length} προϊόντα. Ξεκινά αυτόματη εύρεση barcode…`;await resolveAllBarcodes(status);if(status&&data.requiresManualCompletion)status.textContent=`⚠ Μερικό πρόχειρο ${source}: έλεγξε όλες τις γραμμές, πρόσθεσε όσες λείπουν και αποθήκευσέ το πριν την εκμάθηση.`;else if(status&&data.stabilityIncomplete)status.textContent=`⚠ Η ανάγνωση ${source} είναι μαθηματικά πλήρης, αλλά ο επαναληπτικός έλεγχος καθυστέρησε. Έλεγξε τις γραμμές πριν από την εκμάθηση. Barcode lookup ολοκληρώθηκε.`;else if(status)status.textContent=`Ανάγνωση: ${source}. Barcode lookup ολοκληρώθηκε.`}catch(err){if(status)status.textContent=`AI: ${err.message}`;alert(err.message)}finally{running=false;if(button){button.disabled=false;button.textContent=old}}
  };
  let statusChecked=false;
  const install=()=>{
    document.querySelectorAll('#pdfFile,#photoFile').forEach(el=>{if(el.dataset.aiCapture)return;el.dataset.aiCapture='1';if(el.id==='photoFile')el.multiple=true;el.addEventListener('change',capture)});const host=document.querySelector('.uploadBtns');if(!host)return false;const photoButton=document.querySelector('#photo'),photoLabel='🖼 Φωτογραφίες (έως 5)';if(photoButton&&photoButton.textContent!==photoLabel)photoButton.textContent=photoLabel;
    ...
    if(!statusChecked){statusChecked=true;fetch('/api/platform/invoice-learning/ai-status',{headers:{Authorization:`Bearer ${token()}`}}).then(r=>r.json()).then(s=>{if(s.azureConfigured){b.title=`Azure Document Intelligence ενεργό • ${s.model} • OpenAI fallback ${s.openaiConnected?'ενεργό':'ανενεργό'}`;b.textContent='✨ Επανέλεγχος: Azure → AI'}else if(s.openaiConnected||s.connected){b.title=`Azure μη ρυθμισμένο • OpenAI ${s.model}`;b.textContent='✨ Επανέλεγχος με OpenAI'}else{b.title='Χρειάζεται Azure Document Intelligence ή OPENAI_API_KEY στον server';b.textContent='✨ AI — σύνδεση παρόχου'}}).catch(()=>{b.title='Δεν ήταν δυνατός ο έλεγχος σύνδεσης AI';})}return true;
  };
