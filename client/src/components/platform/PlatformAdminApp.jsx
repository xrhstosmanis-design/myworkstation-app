...
export default function PlatformAdminApp(){
  ...
  const loadCashReport=async(date=cashReportDate)=>{
    ...
    try{setCashReport(await request(`/api/platform/cash-control/daily?date=${encodeURIComponent(date)}`))}catch(err){setError(err.message)}finally{setBusy("")}
  };
  ...
  const downloadShortages=async store=>{
    ...
    try{
      const query=new URLSearchParams({from:cashRangeFrom,to:cashRangeTo,...(store?{storeId:store.storeId}:{}),...(cashOperator.trim()?{operator:cashOperator.trim()}:{})});
      const result=await request(`/api/platform/cash-control/shortages?${query}`);
      const cell=value=>`"${String(value??"").replaceAll('"','""')}"`;
      const lines=[["Ημερομηνία","Εταιρεία","Κατάστημα","POS","Βάρδια","Χειριστής","Έλλειμμα","Διαφορά POS–EFTPOS"],...result.rows.map(row=>[row.date,row.companyName,row.storeName,row.terminalPos||"MAIN",row.shiftLabel,row.openedByName||"—",row.shortage.toFixed(2),row.cardVariance.toFixed(2)])];
