...
const cashReportDate=value=>value?new Intl.DateTimeFormat("el-GR",{timeZone:"Europe/Athens",weekday:"long",day:"2-digit",month:"2-digit",year:"numeric"}).format(new Date(`${value}T12:00:00+03:00`)):"—";
...
export default function PlatformAdminApp(){
  ...
  const openTerminals=async(company,store)=>{
    ...
    try{const result=await request(`/api/platform/companies/${company.id}/stores/${store.id}/installation-terminals`);setTerminalManager({company,store,terminals:result.terminals,activationUrl:""})}
    catch(err){setError(err.message)}finally{setBusy("")}
  };
  const refreshTerminals=async(current=terminalManager)=>{
    const result=await request(`/api/platform/companies/${current.company.id}/stores/${current.store.id}/installation-terminals`);
    setTerminalManager(value=>({...value,terminals:result.terminals}));
  };
  ...
  return <div className="platform-shell">
    ...
    {storeCompany&&!onlineStoreManager&&<div className="online-store-launcher" style={{position:"fixed",right:32,bottom:32,zIndex:1002,display:"grid",gap:8}}>{storeCompany.stores.map(store=><button key={store.id} onClick={()=>openOnlineStore(storeCompany,store)} disabled={busy===`online-store:${store.id}`}><ShoppingBag/>{busy===`online-store:${store.id}`?"Φόρτωση…":`Online Store · ${store.name}`}</button>)}</div>}
