...
export default function SuperAdminChecksAnalytics({companies=[],request,onClose,setMessage,embedded=false}){
  ...
  const run=async()=>{
    ...
    try{
      ...
      const [analytics,bank,bankReview,supplierReview,expenseReview,auditTrail]=await Promise.all([
        ...
        request(`/api/reports/audit-events${reviewSuffix}`)
      ]);
      setResult({analytics,bank,bankReview,supplierReview,expenseReview,auditTrail,filters:{...filters},executedAt:new Date().toISOString()});
  ...
  const auditTrailItems=(result?.auditTrail?.items||[]).slice(0,100);
