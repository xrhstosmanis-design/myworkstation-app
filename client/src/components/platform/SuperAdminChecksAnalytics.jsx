  ...
  return <div className={embedded?"platform-checks-page":"platform-modal"}><section className={`sa-modal${embedded?" sa-page":""}`}>
    ...
    {result&&<>
      ...
      <section className="platform-panel" style={{marginBottom:14}}>
        ...
        {!complete.enabled?<div className="platform-empty">{complete.reason||"Ενεργοποίησε COMPLETE ή PREMIUM Έλεγχο για αυτά τα στοιχεία."}</div>:<><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12,margin:"12px 0"}}><article style={{padding:12,border:"1px solid #d7e1ec",borderRadius:10}}><small>Χωρίς παραστατικό</small><h3>{number(complete.withoutEvidenceCount)}</h3></article><article style={{padding:12,border:"1px solid #d7e1ec",borderRadius:10}}><small>Πιθανές διπλές πληρωμές</small><h3>{number(complete.potentialDuplicateCount)}</h3></article></div>{completeFindings.length?<div style={{display:"grid",gap:10}}>{completeFindings.map(finding=><article key={finding.id} style={{border:"1px solid #fed7aa",borderRadius:12,padding:12,background:"#fffdf9"}}><div style={{display:"flex",justifyContent:"space-between",gap:10,flexWrap:"wrap"}}><div><small>{finding.code==="PAYMENT_WITHOUT_EVIDENCE"?"Πληρωμή / έξοδο":"Έλεγχος προμηθευτή"}</small><h3 style={{margin:"4px 0"}}>{finding.title}</h3></div><span style={{fontWeight:800,color:"#9a4b0b"}}><AlertTriangle/> Χρειάζεται έλεγχο</span></div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(155px,1fr))",gap:9,marginTop:10}}><span><b>Ημερομηνία</b><small style={{display:"block"}}>{athensDateTime(finding.occurredAt)}</small></span><span><b>Προμηθευτής</b><small style={{display:"block"}}>{finding.supplierName||"—"}</small></span><span><b>Ποσό</b><small style={{display:"block"}}>{eur(finding.amount)}</small></span>{finding.transactionCount&&<span><b>Κινήσεις</b><small style={{display:"block"}}>{finding.transactionCount}</small></span>}<span><b>Χειριστής</b><small style={{display:"block"}}>{finding.actorName||"—"}</small></span></div>{finding.description&&<small style={{display:"block",marginTop:9}}>Περιγραφή: {finding.description}</small>}<small style={{display:"block",marginTop:9}}>Αναφορά: {finding.referenceId||"—"}</small></article>)}</div>:<div className="platform-empty">Δεν βρέθηκαν πληρωμές χωρίς παραστατικό ή πιθανά διπλές πληρωμές στα επιλεγμένα φίλτρα.</div>}</>}
      </section>
      <section className="platform-panel" style={{marginBottom:14}}><div style={{display:"flex",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}><div><b>PREMIUM · Πλήρης συσχέτιση ανά βάρδια</b><p style={{marginBottom:0}}>Ίδιο καλάθι, ποσό, πληρωμές, ακυρώσεις και audit μόνο μέσα στην ίδια κλεισμένη βάρδια. Δεν αποτελεί απόδοση ευθύνης.</p></div><span style={{fontWeight:800,color:premium.enabled?(premiumFindings.length?"#9a4b0b":"#08734e"):"#64748b"}}>{premium.enabled?(premiumFindings.length?"Χρειάζεται έλεγχο":"ΟΚ"):"Δεν είναι ενεργό"}</span></div>{!premium.enabled?<div className="platform-empty">{premium.reason}</div>:<><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12,margin:"12px 0"}}><article style={{padding:12,border:"1px solid #d7e1ec",borderRadius:10}}><small>Πιθανές διπλές / αλλαγές πληρωμής</small><h3>{number(premium.potentialDuplicateSaleCount)}</h3></article><article style={{padding:12,border:"1px solid #d7e1ec",borderRadius:10}}><small>Πιθανές διπλές ακυρώσεις</small><h3>{number(premium.potentialRepeatedReversalCount)}</h3></article></div>{premiumFindings.length?<div style={{display:"grid",gap:10}}>{premiumFindings.map(item=><article key={item.id} style={{padding:12,border:"1px solid #fed7aa",borderRadius:10,background:"#fffdf9"}}><b>{item.title}</b><small style={{display:"block",marginTop:5}}>Βάρδια: {item.shiftLabel||item.sessionId||"—"} · POS: {item.terminalPos||"—"} · Χειριστής: {item.operatorName||"—"}</small>{item.amount!==undefined&&<small style={{display:"block"}}>Ποσό: {eur(item.amount)}</small>}{item.saleIds?.length>0&&<small style={{display:"block"}}>Πωλήσεις: {item.saleIds.join(" · ")}</small>}{item.minutesApart!==undefined&&<small style={{display:"block"}}>Χρονική απόσταση: {item.minutesApart} λεπτά</small>}<small style={{display:"block",marginTop:7}}>{item.possibleExplanation}</small></article>)}</div>:<div className="platform-empty">Δεν βρέθηκαν συσχετίσεις που χρειάζονται έλεγχο στις ίδιες κλεισμένες βάρδιες.</div>}</>}</section>
