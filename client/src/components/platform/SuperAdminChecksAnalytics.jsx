...
export default function SuperAdminChecksAnalytics({companies=[],request,onClose,setMessage,embedded=false}){
  ...
  return <div className={embedded?"platform-checks-page":"platform-modal"}><section className={`sa-modal${embedded?" sa-page":""}`}>
    ...
    {result&&<>
      ...
      <section className="platform-panel" style={{marginBottom:14}}><div style={{display:"flex",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}><div><b>PREMIUM · Επαναλαμβανόμενες αποκλίσεις</b><p style={{marginBottom:0}}>Ένδειξη ανά χειριστή από κλεισμένες βάρδιες. Δεν αποτελεί απόδοση ευθύνης.</p></div><span style={{fontWeight:800,color:premium.enabled?(premiumFindings.length?"#9a4b0b":"#08734e"):"#64748b"}}>{premium.enabled?(premiumFindings.length?"Χρειάζεται έλεγχο":"ΟΚ"):"Δεν είναι ενεργό"}</span></div>{!premium.enabled?<div className="platform-empty">{premium.reason}</div>:<><div style={{margin:"12px 0",padding:12,border:"1px solid #d7e1ec",borderRadius:10}}><small>Χειριστές με 2+ αποκλίσεις</small><h3>{number(premium.repeatedVarianceCount)}</h3></div>{premiumFindings.length?<div style={{display:"grid",gap:10}}>{premiumFindings.map(item=><article key={item.id} style={{padding:12,border:"1px solid #fed7aa",borderRadius:10}}><b>{item.operatorName}</b><small style={{display:"block",marginTop:5}}>{item.shiftCount} βάρδιες με απόκλιση · Μετρητά {eur(item.cashVarianceTotal)} · POS–EFTPOS {eur(item.cardVarianceTotal)}</small></article>)}</div>:<div className="platform-empty">Δεν βρέθηκαν επαναλαμβανόμενες αποκλίσεις στα επιλεγμένα φίλτρα.</div>}</>}</section>
