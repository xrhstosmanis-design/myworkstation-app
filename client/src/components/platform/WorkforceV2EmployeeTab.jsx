  ...
  return <div className="workforce-two-column">
    <section className="workforce-editor-card">
      ...
      <div className="workforce-form-grid">
        ...
        <label>Τύπος πληρωμής<select value={form.paymentType} onChange={e=>setField("paymentType",e.target.value)}><option value="HOURLY">Ανά ώρα</option><option value="DAILY">Ημερομίσθιο</option><option value="FIXED_MONTHLY">Σταθερό μηνιαίο</option></select></label>
        {form.paymentType==="HOURLY"?<label>Ωρομίσθιο<input type="number" min="0" step="0.01" value={form.hourlyRate} onChange={e=>setField("hourlyRate",e.target.value)}/></label>:form.paymentType==="DAILY"?<label>Ημερομίσθιο<input type="number" min="0" step="0.01" value={form.dailyRate} onChange={e=>setField("dailyRate",e.target.value)}/></label>:<label>Μηνιαίο ποσό<input type="number" min="0" step="0.01" value={form.fixedMonthlyAmount} onChange={e=>setField("fixedMonthlyAmount",e.target.value)}/></label>}
