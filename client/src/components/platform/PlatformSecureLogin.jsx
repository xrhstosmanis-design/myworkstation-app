...
export default function PlatformSecureLogin({onLogin}){
  ...
  return <div className="platform-login-shell">
    ...
    {step==="password"&&<form className="platform-login-card" onSubmit={submitPassword}>
      ...
      <label>Όνομα αυτής της συσκευής<input value={deviceName} onChange={e=>setDeviceName(e.target.value)} maxLength="80" placeholder="π.χ. HOME PC" required/></label>
    ...
    {step==="setup"&&<form className="platform-login-card secure-step" onSubmit={enable}>
      ...
      <label>Εξαψήφιος κωδικός Authenticator<input inputMode="numeric" autoComplete="one-time-code" value={code} onChange={e=>setCode(e.target.value.replace(/\D/g,"").slice(0,6))} minLength="6" maxLength="6" required autoFocus/></label>
      {error&&<div className="platform-error">{error}</div>}
    ...
    {step==="verify"&&<form className="platform-login-card secure-step" onSubmit={verify}>
      ...
      <label>Κωδικός 2FA ή κωδικός ανάκτησης<input autoComplete="one-time-code" value={code} onChange={e=>setCode(e.target.value.toUpperCase().slice(0,20))} minLength="6" required autoFocus/></label>
      {error&&<div className="platform-error">{error}</div>}
