...
export default function FiscalBridgeDryRunCenter({companies,onClose}){
  ...
  return <div className="platform-modal fiscal-dry-run-overlay"><section className="fiscal-dry-run-dialog" role="dialog" aria-modal="true" aria-label="Fiscal Bridge DRY RUN">
    ...
    <section className="fiscal-dry-run-panel"><h3>Επιλέξιμη NON_FISCAL πώληση</h3>{!candidates.length?<p>Δεν βρέθηκε ολοκληρωμένη μη φορολογική πώληση με ασφαλή, μοναδική αντιστοίχιση POS/RBS.</p>:<><select value={saleId} onChange={event=>setSaleId(event.target.value)}>{candidates.map(row=><option value={row.id} key={row.id}>{when(row.occurredAt)} · {row.terminalPos} → {row.fiscalDeviceCode} · {money(row.total)}</option>)}</select>{selected&&<div className="fiscal-selection"><span>Πώληση: <b>{selected.id}</b></span><span>Διαδρομή: <b>{selected.terminalPos} → {selected.fiscalDeviceCode}</b></span><span>Πληρωμή: <b>{selected.eftposDeviceCode==="NOT_APPLICABLE"?"Μετρητά / χωρίς EFTPOS":selected.eftposDeviceCode}</b></span>{selected.routeOrigin==="RECOVERED_CURRENT_MAPPING"&&<span>Mapping: <b>Ανακτήθηκε από τη βάρδια και ελέγχθηκε ξανά</b></span>}</div>}</>}
