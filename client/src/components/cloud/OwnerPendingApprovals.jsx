...
export default function OwnerPendingApprovals({api,store,onChanged}){
 ...
 const load=async()=>{if(!owner)return;setBusy("load");setError("");try{
   ...
   setItems([
     ...(suppliers.items||[]).map(item=>({...item,kind:"SUPPLIER",label:`Προμηθευτής · ${item.supplierName||"—"}`})),
     ...(expenses.items||[]).map(item=>({...item,kind:"EXPENSE",label:`Λοιπό έξοδο · ${item.description||"—"}`})),
     ...(bank.items||[]).filter(item=>item.storeId===store.id).map(item=>({...item,kind:"BANK",label:`Ταμείο Τράπεζας · ${item.type}`}))
   ]);
