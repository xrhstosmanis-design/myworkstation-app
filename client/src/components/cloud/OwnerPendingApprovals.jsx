...
import {AlertTriangle,CheckCircle2,RefreshCw,ShieldCheck,Upload} from "lucide-react";
...
const supplierMethod={CASH_SHIFT:"Μετρητά ενεργής βάρδιας",CORPORATE_CARD:"Εταιρική κάρτα",BANK_TRANSFER:"Τραπεζική μεταφορά",EMPLOYEE_REIMBURSEMENT:"Πληρωμή υπαλλήλου προς επιστροφή"};
export default function OwnerPendingApprovals({api,store,onChanged,refreshToken}){
 ...
 const load=async()=>{if(!owner)return;setBusy("load");setError("");try{
   ...
   const supplierItems=(suppliers.items||[]).map(item=>({...item,kind:"SUPPLIER",label:`Προμηθευτής · ${item.supplierName||"—"} · ${supplierMethod[item.paymentMethod]||item.paymentMethod||"Άγνωστος τρόπος"}`}));
 ...
 const review=async(item,status)=>{const note=noteFor(status);if(!note||note.trim().length<3)return;setBusy(item.kind+item.id);try{
   ...
   setItems(rows=>status==="DISCREPANCY"?rows.map(row=>row.kind===item.kind&&row.id===item.id?{...row,status}:row):rows.filter(row=>!(row.kind===item.kind&&row.id===item.id)));onChanged?.();
 }catch(e){setError(e.message||"Δεν ενημερώθηκε η επιβεβαίωση.")}finally{setBusy("")}};
 const cancelSupplier=async item=>{const reason=window.prompt("Αιτιολογία ακύρωσης της εκκρεμούς πληρωμής προμηθευτή");if(!reason||reason.trim().length<3)return;setBusy(item.kind+item.id);setError("");try{
   await api(`/api/transactions/supplier-settlements/${item.id}/cancel`,{method:"POST",body:JSON.stringify({reason:reason.trim()})});
   setItems(rows=>rows.filter(row=>!(row.kind===item.kind&&row.id===item.id)));onChanged?.();
 }catch(e){setError(e.message||"Δεν ακυρώθηκε η πληρωμή.")}finally{setBusy("")}};
 const uploadProof=async(item,file)=>{if(!file)return;setBusy(`proof-${item.id}`);setError("");try{
   const result=await api(`/api/transactions/bank-ledger/${item.id}/attachment`,{method:"POST",body:JSON.stringify({attachment:{dataUrl:await read(file),filename:file.name},proofAmount:number(proofAmounts[item.id])||Number(item.amount)})});
   if(result.status==="CONFIRMED")setItems(rows=>rows.filter(row=>row.id!==item.id));
   else setItems(rows=>rows.map(row=>row.id===item.id?{...row,status:result.status,proofAmount:result.proofAmount,attachmentFilename:file.name}:row));
   onChanged?.();
 }catch(e){setError(e.message||"Δεν ανέβηκε το αποδεικτικό.")}finally{setBusy("")}};
 return <section style={{background:"#fff",border:"1px solid #dce5ef",borderRadius:18,padding:18,marginBottom:18,boxShadow:"0 8px 30px rgba(15,42,75,.05)"}}>
   ...
   {items.length?<div style={{display:"grid",gap:8}}>{items.map(item=><article key={item.kind+item.id} style={{display:"grid",gridTemplateColumns:"1fr auto",gap:12,alignItems:"center",padding:11,border:"1px solid #e1e8f0",borderRadius:12}}><div><b>{item.label}</b><small style={{display:"block",color:"#63738b",marginTop:3}}>{euro(item.amount)}{item.proofAmount!=null&&Math.abs(Number(item.proofAmount)-Number(item.amount))>.005?` · Απόκλιση ${euro(Number(item.proofAmount)-Number(item.amount))}`:""} · {new Date(item.paidAt||item.occurredAt||item.createdAt).toLocaleDateString("el-GR")} · {item.attachmentFilename||"Χωρίς συνημμένο"}{item.status==="DISCREPANCY"?" · Σε απόκλιση":""}</small></div><div style={{display:"flex",gap:7,alignItems:"center",flexWrap:"wrap",justifyContent:"flex-end"}}>{item.kind==="BANK"&&!item.attachmentFilename&&["PENDING_PROOF","PENDING_REVIEW"].includes(item.status)&&<><input inputMode="decimal" value={proofAmounts[item.id]??item.amount} onChange={e=>setProofAmounts(values=>({...values,[item.id]:e.target.value}))} aria-label={`Ποσό αποδεικτικού ${item.id}`} style={{width:88,padding:"7px"}}/><label style={{border:"1px solid #087f5b",borderRadius:9,padding:"7px 10px",color:"#087f5b",fontWeight:800,cursor:"pointer"}}><Upload size={15}/> Αποδεικτικό<input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" disabled={Boolean(busy)} style={{display:"none"}} onChange={e=>uploadProof(item,e.target.files?.[0])}/></label></>}<button type="button" disabled={Boolean(busy)} onClick={()=>review(item,"CONFIRMED")} style={{border:0,borderRadius:9,padding:"8px 10px",background:"#087f5b",color:"#fff",fontWeight:800}}><CheckCircle2 size={15}/> Επιβεβαίωση</button><button type="button" disabled={Boolean(busy)||item.status==="DISCREPANCY"} onClick={()=>review(item,"DISCREPANCY")} style={{border:"1px solid #e3b2ae",borderRadius:9,padding:"8px 10px",background:"#fff7f6",color:"#a62b24",fontWeight:800}}><AlertTriangle size={15}/> Απόκλιση</button>{item.kind==="SUPPLIER"&&<button type="button" disabled={Boolean(busy)} onClick={()=>cancelSupplier(item)} style={{border:"1px solid #ba453d",borderRadius:9,padding:"8px 10px",background:"#fff",color:"#9c2722",fontWeight:800}}>Ακύρωση εκκρεμούς πληρωμής</button>}</div></article>)}</div>:<div style={{padding:14,textAlign:"center",color:"#63738b",border:"1px dashed #cbd8e5",borderRadius:12}}>Δεν υπάρχουν εκκρεμείς επιβεβαιώσεις για αυτό το κατάστημα.</div>}
 </section>;
}
