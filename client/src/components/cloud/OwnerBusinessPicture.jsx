...
import {ChevronDown,ChevronRight,Download,Printer,RefreshCw,Search,X} from "lucide-react";
...
const columns=[
  ...
  ["netProfit","Καθ. κέρδος",money],["paymentTotal","Πληρωμές",money],["expenseSalesPercent","Έξοδα/Πωλήσεις",pct],["purchaseVat","ΦΠΑ αγορών",money]
];

export default function OwnerBusinessPicture({api,store,onClose}){
  const [from,setFrom]=useState(monthStart()),[to,setTo]=useState(iso(new Date())),[scope,setScope]=useState("ALL"),[data,setData]=useState(null),[busy,setBusy]=useState(false),[error,setError]=useState(""),[expanded,setExpanded]=useState("");
  ...
  return <div className="business-picture-overlay" onMouseDown={e=>e.target===e.currentTarget&&onClose()}>
    <section className="business-picture-dialog">
      <header><div><small>MYWORKSTATION · ΙΔΙΟΚΤΗΤΗΣ</small><h2>Εικόνα Επιχειρήσεις</h2><p>{store?.name||"Όλα τα καταστήματα"} · πραγματικά δεδομένα εμπορικής λειτουργίας</p></div><button onClick={onClose} aria-label="Κλείσιμο"><X/></button></header>
