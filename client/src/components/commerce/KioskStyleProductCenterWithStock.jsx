...
export default function KioskStyleProductCenterWithStock({api,stores=[]}){
 ...
 const checkedPreparationMaterials=()=>{const root=hostRef.current;if(!root)return[];return [...root.querySelectorAll(".kiosk-tr.items-v3:not(.head)")].map(row=>{const input=row.querySelector('input[type="checkbox"]');if(!input?.checked)return null;const sku=(row.children?.[2]?.textContent||"").trim();return bySku.get(sku)||null}).filter(p=>p&&isPrepMaterial(p))};
 ...
 return <div ref={hostRef} onClickCapture={onCapture}>
  <style>{`.mws-stock-pencil{border:0;background:transparent;font-weight:800;color:#15355d;cursor:pointer}.mws-bulk-preparation-btn{font-weight:900!important;background:#0e6d5a!important;color:white!important}.mws-bulk-manager{width:min(1220px,97vw);max-height:94vh;display:flex;flex-direction:column}.mws-bulk-body{padding:16px;overflow:auto}.mws-bulk-mode{display:flex;gap:10px;margin-bottom:14px}.mws-bulk-mode button{padding:10px 18px;font-weight:900}.mws-bulk-mode button.active{background:#0e6d5a;color:#fff}.mws-material-source{border:1px solid #b8d6ce;background:#eef9f5;padding:12px 14px;margin-bottom:12px}.mws-material-source b{display:block;margin-bottom:7px}.mws-material-chip{display:inline-block;margin:3px 6px 3px 0;padding:5px 9px;background:#fff;border:1px solid #bdd4ce;border-radius:14px;font-weight:800}.mws-bulk-grid{display:grid;grid-template-columns:1fr 1fr 1.5fr auto;gap:10px;margin:10px 0;align-items:end}.mws-bulk-grid select,.mws-bulk-grid input{width:100%;padding:10px}.mws-search-btn{height:40px;padding:0 20px;background:#174b70;color:#fff;border:0;border-radius:4px;font-weight:900;cursor:pointer}.mws-bulk-list{border:1px solid #d8dee6;max-height:360px;overflow:auto}.mws-bulk-list-head,.mws-bulk-row{display:grid;grid-template-columns:44px 120px 1fr 220px 220px;gap:8px;align-items:center;padding:8px 10px;border-bottom:1px solid #edf0f3}.mws-bulk-list-head{position:sticky;top:0;background:#f5f7fa;font-weight:900;z-index:1}.mws-bulk-selected{font-weight:900;margin:10px 0}.mws-bulk-empty{padding:18px;border:1px dashed #b8c3ce;background:#fafcfd;font-weight:800}.mws-bulk-status{flex:0 0 auto;padding:8px 16px;background:#fff;border-top:1px solid #e5e9ef}.mws-bulk-result{padding:10px;background:#ecfbf5;color:#096346;font-weight:900}.mws-bulk-error{padding:11px 12px;background:#fff0f0;border:1px solid #d77;color:#8e1d1d;font-weight:900}.mws-danger{background:#a12626!important;color:#fff!important}.mws-action-count{font-weight:900}@media(max-width:900px){.mws-bulk-grid{grid-template-columns:1fr}.mws-bulk-list-head,.mws-bulk-row{grid-template-columns:36px 1fr}.mws-bulk-list-head span:nth-child(n+3),.mws-bulk-row span:nth-child(n+3){display:none}}`}</style>
