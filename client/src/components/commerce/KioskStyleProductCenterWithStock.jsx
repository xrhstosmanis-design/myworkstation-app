...
export default function KioskStyleProductCenterWithStock({api,stores=[]}){
  ...
  useEffect(()=>{
    ...
    const decorate=()=>{
      ...
      rows.forEach(rowEl=>{
        ...
        const stockCell=cells[cells.length-1];if(stockCell.dataset.stockPencilReady==="1")return;
        const sku=(cells[2]?.textContent||"").trim();
        const product=bySku.get(sku);if(!product)return;
        stockCell.dataset.stockPencilReady="1";
        const currentText=stockCell.textContent.trim();
        stockCell.innerHTML="";
        const button=document.createElement("button");
        button.type="button";button.className="mws-stock-pencil";button.title="Διόρθωση Stock";
        button.innerHTML=`<span class="mws-stock-pencil-icon">✎</span><span>${currentText||"0"}</span>`;
        button.addEventListener("click",event=>{
          event.preventDefault();event.stopPropagation();
          const storeRows=Array.isArray(product.stores)?product.stores:[];
          const preferred=storeRows.find(s=>s.active!==false)||storeRows[0]||null;
          setTarget({product,storeId:preferred?.storeId||stores[0]?.id||""});setMode("SET");setQuantity("");setLogMovement(true);setError("");
        });
  ...
  const submit=async()=>{
    ...
    try{
      await api(`/api/owner-products/${target.product.id}/stock-adjustment`,{method:"POST",body:JSON.stringify({storeId:target.storeId,mode,quantity:Number(quantity),logMovement})});
      setTarget(null);setReloadKey(x=>x+1);
  ...
  return <div ref={hostRef}>
    ...
    {target&&<div className="kiosk-modal-backdrop"><div className="kiosk-modal mws-stock-dialog">
      <div className="kiosk-modal-title">Διόρθωση Stock είδους <button onClick={()=>setTarget(null)}>×</button></div>
      <p className="stock-product">{target.product.name}</p>
      {storeRows.length>1&&<label>Κατάστημα<select value={target.storeId} onChange={e=>setTarget(t=>({...t,storeId:e.target.value}))}>{storeRows.map(s=><option key={s.storeId} value={s.storeId}>{s.storeName}</option>)}</select></label>}
      <div className="mws-stock-current"><span>Τρέχον Stock</span><b>{currentStock}</b></div>
      <div className="mws-stock-modes">
        <label className="mws-stock-mode"><input type="radio" checked={mode==="SET"} onChange={()=>setMode("SET")}/><span><strong>(=) Ακριβώς την «ποσότητα»</strong><small>Η ποσότητα θα γίνει ακριβώς το stock της αποθήκης.</small></span></label>
        <label className="mws-stock-mode"><input type="radio" checked={mode==="ADD"} onChange={()=>setMode("ADD")}/><span><strong>(+) Αύξηση κατά την «ποσότητα»</strong><small>Η ποσότητα θα προστεθεί στο τρέχον stock.</small></span></label>
        <label className="mws-stock-mode"><input type="radio" checked={mode==="SUBTRACT"} onChange={()=>setMode("SUBTRACT")}/><span><strong>(−) Μείωση κατά την «ποσότητα»</strong><small>Η ποσότητα θα αφαιρεθεί από το τρέχον stock.</small></span></label>
      </div>
      <div className="mws-stock-fields"><label>Ποσότητα<input autoFocus type="number" min="0" step="0.001" value={quantity} onChange={e=>setQuantity(e.target.value)}/></label><label>Νέο Stock<input readOnly value={quantity===""?currentStock:calculated}/></label></div>
      <label className="mws-stock-log"><input type="checkbox" checked={logMovement} onChange={e=>setLogMovement(e.target.checked)}/> Καταχώρηση στο ημερολόγιο κινήσεων</label>
      {error&&<div className="kiosk-error">{error}</div>}
      <div className="mws-stock-preview">Η αλλαγή θα εφαρμοστεί μόνο στο επιλεγμένο κατάστημα και προϊόν.</div>
      <div className="kiosk-modal-actions"><button onClick={()=>setTarget(null)}>Επιστροφή</button><button className="primary" disabled={busy} onClick={submit}>{busy?"Καταχώρηση…":"Καταχώρηση"}</button></div>
    </div></div>}
  </div>;
}
