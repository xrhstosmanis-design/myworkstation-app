...
import {CloudDownload,Eye,Inbox,RefreshCw,RotateCw,Search} from "lucide-react";
...
export default function InvoiceInboxPanel({api,stores=[],onOpenAi}){
  ...
  const [myDataStatus,setMyDataStatus]=useState(null);
  ...
  const loadMyDataStatus=async()=>{if(!storeId)return;try{setMyDataStatus(await api(`/api/commerce/documents/mydata/status?storeId=${encodeURIComponent(storeId)}`))}catch{setMyDataStatus(null)}};
  const syncMyData=async automatic=>{if(!storeId||workingId==="MYDATA")return;setWorkingId("MYDATA");setError("");if(!automatic)setMessage("");try{const result=await api("/api/commerce/documents/mydata/sync",{method:"POST",body:JSON.stringify({storeId})});if(!automatic||result.created)setMessage(result.message);await Promise.all([load(),loadMyDataStatus()])}catch(err){if(!automatic)setError(err.message)}finally{setWorkingId("")}};
  useEffect(()=>{loadMyDataStatus();const timer=setInterval(()=>{if(myDataStatus?.enabled&&!myDataStatus?.labLocked)syncMyData(true)},10*60*1000);return()=>clearInterval(timer)},[storeId,myDataStatus?.enabled,myDataStatus?.labLocked]);
  ...
  return <div className="invoice-inbox">
    <section className="invoice-inbox-head"><div><h2><Inbox/>Θυρίδα Τιμολογίων / Αρχείο</h2><p>Τα παραστατικά myDATA μπαίνουν πρώτα ως πρόχειρα. Η αποθήκη ενημερώνεται μόνο μετά τον έλεγχο του πρωτοτύπου.</p>{myDataStatus&&<small className={`mydata-state ${myDataStatus.enabled&&!myDataStatus.labLocked?"ready":"locked"}`}>myDATA: {myDataStatus.enabled?(myDataStatus.labLocked?"κλειδωμένο — απαιτείται SANDBOX":"LAB/SANDBOX ενεργό"):"δεν έχει ενεργοποιηθεί"}{myDataStatus.lastSyncAt?` · τελευταίος έλεγχος ${when(myDataStatus.lastSyncAt)}`:""}</small>}</div><div><select value={storeId} onChange={e=>setStoreId(e.target.value)}>{stores.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select><button onClick={()=>syncMyData(false)} disabled={workingId==="MYDATA"||!myDataStatus?.enabled||myDataStatus?.labLocked}><CloudDownload/>{workingId==="MYDATA"?"Έλεγχος…":"Λήψη από myDATA"}</button><button onClick={load}><RefreshCw/>Ανανέωση</button></div></section>
