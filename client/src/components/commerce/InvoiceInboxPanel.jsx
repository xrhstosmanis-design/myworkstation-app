...
export default function InvoiceInboxPanel({api,stores=[]}){
  ...
  return <div className="invoice-inbox">
    ...
    <section className="commerce-box"><h3>Ανά προμηθευτή → ανά ημέρα</h3>{loading?<div>Φόρτωση…</div>:groups.length===0?<div className="invoice-empty">Δεν βρέθηκαν τιμολόγια με αυτά τα φίλτρα.</div>:<div className="invoice-supplier-groups">{groups.map(group=><section className="invoice-supplier-group" key={group.supplier}><header><b>{group.supplier}</b><span>{group.days.reduce((n,d)=>n+d.rows.length,0)} παραστατικά</span></header>{group.days.map(day=><div className="invoice-day-group" key={day.day}><h4>{day.day}<span>{day.rows.length}</span></h4><div className="invoice-list">{day.rows.map(item=><article key={item.id}><div className="invoice-main"><b>{item.filename||"Παραστατικό"}</b><small>{when(item.receivedAt)}</small>{item.responsibleName&&<small>Υπεύθυνος: {item.responsibleName}</small>}{item.supplierId&&<small>Κωδικός προμηθευτή: {item.supplierId}</small>}{item.note&&<p>{item.note}</p>}</div><div className="invoice-actions"><em className={item.status}>{labels[item.status]||item.status}</em>{item.hasAttachment&&<button onClick={()=>view(item)}><Eye/>Προβολή φωτογραφίας/PDF</button>}</div></article>)}</div></div>)}</section>)}</div>}</section>
  </div>;
}
