...
function setDomValue(input,value){
  if(input?.isContentEditable){input.textContent=value;input.dispatchEvent(new Event("input",{bubbles:true}));return}
...
function ensure(){
  ...
  root.addEventListener("pointerdown",event=>{
    ...
    if(key&&activeInput){event.preventDefault();event.stopPropagation();handle(key.dataset.key);return}
    if(event.target===root){event.preventDefault();close()}
  });
  return root;
}
function render(){
  ...
  if(isNumeric(activeInput)){
    root.innerHTML=`<section class="mws-touch-dialog"><header><div><small>MYWORKSTATION STANDARD POS</small><h2>Αριθμητικό πληκτρολόγιο</h2></div><button data-key="CLOSE" aria-label="Κλείσιμο">✕</button></header><main><div class="mws-touch-numeric">${["7","8","9","4","5","6","1","2","3",",","0","BACK"].map(k=>`<button data-key="${k}">${k==="BACK"?"⌫":k}</button>`).join("")}</div><div class="mws-touch-actions"><button data-key="MINUS">−</button><button data-key="CLEAR">Καθαρισμός</button><button class="enter" data-key="ENTER">Enter ↵</button></div></main></section>`;return;
  }
  ...
  root.innerHTML=`<section class="mws-touch-dialog"><header><div><small>MYWORKSTATION STANDARD POS</small><h2>Πληκτρολόγιο οθόνης <span>${lang==="EL"?"Ελληνικά":"English"}</span></h2></div><button data-key="CLOSE" aria-label="Κλείσιμο">✕</button></header><main><div class="mws-touch-number-row">${numbers.map(k=>`<button data-key="${k}">${k==="BACK"?"⌫":k}</button>`).join("")}</div>${sizes.map(line).join("")}<div class="mws-touch-row mws-touch-special"><button data-key="SHIFT">⇧</button><button data-key="LANG">${lang==="EL"?"EN":"ΕΛ"}</button><button data-key="@">@</button><button data-key="SPACE" class="space">κενό</button><button data-key=".">.</button><button data-key=",">,</button><button data-key="ENTER" class="enter">↵</button></div></main></section>`;
}
function insertText(text){
  const input=activeInput;if(!input)return;
  if(input.isContentEditable){const current=input.textContent||"";setDomValue(input,current+text);return}
  ...
  setDomValue(input,nextValue);try{input.setSelectionRange(next,next)}catch{}
}
function handle(key){
  ...
  if(key==="CLEAR"){setDomValue(activeInput,"");return}
  ...
  if(key==="BACK"){
    ...
    if(start!==end){setDomValue(input,value.slice(0,start)+value.slice(end));try{input.setSelectionRange(start,start)}catch{}}
    else if(start>0){setDomValue(input,value.slice(0,start-1)+value.slice(end));try{input.setSelectionRange(start-1,start-1)}catch{}}
    return;
  }
  if(key==="ENTER"){const input=activeInput;input.dispatchEvent(new KeyboardEvent("keydown",{key:"Enter",code:"Enter",bubbles:true}));if(input.tagName!=="TEXTAREA")close();return}
...
function open(input){if(!editable(input))return;activeInput=input;input.dataset.mwsTouchKeyboard="1";prepareInput(input);render();if(!activeButton)makeButton(input)}
