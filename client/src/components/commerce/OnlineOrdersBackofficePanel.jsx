...
const eventText=event=>{
  ...
  if(event.note==="AUTO_PRINT_REQUESTED")return "Αυτόματη εκτύπωση μετά την αποδοχή";
  if(event.note==="PRINT_REQUESTED")return "Επανεκτύπωση παραγγελίας";
      ...
      return <article className={`online-bo-order ${open?"open":""}`} key={order.id}>
        ...
        {open&&<div className="online-bo-detail">
          <div className="online-bo-tabs">
            ...
            <button className={detail==="events"?"active":""} onClick={()=>setDetail("events")}>Συμβάντα ({(order.events||[]).length})</button>
          </div>
          ...
          {detail==="events"&&<div className="online-bo-events">{(order.events||[]).map(event=><div key={event.id}><span></span><div><b>{eventText(event)}</b><small>{stamp(event.createdAt)}{event.employeeId?` · Εργαζόμενος ${event.employeeId}`:""}</small></div></div>)}</div>}
