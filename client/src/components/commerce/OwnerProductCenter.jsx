...
export default function OwnerProductCenter({api,stores=[],onOpenFullProduct}){
  ...
  const saveMasterAudienceDiscount=async()=>{
    clearStatus();
    if(!selectedMaster?.companyProductId)return setError("Ενεργοποίησε πρώτα το προϊόν στην εταιρεία.");
    ...
    setBusy(true);
    try{
      const result=await api("/api/owner-products/bulk-audience-discount",{method:"PUT",body:JSON.stringify({storeId:masterAudienceStoreId,productIds:[selectedMaster.companyProductId],audience:masterAudience,discountPercent})});
      setMessage(`Αποθηκεύτηκε έκπτωση ${discountPercent}% για το «${selectedMaster.name}» σε ${result.changed} προϊόν.`);
