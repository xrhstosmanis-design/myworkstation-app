...
export default function OwnerProductCenter({api,stores=[]}){
  ...
  const chooseProduct=row=>{
    setSelectedProduct(row);setEditBasePrice(String(Number(row.salePrice||0)));setEditVat(String(Number(row.vatRate||0)));setEditVatVerified(Boolean(row.vatVerified));
  ...
  const savePrices=async()=>{
    ...
    try{
      ...
      const payload={basePrice:base,vatRate:Number(editVat||0),vatVerified:editVatVerified,stores:activeStores.map(s=>({storeId:s.id,active:Boolean(priceStores[s.id]?.active),salePrice:priceStores[s.id]?.salePrice===""?base:Number(priceStores[s.id]?.salePrice)}))};
  ...
  const createPromotion=async event=>{
    ...
    try{
      ...
      event.currentTarget.reset();setMessage("Η προσφορά δημιουργήθηκε.");await loadPromotions();
    }catch(e){setError(e.message)}finally{setBusy(false)}
  };
