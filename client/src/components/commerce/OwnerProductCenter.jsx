...
const barcodeLooksValid=value=>/^\d{8,14}$/.test(String(value||"").trim());
const activeStoreSalePrices=product=>(product.stores||[]).filter(store=>store.active&&n(store.salePrice)>0).map(store=>n(store.salePrice));
...
const productQualityIssues=product=>{
  ...
  if(barcodes.some(row=>!barcodeLooksValid(row.barcode)))issues.push("Μη έγκυρο barcode");
  const numericBarcode=barcodes.find(row=>barcodeLooksValid(row.barcode))?.barcode;
  if(numericBarcode&&String(product.sku||"").includes(numericBarcode))issues.push("Barcode μέσα στο SKU");
  return issues;
};
...
export default function OwnerProductCenter({api,stores=[],onOpenFullProduct}){
  ...
  const catalogWithQuality=useMemo(()=>catalog.map(product=>({...product,qualityIssues:productQualityIssues(product)})),[catalog]);
  ...
  const saveMasterAudienceDiscount=async()=>{
    clearStatus();
    const productIds=masterDiscountMode==="single"?(selectedMaster?.companyProductId?[selectedMaster.companyProductId]:[]):Object.keys(selectedMasterDiscounts);
    if(!productIds.length)return setError(masterDiscountMode==="single"?"Ενεργοποίησε πρώτα το προϊόν στην εταιρεία.":"Επίλεξε ενεργά προϊόντα από τα αποτελέσματα αναζήτησης.");
    if(productIds.length>500)return setError("Μπορείς να αποθηκεύσεις έως 500 προϊόντα κάθε φορά.");
    if(!masterAudienceStoreId)return setError("Επίλεξε κατάστημα.");
    if(masterAudiencePercent.trim()==="")return setError("Συμπλήρωσε ποσοστό έκπτωσης.");
    const discountPercent=Number(masterAudiencePercent);
    if(!Number.isFinite(discountPercent)||discountPercent<0||discountPercent>100)return setError("Η έκπτωση πρέπει να είναι από 0% έως 100%.");
    if(masterDiscountMode==="bulk"&&!window.confirm(`Εφαρμογή έκπτωσης ${discountPercent}% σε ${productIds.length} επιλεγμένα προϊόντα;`))return;
    setBusy(true);
    try{
  ...
  const openProduct=row=>onOpenFullProduct?onOpenFullProduct({productId:row.id,name:row.name,sku:row.sku,storeId:(row.stores||[]).find(store=>store.active!==false)?.storeId||stores.find(store=>store.active!==false)?.id||"",currentStock:(row.stores||[]).find(store=>store.active!==false)?.currentStock||0}):chooseProduct(row);
  ...
  const saveProductCard=async()=>{
    ...
    try{
      ...
      const fresh=await api(`/api/owner-products/catalog?q=${encodeURIComponent(catalogQuery.trim())}`);setCatalog(fresh);const updated=fresh.find(row=>row.id===selectedProduct.id);if(updated)chooseProduct(updated);
    }catch(e){setError(e.message)}finally{setBusy(false)}
  };
  ...
  return <div className="owner-products">
    ...
    {tab==="master"&&<div className="op-grid two"><section className="op-box"><h3>Αναζήτηση 26.656 προϊόντων</h3><form className="op-search" onSubmit={searchMaster}><input value={masterQuery} onChange={e=>setMasterQuery(e.target.value)} placeholder="Περιγραφή, κωδικός ή barcode"/><button disabled={busy}><Search/>Αναζήτηση</button></form><div className="op-list">{masterResults.map(row=><div key={row.id} className="op-master-result"><label className="op-master-select" title={row.companyProductId?"Επιλογή για μαζική έκπτωση":"Ενεργοποίησε πρώτα το προϊόν"}><input type="checkbox" disabled={!row.companyProductId} checked={Boolean(row.companyProductId&&selectedMasterDiscounts[row.companyProductId])} onChange={e=>setSelectedMasterDiscounts(current=>{const next={...current};if(e.target.checked)next[row.companyProductId]={name:row.name,sourceCode:row.sourceCode};else delete next[row.companyProductId];return next})}/></label><button className={`op-product ${selectedMaster?.id===row.id?"selected":""}`} onClick={()=>chooseMaster(row)}><span><b>{row.name}</b><small>{row.sourceCode} · {row.categoryName||"Χωρίς κατηγορία"}</small></span><span>{row.companyProductId?<em className="ok">ΕΝΕΡΓΟ</em>:<em>MASTER</em>}<small>{money(row.defaultRetailPrice)}</small></span></button></div>)}</div><p className="op-master-selection">{Object.keys(selectedMasterDiscounts).length} ενεργά προϊόντα επιλεγμένα για μαζική έκπτωση</p></section>{selectedMaster?<aside className="op-box"><h3>{selectedMaster.name}</h3><p>Βασική λιανική Master: <b>{money(selectedMaster.defaultRetailPrice)}</b></p><label>Βασική τιμή εταιρείας<input type="number" step="0.01" min="0" value={basePrice} onChange={e=>setBasePrice(e.target.value)}/></label><div className="store-price-list"><h4>Καταστήματα</h4>{activeStores.map(store=><div className="store-price" key={store.id}><label className="check"><input type="checkbox" checked={Boolean(activationStores[store.id]?.active)} onChange={e=>setActivationStores(c=>({...c,[store.id]:{...c[store.id],active:e.target.checked}}))}/><Store/>{store.name}</label><input type="number" step="0.01" min="0" value={activationStores[store.id]?.salePrice??""} onChange={e=>setActivationStores(c=>({...c,[store.id]:{...c[store.id],salePrice:e.target.value}}))} placeholder="Τιμή"/></div>)}</div>{selectedMaster.vatVerified?<div className="op-alert success">ΦΠΑ Master επιβεβαιωμένος: {n(selectedMaster.vatRate)}%</div>:<div className="op-alert warning">Ο ΦΠΑ δεν είναι επιβεβαιωμένος. Το προϊόν θα ενεργοποιηθεί χωρίς αυθαίρετη φορολογική τιμή.</div>}<button className="primary" onClick={activateMaster} disabled={busy}><Check/>Ενεργοποίηση / ενημέρωση προϊόντος</button>{selectedMaster.companyProductId&&<div className="op-box op-form"><h4>Έκπτωση δικαιούχου</h4><div className="op-discount-mode"><button type="button" className={masterDiscountMode==="single"?"active":""} onClick={()=>setMasterDiscountMode("single")}>Ένα προϊόν</button><button type="button" className={masterDiscountMode==="bulk"?"active":""} onClick={()=>setMasterDiscountMode("bulk")}>Μαζικά ({Object.keys(selectedMasterDiscounts).length})</button></div><p>{masterDiscountMode==="single"?`${selectedMaster.name} · ${selectedMaster.sourceCode} · ένα προϊόν`:`${Object.keys(selectedMasterDiscounts).length} ενεργά προϊόντα επιλεγμένα από τη λίστα αριστερά`}</p><label>Κατάστημα<select value={masterAudienceStoreId} onChange={e=>setMasterAudienceStoreId(e.target.value)}>{activeStores.map(store=><option key={store.id} value={store.id}>{store.name}</option>)}</select></label><label>Δικαιούχος<select value={masterAudience} onChange={e=>setMasterAudience(e.target.value)}><option value="DOCTOR">Ιατροί</option><option value="NURSE">Νοσηλευτές / Νοσοκόμοι</option><option value="STAFF">Προσωπικό</option><option value="CUSTOMER">Πελάτες</option></select></label><label>Έκπτωση %<input type="number" min="0" max="100" step="0.01" value={masterAudiencePercent} onChange={e=>setMasterAudiencePercent(e.target.value)}/></label><button type="button" className="primary" disabled={busy||!masterAudienceStoreId||masterAudiencePercent.trim()===""||(masterDiscountMode==="bulk"&&!Object.keys(selectedMasterDiscounts).length)} onClick={saveMasterAudienceDiscount}>Αποθήκευση έκπτωσης για {masterDiscountMode==="single"?"1 προϊόν":`${Object.keys(selectedMasterDiscounts).length} προϊόντα`}</button><small>Το 0% απενεργοποιεί την έκπτωση. Η βασική τιμή λιανικής παραμένει ίδια.</small></div>}</aside>:<aside className="op-box empty">Επίλεξε προϊόν από τον Master Catalog.</aside>}</div>}
