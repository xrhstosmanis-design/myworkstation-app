...
export default function BackofficeVideoAuditPanel({api,storeId}){
 ...
 const installer=async()=>{try{setError("");const created=await api(`/api/video-admin/stores/${encodeURIComponent(storeId)}/installer`,{method:"POST",body:JSON.stringify({minutes:60})});window.location.href=created.downloadUrl}catch(e){setError(e.message)}};
