...
export default function StoreSupplierInvoicePremiumFast({api,store,suppliers=[],onChanged,setMessage}){
  ...
  const submit=async()=>{
    ...
    try{
      ...
      const success=mode==="PAID"?`✅ Πληρωμή ${totalGross.toFixed(2)} € με ${paymentMethodLabel} καταχωρίστηκε. Το τιμολόγιο διαβάζεται στο παρασκήνιο.`:`Το τιμολόγιο ${documentNumber.trim()} διαβάζεται στο παρασκήνιο.`;
      ...
      backgroundV244({api,store,pages,supplierId,documentNumber:documentNumber.trim(),documentDate,totalGross,mode,paymentTransactionId,resumeJobId:duplicateCheck?.resumeJobId||null,queuedJobs:Array.isArray(handoff?.jobs)?handoff.jobs:[],onProgress:setStatus})
        .then(created=>{const review=Boolean(created?.reconciliationRequired);const difference=Number(created?.reconciliationDifference||0);setStatus(review?`⚠️ Το ενιαίο τιμολόγιο μεταφέρθηκε ως ΠΡΟΧΕΙΡΟ για έλεγχο BackOffice: διαφορά ${difference.toFixed(2)} €. Η αποθήκη δεν ενημερώθηκε.${mode==="PAID"?" Η πληρωμή έχει ήδη επαναχρησιμοποιηθεί.":""}`:`✅ Το ενιαίο τιμολόγιο καταχωρίστηκε από ${created?.pageCount||pages.length} ${pages.length===1?"σελίδα":"σελίδες"}, με ${created?.lineCount||0} γραμμές στην αρχική τους σειρά, και μετά αρχειοθετήθηκε στη Θυρίδα.`);setMessage?.(review?`⚠️ Τιμολόγιο ${documentNumber.trim()}: αποθηκεύτηκε ως ΠΡΟΧΕΙΡΟ. Χρειάζεται έλεγχο στο BackOffice.`:`✅ Τιμολόγιο ${documentNumber.trim()}: καταχωρίστηκε και αρχειοθετήθηκε (${pages.length} ${pages.length===1?"σελίδα":"σελίδες"}).`);onChanged?.()})
