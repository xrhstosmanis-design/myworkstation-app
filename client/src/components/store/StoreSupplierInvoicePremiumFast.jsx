...
export default function StoreSupplierInvoicePremiumFast({api,store,suppliers=[],onChanged,setMessage}){
  ...
  const submit=async()=>{
    ...
    try{
      ...
      const success=mode==="PAID"?`ℹ️ Η πληρωμή ${totalGross.toFixed(2)} € με ${paymentMethodLabel} διατηρήθηκε. Περίμενε να ολοκληρωθεί η καταχώριση του τιμολογίου.`:`ℹ️ Το τιμολόγιο ${documentNumber.trim()} διαβάζεται. Περίμενε να ολοκληρωθεί η καταχώριση.`;
      ...
      backgroundV244({api,store,pages,supplierId,documentNumber:documentNumber.trim(),documentDate,totalGross,mode,paymentTransactionId,resumeJobId:duplicateCheck?.resumeJobId||null,queuedJobs:Array.isArray(handoff?.jobs)?handoff.jobs:[],onProgress:setStatus})
        .then(created=>{const review=Boolean(created?.reconciliationRequired);const difference=Number(created?.reconciliationDifference||0),archived=created?.archived!==false;setBusy(false);setStatus(review?`⚠️ Το ενιαίο τιμολόγιο μεταφέρθηκε ως ΠΡΟΧΕΙΡΟ για έλεγχο BackOffice: διαφορά ${difference.toFixed(2)} €. Η αποθήκη δεν ενημερώθηκε.${mode==="PAID"?" Η πληρωμή έχει ήδη επαναχρησιμοποιηθεί.":""}`:archived?`✅ Το ενιαίο τιμολόγιο καταχωρίστηκε από ${created?.pageCount||pages.length} ${pages.length===1?"σελίδα":"σελίδες"}, με ${created?.lineCount||0} γραμμές στην αρχική τους σειρά, και αρχειοθετήθηκε στη Θυρίδα.`:`✅ Το ενιαίο τιμολόγιο καταχωρίστηκε ως ΠΡΟΧΕΙΡΟ με ${created?.lineCount||0} γραμμές. Η αρχειοθέτηση των φωτογραφιών εκκρεμεί, αλλά η αγορά δεν χάθηκε.`);setMessage?.(review?`⚠️ Τιμολόγιο ${documentNumber.trim()}: αποθηκεύτηκε ως ΠΡΟΧΕΙΡΟ. Χρειάζεται έλεγχο στο BackOffice.`:archived?`✅ Τιμολόγιο ${documentNumber.trim()}: καταχωρίστηκε και αρχειοθετήθηκε (${pages.length} ${pages.length===1?"σελίδα":"σελίδες"}).`:`✅ Τιμολόγιο ${documentNumber.trim()}: καταχωρίστηκε ως ΠΡΟΧΕΙΡΟ. Η αρχειοθέτηση φωτογραφιών εκκρεμεί.`);onChanged?.()})
        .catch(error=>{setBusy(false);setStatus(`⚠️ Δεν ολοκληρώθηκε η καταχώριση τιμολογίου. Η πληρωμή διατηρήθηκε και μπορείς να πατήσεις ξανά μόνο την καταχώριση για ασφαλή επανάληψη. ${error?.message||error}`);setMessage?.(`⚠️ Η πληρωμή διατηρήθηκε. Η καταχώριση τιμολογίου χρειάζεται ασφαλή επανάληψη: ${error?.message||error}`)});
    }catch(error){
