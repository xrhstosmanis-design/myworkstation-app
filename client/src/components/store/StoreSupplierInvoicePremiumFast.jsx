...
async function backgroundV244({api,store,pages,supplierId,documentNumber,documentDate,totalGross,mode,paymentTransactionId,resumeJobId=null,onProgress}){
  ...
  try{
    ...
    for(const [pageIndex,page] of pages.entries()){
      ...
      if(!pageLines.length)throw new Error(`Δεν βρέθηκαν ασφαλείς γραμμές προϊόντων στη σελίδα ${pageIndex+1}.`);
      combinedLines.push(...pageLines);
    }
    stage="Επικύρωση γραμμών προϊόντων";
    if(!combinedLines.length)throw new Error("Δεν βρέθηκαν ασφαλείς γραμμές προϊόντων.");
  ...
  const submit=async()=>{
    ...
    try{
      ...
      backgroundV244({api,store,pages,supplierId,documentNumber:documentNumber.trim(),documentDate,totalGross,mode,paymentTransactionId,resumeJobId:duplicateCheck?.resumeJobId||null,onProgress:setStatus})
        .then(created=>{setStatus(`✅ Το τιμολόγιο καταχωρίστηκε από ${created?.pageCount||pages.length} ${pages.length===1?"σελίδα":"σελίδες"}, με ${created?.lineCount||0} γραμμές, και μετά αρχειοθετήθηκε στη Θυρίδα.`);setMessage?.(`✅ Το τιμολόγιο ${documentNumber.trim()} καταχωρίστηκε στις Παραγγελίες & Αγορές από ${pages.length} ${pages.length===1?"σελίδα":"σελίδες"} και αρχειοθετήθηκε στη Θυρίδα.`);onChanged?.()})
