...
export default function StoreSupplierOpenInvoicePayment({api,store,suppliers=[],onChanged,setMessage,allowCash=true}){
  const [supplierId,setSupplierId]=useState(""),[items,setItems]=useState([]),[allocations,setAllocations]=useState({}),[paymentMethod,setPaymentMethod]=useState("CASH_SHIFT"),[file,setFile]=useState(null),[paidAt,setPaidAt]=useState(new Date().toISOString().slice(0,10)),[note,setNote]=useState(""),[feedback,setFeedback]=useState(null),[loading,setLoading]=useState(false),[busy,setBusy]=useState(false),[cameraOpen,setCameraOpen]=useState(false),[stream,setStream]=useState(null);
  ...
  const chooseFile=async next=>{if(!next)return;setBusy(true);try{const prepared=await prepareDocumentFile(next,{strict:true,maxSide:3000,enhance:true});setFile(prepared.file);if(prepared.quality)setMessage?.("✅ Το αποδεικτικό καθαρίστηκε, ισιώθηκε και πέρασε τον έλεγχο ευκρίνειας.")}catch(error){setFile(null);setMessage?.(`❌ ${error?.message||"Η εικόνα δεν πέρασε τον έλεγχο ποιότητας."}`)}finally{setBusy(false)}};
  ...
  const submit=async()=>{
    ...
    try{
      ...
      const success=`✅ Πληρωμή ${euro(total)} για ${selected.name} ${result?.status==="CONFIRMED"?"επιβεβαιώθηκε ως πληρωμή ιδιοκτήτη":"καταχωρίστηκε σε αναμονή ελέγχου"}. Δεν αλλάζει δεύτερη φορά κανένα τιμολόγιο ή απόθεμα.`;setFeedback({ok:true,text:success});setMessage?.(success);
      setItems([]);setAllocations({});setFile(null);setNote("");onChanged?.();
    }catch(error){const failure=`❌ ${error?.message||"Δεν καταχωρίστηκε η πληρωμή."}`;setFeedback({ok:false,text:failure});setMessage?.(failure)}finally{setBusy(false)}
  };
  return <div className="pos-payment-form-v3-root">
    ...
    {supplierId&&<div className="supplier-open-invoices">{loading?<div style={{padding:12,fontWeight:800}}>Φόρτωση ανοιχτών τιμολογίων…</div>:items.length?<>{items.map(item=><div key={item.id} className="supplier-open-invoice-row"><div><b>{item.documentNumber||"Χωρίς αριθμό"}</b><small style={{display:"block"}}>{String(item.documentDate||"").slice(0,10)} · {item.storeName} · Υπόλοιπο {euro(item.outstandingAmount)}</small></div><input className="supplier-open-invoice-amount" aria-label={`Ποσό για ${item.documentNumber||item.id}`} inputMode="decimal" value={allocations[item.id]||""} max={item.outstandingAmount} placeholder="0,00" disabled={busy} onChange={event=>setAllocations(current=>({...current,[item.id]:event.target.value}))}/></div>)}<div style={{padding:10,fontWeight:900,textAlign:"right"}}>Σύνολο επιλεγμένης πληρωμής: {euro(total)}</div></>:<div style={{padding:12,fontWeight:800}}>Δεν υπάρχουν απλήρωτα τιμολόγια με πίστωση για αυτόν τον προμηθευτή.</div>}</div>}
    {Boolean(items.length)&&<>
      <div className="pos-expense-payment-sources"><b>Τρόπος πληρωμής</b><div>{[["CASH_SHIFT","Μετρητά από ενεργή βάρδια"],["CORPORATE_CARD","Εταιρική κάρτα"],["BANK_TRANSFER","Τραπεζική μεταφορά"],["EMPLOYEE_REIMBURSEMENT","Πληρωμή υπαλλήλου προς επιστροφή"]].map(([value,label])=><button key={value} type="button" className={paymentMethod===value?"active":""} disabled={busy||(value==="CASH_SHIFT"&&!allowCash)} onClick={()=>setPaymentMethod(value)}>{label}</button>)}</div><small>{paymentMethod==="CASH_SHIFT"?"Μόνο αυτή η επιλογή αφαιρεί άμεσα το ποσό από την ενεργή βάρδια.":"Η καταχώριση είναι εξωτερική· το φυσικό ταμείο της βάρδιας δεν αλλάζει."}</small></div>
      {["CORPORATE_CARD","BANK_TRANSFER"].includes(paymentMethod)&&<small>Η κίνηση καταχωρίζεται στο ενιαίο εικονικό «Ταμείο Τράπεζας» του καταστήματος. Δεν γίνεται σύνδεση με πραγματικό τραπεζικό λογαριασμό.</small>}
    </>}
    <label>Ημερομηνία πραγματικής πληρωμής<input type="date" value={paidAt} disabled={busy} onChange={event=>setPaidAt(event.target.value)}/></label><label>Σημείωση <small>(προαιρετική)</small><input value={note} disabled={busy} onChange={event=>setNote(event.target.value)} placeholder="π.χ. αριθμός μεταφοράς"/></label><div className="pos-photo-actions"><strong><Upload/> Αποδεικτικό πληρωμής</strong><div><label><Camera/> Φωτογραφία από PC<input type="file" accept="image/jpeg,image/png,image/webp" disabled={busy} onChange={event=>chooseFile(event.target.files?.[0]||null)}/></label><label><FileText/> PDF από PC<input type="file" accept="application/pdf" disabled={busy} onChange={event=>chooseFile(event.target.files?.[0]||null)}/></label><button type="button" disabled={busy} onClick={startCamera}><Camera/> Λήψη από κάμερα</button></div><b>{file?.name||"Δεν επιλέχθηκε αποδεικτικό"}</b></div>{feedback&&<div role="status" style={{padding:"10px 12px",borderRadius:8,background:feedback.ok?"#e7f8ef":"#fee2e2",color:feedback.ok?"#08783e":"#991b1b",fontWeight:800}}>{feedback.text}</div>}{cameraOpen&&<div className="pos-camera-live"><video ref={videoRef} autoPlay playsInline/><canvas ref={canvasRef} hidden/><div><button type="button" onClick={capture}>Φωτογράφιση</button><button type="button" onClick={stopCamera}>Κλείσιμο κάμερας</button></div></div>}<button className="pos-primary-action" disabled={busy||!total||!file} onClick={submit}><Wallet/> {busy?"Καταχώριση…":"Καταχώριση πληρωμής προς έλεγχο"}</button>
  </div>;
}
