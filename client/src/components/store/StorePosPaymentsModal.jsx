...
export default function StorePosPaymentsModal({api,store,onClose,onChanged,setMessage,setError}){
 ...
 const [busy,setBusy]=useState(false),[ledger,setLedger]=useState(null),[access,setAccess]=useState(null),[expenseCategories,setExpenseCategories]=useState([]),[premiumInvoice,setPremiumInvoice]=useState(null),[cameraOpen,setCameraOpen]=useState(false),[stream,setStream]=useState(null),[localError,setLocalError]=useState("");const videoRef=useRef(null),canvasRef=useRef(null);
 const [form,setForm]=useState({type:initialType,amount:"",description:"",expenseCategoryId:"",subtractFromShift:true,file:null});
 const setPaymentType=type=>{paymentTabByStore.set(storeKey,type);setLocalError("");setForm(c=>({...c,type}))};
 const fail=message=>{setLocalError(message);setError?.(message)};
 const stopCamera=()=>{stream?.getTracks?.().forEach(t=>t.stop());setStream(null);setCameraOpen(false)};
 useEffect(()=>{
  api(`/api/transactions/stores/${store.id}/overview`).then(setLedger).catch(e=>fail(e.message));
  api(`/api/store-pos/stores/${store.id}`).then(result=>setAccess(result?.access||{})).catch(e=>fail(e.message));
 ...
 const startCamera=async()=>{try{setLocalError("");stopCamera();const s=await navigator.mediaDevices.getUserMedia({video:{facingMode:"environment"},audio:false});setStream(s);setCameraOpen(true);setTimeout(()=>{if(videoRef.current)videoRef.current.srcObject=s},0)}catch{fail("Δεν μπόρεσε να ανοίξει η κάμερα. Έλεγξε την άδεια κάμερας του browser.")}};
 const capture=()=>{const v=videoRef.current,c=canvasRef.current;if(!v||!c)return;c.width=v.videoWidth||1280;c.height=v.videoHeight||720;c.getContext("2d").drawImage(v,0,0,c.width,c.height);c.toBlob(blob=>{if(!blob)return;setLocalError("");setForm(x=>({...x,file:new File([blob],`parastatiko-${Date.now()}.jpg`,{type:"image/jpeg"})}));stopCamera()},"image/jpeg",.9)};
 const submitOther=async()=>{setLocalError("");if(!canOther)return fail("Δεν έχεις δικαίωμα Πληρωμής προς Τρίτους από το BackOffice.");const amount=num(form.amount);if(amount<=0)return fail("Βάλε ποσό πληρωμής.");if(expenseCategories.length&&!form.expenseCategoryId)return fail("Επίλεξε κατηγορία εξόδου.");if(!form.file)return fail("Για καταχώριση Λοιπού Εξόδου χρειάζεται φωτογραφία ή αρχείο παραστατικού. Πάτησε «Λήψη από κάμερα» ή «Επιλογή αρχείου».");const category=expenseCategories.find(x=>x.id===form.expenseCategoryId);const description=[category?.description,form.description].filter(Boolean).join(" — ")||null;setBusy(true);try{const dataUrl=await fileToDataUrl(form.file);await api(`/api/transactions/stores/${store.id}`,{method:"POST",body:JSON.stringify({type:"OTHER_EXPENSE",amount,description,subtractFromShift:Boolean(form.subtractFromShift),attachment:{dataUrl,filename:form.file.name||"parastatiko.jpg"}})});setLocalError("");setError?.("");setMessage(`Το έξοδο ${euro(amount)}${category?.description?` (${category.description})`:""} καταχωρίστηκε ${form.subtractFromShift?"στη βάρδια":"ως εξωτερική πληρωμή"} και στο BackOffice.`);onChanged?.();onClose()}catch(e){fail(e.message)}finally{setBusy(false)}};
 ...
 const blockPaidWithoutPermission=e=>{if(canSameShift)return;const button=e.target.closest?.("button");if(!button)return;const text=String(button.textContent||"").toLocaleUpperCase("el-GR");if(text.includes("ΠΛΗΡΩΜΕΝΟ")||text.includes("ΠΛΗΡΩΜΗ &")){e.preventDefault();e.stopPropagation();fail("Δεν έχεις δικαίωμα «Οι πληρωμές να αφαιρούνται από την ίδια βάρδια» από το BackOffice.")}};
 return <div className="pos-standard-modal" onMouseDown={e=>e.target===e.currentTarget&&!busy&&onClose()}><section><header><div><small>MYWORKSTATION STANDARD POS</small><h2>Πληρωμές</h2></div><button onClick={()=>!busy&&onClose()}><X/></button></header><main><div data-invoice-v244="1">
  {localError&&<div style={{marginBottom:12,padding:"12px 14px",borderRadius:10,background:"#fff0f0",border:"1px solid #d94848",color:"#a61b1b",fontWeight:900}}>{localError}</div>}
  ...
  {supplierMode?<div onClickCapture={blockPaidWithoutPermission}>{!canSameShift&&<div style={{padding:"8px 10px",marginBottom:8,borderRadius:8,background:"#fff5df",fontWeight:800}}>Δεν επιτρέπεται πληρωμή από τα μετρητά της βάρδιας. Μπορείς να καταχωρίσεις το τιμολόγιο με πίστωση.</div>}{supplierInvoice}</div>:<div className="pos-payment-form">
   {expenseCategories.length>0&&<label>Κατηγορία εξόδου<select value={form.expenseCategoryId} onChange={e=>{setLocalError("");setForm(c=>({...c,expenseCategoryId:e.target.value}))}}><option value="">— Επιλογή κατηγορίας —</option>{expenseCategories.map(category=><option key={category.id} value={category.id}>{category.description}</option>)}</select></label>}
   <label>Ποσό<input readOnly inputMode="decimal" value={form.amount}/></label><Pad value={form.amount} onChange={amount=>{setLocalError("");setForm(c=>({...c,amount}))}}/><label>Παρατηρήσεις<input value={form.description} onChange={e=>{setLocalError("");setForm(c=>({...c,description:e.target.value}))}}/></label>
   <div className="pos-photo-actions"><button type="button" onClick={startCamera}><Camera/> Λήψη από κάμερα</button><label><Camera/> Επιλογή αρχείου<input type="file" accept="image/*,application/pdf" onChange={e=>{setLocalError("");setForm(c=>({...c,file:e.target.files?.[0]||null}))}}/></label><b>{form.file?.name||"Δεν επιλέχθηκε φωτογραφία"}</b></div>
