...
export default function StorePosPanel({api,store,onChanged}){
  ...
  const quickProduct=button=>products.find(p=>p.name.toLocaleLowerCase("el-GR").includes(String(button.productQuery||button.label).toLocaleLowerCase("el-GR"))||String(p.sku||"")===String(button.productQuery||""));
  ...
  return <article className="store-pos-card" style={{"--pos-head":layout.theme?.headerColor||"#033d2f","--pos-accent":layout.theme?.accentColor||"#087a52"}}>
    ...
    <div className="store-pos-body">
      <aside className="store-pos-quick"><h3>ΓΡΗΓΟΡΑ</h3>{(layout.quickKeys||[]).filter(x=>x.visible).map(button=>{const product=quickProduct(button);return <button key={button.id} style={{background:button.color}} disabled={!product} onClick={()=>product&&add(product)}>{button.label}<small>{product?euro(product.salePrice):"Δεν έχει συνδεθεί"}</small></button>})}</aside>
