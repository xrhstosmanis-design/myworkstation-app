...
export default function KatInvoicePreview({api,store}){
  ...
  return <div style={{border:"2px solid #4b7f6b",borderRadius:10,padding:12,marginTop:12,background:"#f7fffb"}} data-kat-invoice-preview="true">
    ...
    {result&&<div style={{marginTop:12}}>
      ...
      <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}><thead><tr>{["#","Κωδικός","Περιγραφή","Ποσότητα","Μονάδα","Τεμ./συσκ.","Τιμή","Καθαρή αξία","ΦΠΑ","Μικτή αξία","Confidence"].map(h=><th key={h} style={{textAlign:"left",padding:6,borderBottom:"1px solid #ccd9d4"}}>{h}</th>)}</tr></thead><tbody>{lines.map((line,index)=><tr key={`${index}-${line.code||line.description||"line"}`}><td style={{padding:6}}>{index+1}</td><td style={{padding:6}}>{line.code||"—"}</td><td style={{padding:6,fontWeight:700}}>{line.description||"—"}</td><td style={{padding:6}}>{Number(line.quantity||0)}</td><td style={{padding:6}}>{line.unit||"—"}</td><td style={{padding:6}}>{Number(line.unitsPerPackage||0)}</td><td style={{padding:6}}>{money(line.unitCost)}</td><td style={{padding:6}}>{money(line.netAmount)}</td><td style={{padding:6}}>{Number(line.vatRate||0)}%</td><td style={{padding:6}}>{money(line.grossAmount)}</td><td style={{padding:6}}>{Number(line.confidence||0)}%</td></tr>)}</tbody></table></div>
