...
const groupNameOf=(g,index)=>String(g?.name??g?.label??g?.description??`Ομάδα ${index+1}`);
...
function DynamicField({name,spec,value,onChange,required}){
  ...
  return <label className="netlink-field"><span>{label}{required?" *":""}</span><input inputMode={inputMode} value={valueForInput(spec,value)} onChange={e=>onChange(e.target.value)} placeholder={spec?.example!==undefined?String(spec.example):""}/></label>;
}
