...
const setNative=(el,value)=>{const set=Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype,'value')?.set;if(set)set.call(el,value);else el.value=value;el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}));};
...
function applyMatch(panel,match){
  ...
  if(String(match.select.value)!==String(match.option.value))setNative(match.select,match.option.value);
  ...
  window.dispatchEvent(new CustomEvent('mws-invoice-supplier-matched',{detail:{supplierId:match.option.value,supplierName:supplierText}}));
  return true;
}
