...
## 2026-09-23 follow-up: bounded reread evidence — AWAITING LAB

- The signed-in LAB POS was visible, but the exact ΒΒ 6529 job's raw provider output was not available in its current screen. Automatic review again refused a broad administrative DOM snapshot because it could reveal unrelated tenants or employees. The investigation did not bypass that refusal. Original failure and the unsafe draft remain LAB FAIL.
- For a POS job whose next parsed result explicitly requires a complete printed-table check, the successful full-reread result now retains three bounded snapshots in its existing tenant-scoped job JSON: previous original product rows, provider product rows before recovery, and final reread product rows. Each snapshot contains at most 100 rows and 320 characters of each raw row. It does not change quantity, price, draft, approval, stock or payment logic. It does not capture provider calls that fail before returning rows; the preexisting failed ΒΒ 6529 reread cannot be reconstructed by this change.
- Unit checks establish preserved snapshots and no evidence copied to a different profile or a non-POS flow. A fresh LAB reread and a genuinely new invoice submission remain necessary to identify the faulty stage and establish acceptance. This is diagnostic instrumentation, not an OCR fix.
