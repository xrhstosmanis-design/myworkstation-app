...
- Active bounded fix: recover a mixed VAT allocation only when the current printed footer has exact per-rate taxable/VAT/gross equations, all line taxable bases match the footer, and one unique minimum-change allocation exists. A cent-level VAT rounding residual is reconciled only inside its printed rate group. **LAB NOT TESTED** until CI, merge and exact deploy.
