## 2026-09-25 — Gate 3 γενικό σύνολο FAST από τυπωμένη ανάλυση ΦΠΑ — AWAITING CI + LAB

- [ ] PANINI 13461 είχε πρώτη επιλογή 70,36 € από απόδειξη είσπραξης αντί πληρωτέου τιμολογίου 75,60 €. Ο αυστηρός υπάρχων κανόνας ΜΑΝΤΖΙΛΑΣ για τυπωμένο `ΣΥΝΟΛΑ` με καθαρό + ΦΠΑ = μικτό επεκτείνεται στην αρχική POS FAST ανάγνωση κάθε προμηθευτή, μόνο όταν η τριπλή αριθμητική απόδειξη βρίσκεται στην πρωτότυπη Azure μεταγραφή· διαφορετικά μένει η παλιά διαδρομή. Συνθετικό regression για ξεχωριστή απόδειξη, προστασία παλιού ΜΑΝΤΖΙΛΑΣ· δεν είναι γνήσιο LAB PASS. Δεν τροποποιεί τα υπάρχοντα πρόχειρα ή το λανθασμένο PANINI. Checkpoint: `CHECKPOINTS/CHANGES/2026-09-25-gate3-generic-fast-vat-footer-awaiting-lab.md`.
...
## 2026-09-25 — Gate 4 αντίγραφο NON_FISCAL πώλησης — LAB PASS προεπισκόπησης / υπόλοιπα PENDING

- [ ] PR #1188: CI #3015 PASS, merge `54deda3f5b974b1c87a082af1430c97245d7348f`. Το δημόσιο Render `/api/health` επέστρεψε ακριβώς αυτό το revision στις 25/09. Η προεπισκόπηση και η ορατότητα της υπάρχουσας εγγραφής στο κεντρικό Audit έχουν περιορισμένο πραγματικό PASS· μόνο η έξοδος σε χαρτί μένει ανεπιβεβαίωτη. Χρησιμοποιείται μόνο η υπάρχουσα πώληση POS02 23:22:35, χωρίς νέα πώληση.
- [ ] Στις «Συναλλαγές βάρδιας» προστέθηκε κουμπί «Εκτύπωση αντιγράφου NON_FISCAL» μόνο σε ήδη καταχωρισμένες πωλήσεις της ορατής ενεργής βάρδιας. Πρώτα καταγράφει `NON_FISCAL_COPY_PRINT_REQUEST` στο Audit, έπειτα δημιουργεί αντίγραφο από τα αποθηκευμένα στοιχεία πώλησης με σαφή ένδειξη ότι δεν είναι φορολογική απόδειξη. Δεν καλεί checkout, δεν ανοίγει βάρδια, δεν αλλάζει stock/ταμείο και δεν εκτυπώνει μέσω fiscal bridge. **LAB PASS μόνο για το περιεχόμενο της φυσικής προεπισκόπησης.** POS02 10 κινήσεις / 3,80 € μετρητά / 4,30 € σύνολο, POS01 1 / 0,50 € μετρητά + 0,50 € κάρτα, SKU 2269 −57, τελευταία 23:22:35. Η κεντρική οθόνη Audit απέκλειε το νέο actionType· μετά το PR #1195 / CI #3035 / exact deploy `aa1e63e7`, η **ίδια εγγραφή** εμφανίστηκε 25/09 00:55, LAB POS 2, πώληση `58b6d371-fa70-40ab-89cc-c01b3262898a`, ποσό Audit 0,00 €. **PASS της ορατότητας Audit**, χωρίς δεύτερη εκτύπωση. Φυσικό χαρτί δεν έχει επιβεβαιωθεί. Καμία νέα πώληση. Checkpoint: `CHECKPOINTS/CHANGES/2026-09-24-gate4-before-after-test-protocol.md`.
