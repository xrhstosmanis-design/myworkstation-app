...
| --- | --- | --- |
| HOME-01 | Πρακτικός έλεγχος ελληνικών πινάκων, φίλτρων, ανανέωσης, ποσών, ρόλων και στηλών με προσωρινά δεδομένα. | ΕΠΟΜΕΝΟ |
...
| HOME-04 | Fiscal Bridge DRY RUN: έλεγχος πώλησης, γραμμών, πληρωμών, POS/RBS mapping, idempotency key και payload hash, χωρίς εξωτερική εντολή. | ΥΛΟΠΟΙΗΘΗΚΕ ΤΟΠΙΚΑ - ΑΝΑΜΟΝΗ CI/MERGE |
...
- Fiscal Bridge DRY RUN: commit `eeb42961`, 3/3 στοχευμένα tests PASS και frontend build PASS. Δεν υπάρχει κλήση προς CapDriver/RBS και δεν δημιουργείται `FiscalDocument`. Αναμένει push, PR και CI.
