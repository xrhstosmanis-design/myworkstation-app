Warning: truncated output (original token count: 53531)
...
- [ ] Require green CI, merge, exact deployed revision and one fresh single-submission POS-front LAB with no BackOffice refresh, duplicate settlement, stock posting or finaliz…23531 tokens truncated…only when every selected page returned safe product lines; otherwise the existing full OCR path remains authoritative.
