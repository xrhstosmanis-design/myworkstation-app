...
- [x] Live deploy check: μετά το merge η δημόσια εφαρμογή άλλαξε από το παλιό asset set (`entry-BUq92ZTN`) σε νέο (`entry-QNZA6VPw`) και το νεότερο `main` παραμένει το `2ecc924e`.
- [ ] Επόμενο test αποκλειστικά από τον Χρήστο στο ΕΡΓΑΣΤΗΡΙΟ ΔΟΚΙΜΩΝ με τις ίδιες δύο φωτογραφίες. Αναμενόμενα: μία εγγραφή, 38 γραμμές και 2.369,99 €, χωρίς οριστικοποίηση πριν τον οπτικό έλεγχο.
