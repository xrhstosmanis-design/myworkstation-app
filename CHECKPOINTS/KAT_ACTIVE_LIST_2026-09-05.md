...
- [ ] LAB FAIL / Ανανέωση: στο νεότερο screenshot οι Παραγγελίες & Αγορές παραμένουν στις 2 παλιές εγγραφές (43,41 € / 49,45 €). Χρειάζεται επαλήθευση ότι εμφανίζεται το ήδη αποθηκευμένο πρόχειρο, χωρίς νέα καταχώρηση ή πληρωμή.
...
- [x] Render deploy #1063: PASS. Το βήμα «Wait for exact production revision» επιβεβαίωσε υγιές `/api/health` στο ακριβές merge `bc91ec6a5a1b8b1e0c57c011de184a9869486475` ([run 34747125725](https://github.com/xrhstosmanis-design/myworkstation-app/actions/runs/34747125725)). Το CI του main #2018 είναι επίσης πράσινο. Η LAB επιβεβαίωση του Χρήστου παραμένει εκκρεμής.
- [ ] Επόμενο LAB βήμα από τον Χρήστο: ανανέωση της υπάρχουσας λίστας, εύρεση του 2612188 και οπτικός έλεγχος του προχείρου. Δεν ξαναδημιουργούμε τιμολόγιο/πληρωμή και δεν οριστικοποιούμε. Αναμενόμενα: μία εγγραφή, 38 πλήρεις γραμμές, τελική αξία 2.369,99 €. Το Gate 3 παραμένει ανοιχτό.
...
- Η αναζήτηση υπάρχοντος προμηθευτή προσαρμόστηκε στον δυναμικό πίνακ…1006 tokens truncated… του, η εργασία AI, η εγγραφή Θυρίδας και η φωτογραφία/PDF.
...
- [x] Προστέθηκε πλήρες Web Push με μόνιμα VAPID keys στη βάση.
...
- [x] Προστέθηκε πλήρες Web Push όταν η σελίδα/browser είναι κλειστά.
- [ ] Εκκρεμεί LAB δοκιμή.

## 13/09/2026 — Πλήρες Push Chat ανά κατάστημα

- [x] Προστέθηκε πραγματική Web Push συνδρομή με μόνιμα VAPID keys στη βάση του MyWorkStation.
- [x] Κάθε συνδρομή συνδέεται server-side με εταιρεία, κατάστημα και συνδεδεμένο χρήστη.
- [x] Νέο μήνυμα ειδοποιεί μόνο τις εγγεγραμμένες συσκευές του ίδιου καταστήματος και ποτέ τον αποστολέα.
- [x] Η ειδοποίηση χρησιμοποιεί τον ήχο/δόνηση του Android ή της συσκευής και λειτουργεί με κλειστή τη σελίδα.
- [x] Για ασφάλεια, η ειδοποίηση δεν εμφανίζει κείμενο μηνύματος ή φωτογραφία στην οθόνη κλειδώματος.
- [x] 1/1 στοχευμένη δοκιμή, 1131/1131 πλήρης server suite, syntax checks και client production build: PASS.
- [ ] Εκκρεμούν CI, Render deploy και LAB δοκιμή με δύο λογαριασμούς/συσκευές.
...
- [x] Πλήρες Web Push με VAPID χωρίς χειροκίνητα Render secrets.
...
- [x] Πλήρες Web Push με subscription/VAPID για κλειστή σελίδα/browser.
- [ ] LAB δοκιμή.
