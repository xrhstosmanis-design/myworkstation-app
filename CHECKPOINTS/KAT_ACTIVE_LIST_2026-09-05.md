...
## 2026-09-25 — Gate 3 αναγνώσιμος πίνακας πρόχειρου ΡΗΓΑΣ — AWAITING LAB

- [ ] Νέα πραγματική εικόνα LAB 25/09 05:49 μετά το PR #1208: το παράθυρο γεμίζει όλη την οθόνη, αλλά οι 21 στήλες έχουν υπερβολικά στενά γράμματα/σπασμένους τίτλους, ο πίνακας τελειώνει αριστερά και αφήνει μεγάλο λευκό κενό. Ο ιδιοκτήτης το χαρακτήρισε μη αποδεκτό. Επαναφορά άνετων πλατών πίνακα και παράθυρο σχεδόν πλήρους πλάτους με ύψος περιεχομένου, μία κύλιση μέσα στον πίνακα σε στενότερη οθόνη. Μόνο UI· οικονομικό αποτέλεσμα ΡΗΓΑΣ 3 γραμμές / 39,83 € ανεξάρτητο. Μετά το CI/merge/exact deploy οπτικός έλεγχος στο ίδιο πρόχειρο χωρίς δεύτερη υποβολή ή οριστικοποίηση. Checkpoint: `CHECKPOINTS/CHANGES/2026-09-25-gate3-readable-wide-review-awaiting-lab.md`.

## 2026-09-25 — Gate 4 έκπτωση ενεργού Master προϊόντος — CODE READY / AWAITING CI / LAB PENDING

- [ ] Ο ιδιοκτήτης έδειξε στον Master Catalog ενεργό ΝΕΡΟ 500ML / 2269, ενώ ο διάλογος μωβ «Ομαδικές εκπτώσεις» έδειχνε 0 επιλεγμένα και 0 εμφανιζόμενα. Η πώληση LAB POS 2 στις 08:33, 2 × ΝΕΡΟ 500ML / 0,90 € μετρητά, είναι πραγματική αλλά δεν αποδεικνύει αν προήλθε από χειροκίνητη τιμή 0,45 € ή από δικαιούχο. Δεν χαρακτηρίζεται PASS έκπτωσης ούτε επαναλαμβάνεται η πώληση. Ειδική διόρθωση `agent/gate4-master-audience-discount-20260925`: έκπτωση στην καρτέλα του συγκεκριμένου ενεργού προϊόντος μέσω εταιρικού ID και μεταφορά της μαζικής ενέργειας στη δική της λίστα. Πριν από νέα LAB ενέργεια: πράσινο CI, merge, exact deploy, ανανέωση και baseline βάρδιας/stock. Checkpoint `CHECKPOINTS/CHANGES/2026-09-25-gate4-master-audience-discount-awaiting-lab.md`.
...
- [ ] Issue #1202: με δύο τεμάχια 0,50 € και υποθετική έκπτωση δικαιούχου 10%, η οθόνη υπολογίζει 1,00 € (στρογγυλοποίηση ανά τεμάχιο), το checkout 0,90 € (ανά γραμμή). Το αριθμητικό παράδειγμα είναι μόνο έλεγχος κώδικα. Ο ιδιοκτήτης επιβεβαιώνει ότι η γενική δοκιμή έκπτωσης POS έχει ήδη γίνει· προϋπάρχει καταγραφή κάρτας δικαιούχου και server ελέγχων από 25/08 (`e14ba8f`). Δεν επαναλαμβάνεται. Η ανατεθειμένη εργασία Gate 4 ελέγχει μόνο την ειδική ποσότητα >1 στον κώδικα και αναζητά υπάρχον αριθμητικό LAB τεκμήριο· δεν απαιτεί νέα πώληση για συμπλήρωση αρχείου. Η περιορισμένη αλλαγή UI υπολογίζει πλέον ποσό ανά γραμμή όπως το checkout, με αριθμητικούς ελέγχους 2×0,50/10%=0,90 €, 3×0,50/10%=1,40 € και κανονικό 1,65 € αμετάβλητο. **CI/merge/deploy και τυχόν ειδικό LAB τεκμήριο PENDING· κανένα νέο LAB PASS.** Checkpoint: `CHECKPOINTS/CHANGES/2026-09-25-gate4-discount-quantity-source-finding.md`.
...
- [ ] Στο LAB μετρήθηκαν 46 OCR jobs (13 αποτυχημένα, 19 σε αναμονή, 10 AI ολοκληρωμένα, 4 τοπικά). Οι ετικέτες δεν αποδεικνύουν σωστή ανάγνωση. Το κουμπί χειροκίνητης διόρθωσης ακυρώνεται κατόπιν εντολής ιδιοκτήτη. PR #1194 / CI PASS / Render `f93ed98b2d137896b3cdc813c751f5536314b092`. Κοινή αλλαγή στην πρώτη ροή POS: ανεξάρτητη πλήρης ανάγνωση πρωτότυπης εικόνας **πριν** συγχωνευτούν δεύτερος πίνακας/Azure, αποδοχή μόνο μετά από όλες τις γραμμές, αριθμητική, ΦΠΑ και σύνολο. Παλαιότερα LAB FAIL ΜΟΥΧΛΑΚΗΣ, ΟΛΥΜΠΟΣ, ΠΡΑΤΤΕΪΝ, Fresh Delicacies, PANINI παραμένουν FAIL. Επόμενη αποδοχή μόνο σε νέα γνήσια POS υποβολή· καμία έγκριση/πληρωμή/απόθεμα. Checkpoint: `CHECKPOINTS/CHANGES/2026-09-25-gate3-cross-supplier-source-before-merge.md`.
...
## 2026-09-25 — Gate 3 ΜΟΥΧΛΑΚΗΣ 01/129707 — LAB FAIL / διόρθωση AWAITING LAB
