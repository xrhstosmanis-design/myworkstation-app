...
| 3 | Χειροκίνητα/αυτόματα τιμολόγια | **ΑΝΑΤΕΘΗΚΕ - ΑΛΛΗ ΣΕΛΙΔΑ** | Κλειδωμένο για την υπεύθυνη σελίδα. Ισχύει ο κανόνας έως δύο ορατών γραμμών προς έλεγχο. |
...
| 5 | Πληρωμές, πιστώσεις και συμφωνίες | **PASS** | Επιβεβαιωμένο από τον ιδιοκτήτη. Καταγράφεται στο `docs/manual/payments-suppliers/PASS.md` και δεν επαναλαμβάνεται. |
...
**Νεότερο LAB PASS κοινής αποθήκης 24/09 (περιορισμένο):** μία NON_FISCAL πώληση 0,50 € στο `LAB-POS-02` στις 14:35 αύξησε μόνο τη δική του βάρδια (2→3 κινήσεις, 1,00→1,50 €), άφησε POS01 αμετάβλητο (3 κινήσεις, 0,50 €) και μείωσε το κοινό stock ΝΕΡΟ 500ML **−53→−54** και το συνολικό stock 44.947→44.946. Αυτή η συγκεκριμένη δοκιμή ολοκληρώθηκε και **δεν επαναλαμβάνεται**. Η αρνητική αρχική ποσότητα δεν θεωρείται αποδεκτή απογραφή πιλοτικού καταστήματος. Εκκρεμούν ιστορική απόκλιση, μερική επιστροφή, κλείσιμο βαρδιών και προετοιμασία εγκατάστασης. Τα πλήρη end-to-end σημεία παρακάτω παραμένουν pending. **Gate 4 PENDING**.

**Νεότερο LAB PASS 24/09 (περιορισμένο):** η νέα πλήρης επιστροφή 0,50 € στις 14:02 καταχωρίστηκε στο `LAB-POS-01`, με 3 κινήσεις και καθαρό 0,50 €· το `LAB-POS-02` έμεινε στις 2 κινήσεις και 1,00 €. Η συγκεκριμένη δοκιμή πλήρους επιστροφής στο σωστό terminal ολοκληρώθηκε και **δεν επαναλαμβάνεται**. Εκκρεμούν συμφωνία της παλιάς λάθος επιστροφής στο POS02 στις 12:44, κοινό stock, μερική επιστροφή, κλείσιμο δύο βαρδιών και εγκατάσταση. Τα σύνθετα end-to-end σημεία παρακάτω δεν έχουν PASS και παραμένουν pending. **Gate 4 PENDING**.

**Νεότερο LAB 24/09:** στην παλιά έκδοση, επιστροφή POS01 χρεώθηκε στη βάρδια POS02 στις 12:44 (**πραγματικό cross-terminal FAIL**). Η διόρθωση PR #1156 αναπτύχθηκε ως `c54011e` και νέα πώληση 13:58 εμφανίστηκε μόνη στη λίστα επιστροφής POS01· καταγράφηκε οπτικά επιστροφή −0,50 € στη συσκευή. Η κεντρική λογιστική απόδοση δεν έχει επιβεβαιωθεί λόγω ληγμένης συνεδρίας Backoffice και αυτόματου αποκλεισμού ασφαλούς επανασύνδεσης. Απαιτούνται διασταύρωση δύο βαρδιών, συμφωνία της ιστορικής απόκλισης, κοινό stock, κλείσιμο και πιλοτικό preflight. **Gate 4 PENDING**, καμία διαγραφή από τη λίστα.
...
- Ρέστα, εκπτώσεις, ακύρωση πριν/μετά ολοκλήρωση, πλήρης/μερική επιστροφή και επανεκτύπωση.
- Μία και μόνο κίνηση αποθέματος, σωστά reports και Audit.
...
3. Gate 1, Gate 2 και Gate 5 έχουν PASS και δεν ξαναϋλοποιούνται.
