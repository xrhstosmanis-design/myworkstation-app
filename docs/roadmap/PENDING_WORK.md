...
| 5 | Πληρωμές, πιστώσεις και συμφωνίες | **OPEN — NOT TESTED** | Ο ιδιοκτήτης διόρθωσε στις 24/09 ότι καμία σελίδα δεν έκανε ολοκληρωμένο Gate 5. Όλα τα κριτήρια του επίσημου checklist παραμένουν ατσεκάριστα. Ανάθεση και δοκιμές με πριν/μετά μετά το Gate 4. |
...
- Ρέστα, εκπτώσεις, επανεκτύπωση και επαλήθευση Audit. **Η ακύρωση καλαθιού πριν από πληρωμή αφαιρέθηκε μετά από πραγματικό LAB PASS με πριν/μετά στις 24/09**, όπως και οι χωριστά καταγεγραμμένες πλήρης/μερική επιστροφή. Στο μεταγενέστερο νέο VOID μετρητών POS02, το ποσό βάρδιας 4,30→3,80 € και το SKU 2269 −57→−56 είναι LAB PASS· το ιστορικό έδειξε μία σωστή «ΑΚΥΡΩΣΗ / VOID» στις 22:42:01 (PASS). Η ανεξάρτητη εικόνα Audit δείχνει `CANCEL` −0,50 €, LAB POS 2, 22:42:01 με ID `0fa74ea2…` (PASS παρουσίας). `relatedSaleId` προς την πώληση 22:38:39 και μεταβολή πλήθους Audit παραμένουν NOT TESTED, χωρίς δεύτερο VOID.
...
3. Gate 1 και Gate 2 έχουν PASS. Το Gate 5 είναι OPEN/NOT TESTED και ακολουθεί μετά την ολοκλήρωση του Gate 4.
