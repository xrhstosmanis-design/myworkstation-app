...
function patchDefaults(source) {
  let s = source;
  s = s.replace(/const RECIPE_PROFILE_VERSION=\d+;/, "const RECIPE_PROFILE_VERSION=7;");

  // Cold recipes never carry ice as a base stock line.
  s = s.replace(
  ...
  // No physical milk, milk slot or evaporated milk is allowed in a base recipe.
  s = s.replace(/,\[ingredientSku\.(?:milk|milkSlot|milkEvap|milkLf|almond|oat|soy),\s*\d+(?:\.\d+)?,\s*"ML"\]/g, "");

  // Never seed fixed milk modifier quantities.
...
function patchCleanup(source) {
  ...
  // Cleanup must never recreate fixed milk mappings.
  s = s.replace(
...
async function main() {
  ...
  await cleanDatabase();
  console.log('Kiosk modifier-only model v7: milk and ice removed before and after KAT seed.');
}
