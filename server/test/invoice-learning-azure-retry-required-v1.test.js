...
test("Invoice Learning uses the configured POS-style fallback after an Azure request failure",()=>{
  ...
  assert.match(route,/if\(!completeness\.complete\)return res\.json\(cacheResult\(\{\.\.\.result,azureState,completeness,requiresManualCompletion:true,partialResult:true\}\)\)/);
});
