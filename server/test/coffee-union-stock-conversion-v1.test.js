...
test("Coffee Union converts kilograms to grams without changing the line value",()=>{
  ...
  assert.equal(line.invoiceQuantity,24);assert.equal(line.quantity,24000);assert.equal(line.unit,"GR");assert.equal(line.unitPrice,.0362);assert.equal(line.netAmount,571.24);assert.equal(line.discount1,34.25);
});
test("Coffee Union converts a five-pack cup line to 500 pieces",()=>{
  ...
  assert.equal(line.invoiceQuantity,5);assert.equal(line.quantity,500);assert.equal(line.unit,"ΤΜΧ");assert.equal(line.unitPrice,.053);assert.equal(line.netAmount,22.53);
});
