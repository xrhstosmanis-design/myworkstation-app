...
test("an incomplete AI job is resumed without a second upload or payment",()=>{
  ...
  assert.match(route,/paymentTransactionId:payment\?\.id\|\|null/);
  assert.match(route,/assertReusableInvoicePayment/);
...
test("POS invoice reuses a validated payment when no purchase draft exists",()=>{
  assert.match(route,/FROM "StoreTransaction" t[\s\S]*t\."type"='SUPPLIER_PAYMENT'/);
  assert.match(route,/t\."attachmentChecksum"=\$\{checksum\}/);
  assert.match(route,/findInvoicePayment/);
  assert.match(route,/paymentReused:Boolean\(payment\)/);
...
test("background intake cannot create a second supplier invoice payment",()=>{
  ...
  assert.match(intake,/assertReusableInvoicePayment/);
