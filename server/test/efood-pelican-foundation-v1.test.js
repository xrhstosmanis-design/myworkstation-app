...
import {assertEfoodLabContext,buildEfoodCatalogPreview,buildEfoodOrderRecoveryPreview,buildEfoodPromoPreview,isEfoodLabContext,normalizeEfoodWebhook} from "../src/integrations/efood/foundation.js";
...
test("efood Phase A accepts only the permanent MYWORKSTATION LAB scope",()=>{
  assert.equal(isEfoodLabContext({companyName:"MYWORKSTATION LAB",storeName:"ΕΡΓΑΣΤΗΡΙΟ ΔΟΚΙΜΩΝ"}),true);
  assert.equal(isEfoodLabContext({companyName:"MYWORKSTATION LAB",storeName:"ΚΥΛΙΚΕΙΟ ΚΑΤ"}),false);
  assert.equal(isEfoodLabContext({companyName:"ΠΕΛΑΤΗΣ",storeName:"ΕΡΓΑΣΤΗΡΙΟ ΔΟΚΙΜΩΝ"}),false);
  assert.throws(()=>assertEfoodLabContext({companyName:"MYWORKSTATION LAB",storeName:"ΚΥΛΙΚΕΙΟ ΚΑΤ"}),error=>error?.code==="EFOOD_LAB_ONLY"&&error?.status===409);
});
...
test("all efood configuration, mocks, mappings and previews are server-enforced as LAB-only",()=>{
  assert.match(platformRoute,/function requireEfoodLabStore/);
  assert.match(platformRoute,/requireEfoodLabStore\(await context/);
  assert.match(platformRoute,/testScope:"MYWORKSTATION_LAB_ONLY"/);
  assert.match(platformRoute,/STORE_INTEGRATION_PREPARED_LAB_ONLY/);
  assert.match(webhookRoute,/ensureEfoodLabIntegrationScope/);
  assert.match(webhookRoute,/assertEfoodLabContext/);
  assert.match(webhookRoute,/JOIN "Company"/);
});
...
test("Super Admin UI exposes one full local LAB validation flow and blocks every other store",()=>{
  assert.match(ui,/efood \/ Pelican — Indirect POS/);
  assert.match(ui,/Αποκλειστικά \{LAB_COMPANY\}/);
  assert.match(ui,/Πλήρης ασφαλής LAB δοκιμή/);
  assert.match(ui,/READY_FOR_PICKUP/);
  assert.match(ui,/CANCELLED/);
  assert.match(ui,/previews\/CATALOG/);
  assert.match(ui,/previews\/PROMO/);
  assert.match(ui,/previews\/ORDERS/);
  assert.match(ui,/replay\.idempotent===true/);
  assert.match(ui,/!efoodLabAllowed/);
  assert.match(ui,/External call: ΟΧΙ/);
