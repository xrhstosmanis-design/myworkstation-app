...
test("core espresso doses and takeaway packaging stay deterministic",()=>{
  ...
  assert.match(defaults,/withColdPack=rows=>\[\.\.\.rows,\[ingredientSku\.cupL,1,"PCS"\],\[ingredientSku\.lidL,1,"PCS"\],\[ingredientSku\.straw,1,"PCS"\]\]/);
  assert.doesNotMatch(defaults,/withColdPack=rows=>\[\.\.\.rows,\[ingredientSku\.ice,/);
...
test("frappe with milk has a distinct base recipe and modifier-owned milk quantity",()=>{
  assert.match(defaults,/ΦΡΑΠΕ ΜΕ ΓΑΛΑ[\s\S]*?instant,2,"GR"[\s\S]*?water,150,"ML"/);
