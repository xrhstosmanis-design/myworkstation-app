...
`ALTER TABLE "OnlineOrder" ADD COLUMN IF NOT EXISTS "saleId" TEXT`,`ALTER TABLE "OnlineOrder" ADD COLUMN IF NOT EXISTS "commercialPostedAt" TIMESTAMP(3)`,`ALTER TABLE "OnlineOrder" ADD COLUMN IF NOT EXISTS "cancelledAt" TIMESTAMP(3)`,`ALTER TABLE "OnlineOrder" ADD COLUMN IF NOT EXISTS "cancelReason" TEXT`,`ALTER TABLE "OnlineOrder" ADD COLUMN IF NOT EXISTS "cancellationStage" TEXT`,`ALTER TABLE "OnlineOrder" ADD COLUMN IF NOT EXISTS "cancellationDisposition" TEXT`,`CREATE UNIQUE INDEX IF NOT EXISTS "OnlineOrder_sale_key" ON "OnlineOrder"("saleId") WHERE "saleId" IS NOT NULL`,`CREATE UNIQUE INDEX IF NOT EXISTS "OnlineOrder_store_number_key" ON "OnlineOrder"("storeId","orderNumber")`,`CREATE UNIQUE INDEX IF NOT EXISTS "OnlineOrder_store_idempotency_key" ON "OnlineOrder"("storeId","idempotencyKey")`,`CREATE INDEX IF NOT EXISTS "OnlineOrder_store_status_idx" ON "OnlineOrder"("storeId","status","createdAt")`,
...
async function ensureRecipeIngredientsTracked(){
  try{
    ...
    `);
    await prisma.$executeRawUnsafe(`
      INSERT INTO "PreparationProductSettings" ("companyId","productId","preparationEnabled","environmentalFee","productionStation","autoPrint","updatedAt")
      SELECT DISTINCT r."companyId",r."productId",TRUE,0,NULL,TRUE,CURRENT_TIMESTAMP
      FROM "PreparationRecipeLine" r
      JOIN "Product" p ON p."id"=r."productId" AND p."companyId"=r."companyId" AND p."active"=TRUE
      WHERE r."automatic"=TRUE
      ON CONFLICT ("companyId","productId") DO UPDATE
      SET "preparationEnabled"=TRUE,"updatedAt"=CURRENT_TIMESTAMP
    `);
...
async function ensureOnlineActorProtection(){
  try{
    ...
    await prisma.$executeRawUnsafe(`
      ...
      BEGIN
        ...
        IF order_no IS NOT NULL THEN
          ...
          IF resolved_actor IS NOT NULL THEN NEW."actorId":=resolved_actor; END IF;
        END IF;
        RETURN NEW;
      END;
      $$ LANGUAGE plpgsql
    ...
    await prisma.$executeRawUnsafe(`CREATE TRIGGER mws_preserve_online_transaction_actor_trg BEFORE UPDATE ON "StoreTransaction" FOR EACH ROW EXECUTE FUNCTION mws_preserve_online_transaction_actor()`);
  }catch(error){console.warn("Online ordering actor protection skipped:",error?.message||error)}
}
...
async function reconcileDeliveredOnlineSales(){
    ...
    await prisma.$transaction(async tx=>{
      ...
      await tx.$executeRawUnsafe(`
        WITH delivered AS (
          ...
          SELECT d.*,cs."id" AS "sessionId",COALESCE(oa."actorId",cs."openedBy") AS "actorId",COALESCE(cs."openedByName",'Online') AS "actorName"
          FROM delivered d
          JOIN LATERAL (
            SELECT s."id",s."openedBy",s."openedByName",s."openedAt"
            FROM "CashShiftSession" s
            WHERE s."companyId"=d."companyId"
              AND s."storeId"=d."storeId"
              AND s."status"='OPEN'
              AND s."openedAt"<=d."postedAt"
            ORDER BY s."openedAt" DESC LIMIT 1
          ) cs ON TRUE
          LEFT JOIN LATERAL (
            SELECT a."actorId"
            FROM "StoreOperatorAudit" a
            WHERE a."companyId"=d."companyId" AND a."storeId"=d."storeId"
              AND a."eventType"='ONLINE_SALE_COMPLETED'
              AND a."details"->>'onlineOrderId'=d."orderId"
            ORDER BY a."createdAt" DESC LIMIT 1
          ) oa ON TRUE
        )
      ...
      await tx.$executeRawUnsafe(`
        ...
        SELECT 'online-order-' || r."orderId",r."companyId",r."storeId",r."sessionId",
               ...
               r."total",'ONLINE ΠΑΡΑΓΓΕΛΙΑ ' || r."orderNumber",r."actorId",r."actorName",r."postedAt",CURRENT_TIMESTAMP
        FROM resolved r
        WHERE NOT EXISTS (
          SELECT 1
  ...
  }catch(error){console.warn("Online ordering active-shift reconcile failed:",error?.message||error)}
}
