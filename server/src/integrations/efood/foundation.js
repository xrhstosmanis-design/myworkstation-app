...
export const EFOOD_LAB_COMPANY_NAME="MYWORKSTATION LAB";
export const EFOOD_LAB_STORE_NAME="ΕΡΓΑΣΤΗΡΙΟ ΔΟΚΙΜΩΝ";

function scopeName(value){return String(value??"").trim().replace(/\s+/g," ").toLocaleUpperCase("el-GR")}
export function isEfoodLabContext(value={}){
  const companyName=scopeName(value.companyName??value.company?.name);
  const storeName=scopeName(value.storeName??value.name??value.store?.name);
  return companyName===scopeName(EFOOD_LAB_COMPANY_NAME)&&storeName===scopeName(EFOOD_LAB_STORE_NAME);
}
export function assertEfoodLabContext(value={}){
  if(!isEfoodLabContext(value))throw Object.assign(new Error(`Οι δοκιμές efood/Pelican επιτρέπονται αποκλειστικά στο ${EFOOD_LAB_COMPANY_NAME} / ${EFOOD_LAB_STORE_NAME}.`),{status:409,code:"EFOOD_LAB_ONLY"});
  return true;
}
