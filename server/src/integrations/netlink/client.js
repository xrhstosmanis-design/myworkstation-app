...
export class NetlinkMockClient{
  async menu(){
    return {groups:[{id:"mock-mobile",name:"TEST NETLINK",products:[{id:"MOCK-10",name:"Mock κάρτα 10€",amount:10,flow:"execute"},{id:"MOCK-20",name:"Mock κάρτα 20€",amount:20,flow:"prepare"}]}]};
  }
  async prepare(productId,{requestId,payload}){
    const amount=Number(payload?.amount??(String(productId).includes("20")?20:10));
    return {data:{requestId,reference:`PREP-${requestId}`,amount,prepared:true}};
  }
  async execute(productId,{requestId,payload}){
    const amount=Number(payload?.amount??(String(productId).includes("20")?20:10));
    return {data:{requestId,transactionId:`MOCK-TX-${requestId}`,reference:`MOCK-REF-${requestId}`,amount,pin:`TEST-${String(requestId).slice(0,8).toUpperCase()}`,lines:["TEST NETLINK","ΜΗ ΠΡΑΓΜΑΤΙΚΗ ΚΑΡΤΑ"]}};
  }
}
...
export class NetlinkClient{
  ...
  async request(path,{method="GET",body,requestId}={}){let token=await this.token();const call=()=>fetch(`${this.apiBase}${path}`,{method,headers:{Authorization:`Bearer ${token}`,Accept:"application/json",...(body!==undefined?{"Content-Type":"application/json"}:{}),...(requestId?{"X-Request-Id":requestId}:{})},body:body!==undefined?JSON.stringify(body):undefined});let response=await call();if(response.status===401){token=(await this.refreshAccessToken()).access_token;response=await call()}return parseResponse(response)}
