...
router.get("/recipes",requireCompanyModule("INVENTORY"),async(req,res,next)=>{
  try{const recipes=await prisma.$queryRaw`SELECT r."id",r."productId",r."yieldQuantity",r."active",p."name" AS "productName",p."sku",p."salePrice" FROM "Recipe" r JOIN "Product" p ON p."id"=r."productId" WHERE r."companyId"=${req.user.companyId} ORDER BY p."name"`;const items=await prisma.$queryRaw`SELECT i."id",i."recipeId",i."ingredientProductId",i."quantity",i."unit",p."name" AS "ingredientName",p."sku",p."costPrice",p."trackStock" FROM "RecipeItem" i JOIN "Recipe" r ON r."id"=i."recipeId" JOIN "Product" p ON p."id"=i."ingredientProductId" WHERE r."companyId"=${req.user.companyId} ORDER BY p."name"`;const grouped=new Map();for(const item of items){if(!grouped.has(item.recipeId))grouped.set(item.recipeId,[]);grouped.get(item.recipeId).push(item)}res.json(recipes.map(recipe=>{const recipeItems=grouped.get(recipe.id)||[],yieldQuantity=Number(recipe.yieldQuantity||1),recipeCost=recipeItems.reduce((sum,item)=>sum+Number(item.quantity||0)*Number(item.costPrice||0),0)/yieldQuantity;return {...recipe,items:recipeItems,recipeCost}}))}catch(error){next(error)}
});

router.put("/recipes/:productId",requireCompanyModule("INVENTORY"),async(req,res,next)=>{
  try{const body=z.object({yieldQuantity:z.coerce.number().positive().max(100000),active:z.boolean().optional(),items:z.array(z.object({ingredientProductId:z.string(),quantity:z.coerce.number().positive().max(100000),unit:z.string().max(30).optional()})).min(1).max(100)}).parse(req.body||{});const ids=[req.params.productId,...body.items.map(item=>item.ingredientProductId)],unique=[...new Set(ids)];if(unique.length!==ids.length&&body.items.some(item=>item.ingredientProductId===req.params.productId))return res.status(400).json({error:"Ένα προϊόν δεν μπορεί να είναι συστατικό του εαυτού του."});const products=await prisma.$queryRaw`SELECT "id" FROM "Product" WHERE "companyId"=${req.user.companyId} AND "active"=true AND "id"=ANY(${unique}::text[])`;if(products.length!==unique.length)return res.status(404).json({error:"Ένα ή περισσότερα προϊόντα της συνταγής δεν ανήκουν στην εταιρεία."});const recipeId=id();await prisma.$transaction(async tx=>{const rows=await tx.$queryRaw`INSERT INTO "Recipe" ("id","companyId","productId","yieldQuantity","active") VALUES (${recipeId},${req.user.companyId},${req.params.productId},${body.yieldQuantity},${body.active??true}) ON CONFLICT ("productId") DO UPDATE SET "yieldQuantity"=EXCLUDED."yieldQuantity","active"=EXCLUDED."active","updatedAt"=CURRENT_TIMESTAMP RETURNING "id"`;const actualId=rows[0].id;await tx.$executeRaw`DELETE FROM "RecipeItem" WHERE "recipeId"=${actualId}`;for(const item of body.items)await tx.$executeRaw`INSERT INTO "RecipeItem" ("id","recipeId","ingredientProductId","quantity","unit") VALUES (${id()},${actualId},${item.ingredientProductId},${item.quantity},${item.unit||"PIECE"})`});res.json({ok:true,productId:req.params.productId})}catch(error){next(error)}
});
...
router.post("/sales",requireCompanyModule("POS"),async(req,res,next)=>{
  try{
    ...
    await prisma.$transaction(async tx=>{
      ...
      for(const line of body.lines){const lineTotal=line.quantity*line.unitPrice-(line.discount||0);await tx.$executeRaw`INSERT INTO "SaleLine" ("id","saleId","productId","description","quantity","unitPrice","discount","vatRate","lineTotal") VALUES (${id()},${saleId},${line.productId||null},${line.description},${line.quantity},${line.unitPrice},${line.discount||0},${line.vatRate??24},${lineTotal})`;if(line.productId){const recipe=await tx.$queryRaw`SELECT "id","yieldQuantity" FROM "Recipe" WHERE "companyId"=${req.user.companyId} AND "productId"=${line.productId} AND "active"=true LIMIT 1`;const ingredients=recipe[0]?await tx.$queryRaw`SELECT i."ingredientProductId",i."quantity",p."trackStock" FROM "RecipeItem" i JOIN "Product" p ON p."id"=i."ingredientProductId" WHERE i."recipeId"=${recipe[0].id} AND p."companyId"=${req.user.companyId}`:[];if(ingredients.length){for(const ingredient of ingredients){if(!ingredient.trackStock)continue;const delta=-Math.abs(line.quantity*Number(ingredient.quantity)/Number(recipe[0].yieldQuantity||1));await tx.$executeRaw`INSERT INTO "StoreProduct" ("id","storeId","productId","currentStock") VALUES (${id()},${body.storeId},${ingredient.ingredientProductId},${delta}) ON CONFLICT ("storeId","productId") DO UPDATE SET "currentStock"="StoreProduct"."currentStock"+${delta},"updatedAt"=CURRENT_TIMESTAMP`;await tx.$executeRaw`INSERT INTO "StockMovement" ("id","storeId","productId","movementType","quantity","sourceType","sourceId","note","createdByUserId") VALUES (${id()},${body.storeId},${ingredient.ingredientProductId},'SALE',${delta},'RECIPE_SALE',${saleId},${`Ανάλωση συνταγής: ${line.description}`},${req.user.id})`}}else{const product=await ownedProduct(req.user.companyId,line.productId);if(product?.trackStock){const delta=-Math.abs(line.quantity);await tx.$executeRaw`INSERT INTO "StoreProduct" ("id","storeId","productId","currentStock") VALUES (${id()},${body.storeId},${line.productId},${delta}) ON CONFLICT ("storeId","productId") DO UPDATE SET "currentStock"="StoreProduct"."currentStock"+${delta},"updatedAt"=CURRENT_TIMESTAMP`;await tx.$executeRaw`INSERT INTO "StockMovement" ("id","storeId","productId","movementType","quantity","sourceType","sourceId","createdByUserId") VALUES (${id()},${body.storeId},${line.productId},'SALE',${delta},'SALE',${saleId},${req.user.id})`;}}}}
...
router.get("/sales/advanced-report",requireCompanyModule("SALES_ANALYTICS"),async(req,res,next)=>{
  try{
    const storeId=req.query.storeId?String(req.query.storeId):null;if(storeId&&!await ownedStore(req.user.companyId,storeId))return res.status(404).json({error:"Δεν βρέθηκε το κατάστημα."});
    const to=req.query.to?new Date(String(req.query.to)):new Date(),from=req.query.from?new Date(String(req.query.from)):new Date(to.getTime()-30*86400000);if(!Number.isFinite(from.getTime())||!Number.isFinite(to.getTime())||from>to)return res.status(400).json({error:"Μη έγκυρο διάστημα ημερομηνιών."});
    const duration=to.getTime()-from.getTime(),compareTo=req.query.compareTo?new Date(String(req.query.compareTo)):new Date(from.getTime()-1),compareFrom=req.query.compareFrom?new Date(String(req.query.compareFrom)):new Date(compareTo.getTime()-duration);
    const [summary,comparison,daily,hours,categories,products,employees,storeRows,methods,statuses]=await Promise.all([
      prisma.$queryRaw`SELECT COUNT(*) FILTER (WHERE "status"='COMPLETED')::int AS sales,COALESCE(SUM("total") FILTER (WHERE "status"='COMPLETED'),0) AS revenue,COALESCE(AVG("total") FILTER (WHERE "status"='COMPLETED'),0) AS average,COALESCE(SUM("discount") FILTER (WHERE "status"='COMPLETED'),0) AS discounts FROM "Sale" WHERE "companyId"=${req.user.companyId} AND (${storeId}::text IS NULL OR "storeId"=${storeId}) AND "occurredAt">=${from} AND "occurredAt"<=${to}`,
      prisma.$queryRaw`SELECT COUNT(*) FILTER (WHERE "status"='COMPLETED')::int AS sales,COALESCE(SUM("total") FILTER (WHERE "status"='COMPLETED'),0) AS revenue FROM "Sale" WHERE "companyId"=${req.user.companyId} AND (${storeId}::text IS NULL OR "storeId"=${storeId}) AND "occurredAt">=${compareFrom} AND "occurredAt"<=${compareTo}`,
      prisma.$queryRaw`SELECT DATE("occurredAt") AS day,COUNT(*)::int AS sales,SUM("total") AS revenue FROM "Sale" WHERE "companyId"=${req.user.companyId} AND "status"='COMPLETED' AND (${storeId}::text IS NULL OR "storeId"=${storeId}) AND "occurredAt">=${from} AND "occurredAt"<=${to} GROUP BY DATE("occurredAt") ORDER BY day`,
      prisma.$queryRaw`SELECT EXTRACT(HOUR FROM "occurredAt")::int AS hour,COUNT(*)::int AS sales,SUM("total") AS revenue FROM "Sale" WHERE "companyId"=${req.user.companyId} AND "status"='COMPLETED' AND (${storeId}::text IS NULL OR "storeId"=${storeId}) AND "occurredAt">=${from} AND "occurredAt"<=${to} GROUP BY EXTRACT(HOUR FROM "occurredAt") ORDER BY hour`,
      prisma.$queryRaw`SELECT COALESCE(c."name",'Χωρίς κατηγορία') AS category,SUM(l."quantity") AS quantity,SUM(l."lineTotal") AS revenue FROM "SaleLine" l JOIN "Sale" s ON s."id"=l."saleId" LEFT JOIN "Product" p ON p."id"=l."productId" LEFT JOIN "ProductCategory" c ON c."id"=p."categoryId" WHERE s."companyId"=${req.user.companyId} AND s."status"='COMPLETED' AND (${storeId}::text IS NULL OR s."storeId"=${storeId}) AND s."occurredAt">=${from} AND s."occurredAt"<=${to} GROUP BY c."name" ORDER BY revenue DESC`,
      prisma.$queryRaw`SELECT l."description",SUM(l."quantity") AS quantity,SUM(l."lineTotal") AS revenue,SUM(l."discount") AS discounts FROM "SaleLine" l JOIN "Sale" s ON s."id"=l."saleId" WHERE s."companyId"=${req.user.companyId} AND s."status"='COMPLETED' AND (${storeId}::text IS NULL OR s."storeId"=${storeId}) AND s."occurredAt">=${from} AND s."occurredAt"<=${to} GROUP BY l."description" ORDER BY revenue DESC LIMIT 100`,
      prisma.$queryRaw`SELECT COALESCE(e."fullName",'Χωρίς εργαζόμενο') AS employee,COUNT(*)::int AS sales,SUM(s."total") AS revenue FROM "Sale" s LEFT JOIN "Employee" e ON e."id"=s."operatorEmployeeId" WHERE s."companyId"=${req.user.companyId} AND s."status"='COMPLETED' AND (${storeId}::text IS NULL OR s."storeId"=${storeId}) AND s."occurredAt">=${from} AND s."occurredAt"<=${to} GROUP BY e."fullName" ORDER BY revenue DESC`,
      prisma.$queryRaw`SELECT st."id",st."name",COUNT(*)::int AS sales,SUM(s."total") AS revenue FROM "Sale" s JOIN "Store" st ON st."id"=s."storeId" WHERE s."companyId"=${req.user.companyId} AND s."status"='COMPLETED' AND (${storeId}::text IS NULL OR s."storeId"=${storeId}) AND s."occurredAt">=${from} AND s."occurredAt"<=${to} GROUP BY st."id",st."name" ORDER BY revenue DESC`,
      prisma.$queryRaw`SELECT p."method",COUNT(*)::int AS payments,SUM(p."amount") AS total FROM "Payment" p JOIN "Sale" s ON s."id"=p."saleId" WHERE s."companyId"=${req.user.companyId} AND (${storeId}::text IS NULL OR s."storeId"=${storeId}) AND s."occurredAt">=${from} AND s."occurredAt"<=${to} GROUP BY p."method" ORDER BY total DESC`,
      prisma.$queryRaw`SELECT "status",COUNT(*)::int AS count,COALESCE(SUM("total"),0) AS total FROM "Sale" WHERE "companyId"=${req.user.companyId} AND (${storeId}::text IS NULL OR "storeId"=${storeId}) AND "occurredAt">=${from} AND "occurredAt"<=${to} GROUP BY "status" ORDER BY count DESC`
    ]);
    const current=summary[0]||{},previous=comparison[0]||{},revenue=Number(current.revenue||0),previousRevenue=Number(previous.revenue||0);res.json({from,to,compareFrom,compareTo,summary:{...current,revenueChangePercent:previousRevenue?((revenue-previousRevenue)/previousRevenue)*100:null},comparison:previous,daily,hours,categories,products,employees,stores:storeRows,methods,statuses,nonFiscal:true});
  }catch(error){next(error)}
});
...
router.delete("/documents/inbox/:inboxId",requireCompanyModule("DOCUMENTS"),async(req,res,next)=>{try{const companyId=req.user.companyId;const result=await prisma.$transaction(async tx=>{const rows=await tx.$queryRaw`SELECT "id","attachmentId","status" FROM "DocumentInbox" WHERE "id"=${req.params.inboxId} AND "companyId"=${companyId} FOR UPDATE`;const item=rows[0];if(!item)throw Object.assign(new Error("Δεν βρέθηκε η φωτογραφία/PDF."),{status:404});if(item.status==="PROCESSED")throw Object.assign(new Error("Δεν διαγράφεται φωτογραφία από επιβεβαιωμένο τιμολόγιο."),{status:409});await tx.$executeRaw`DELETE FROM "DocumentInbox" WHERE "id"=${item.id} AND "companyId"=${companyId}`;await tx.$executeRaw`DELETE FROM "AiReaderJob" WHERE "companyId"=${companyId} AND "attachmentId"=${item.attachmentId} AND "status" NOT IN ('AWAITING_APPROVAL','CONFIRMED')`;await tx.$executeRaw`DELETE FROM "DocumentAttachment" WHERE "id"=${item.attachmentId} AND "companyId"=${companyId}`;return {ok:true,deleted:true}});res.json(result)}catch(error){next(error)}});
