...
function canonicalRow(entry){
  ...
  for(const [header,value] of Object.entries(entry.raw||{})){
    const field=aliasLookup.get(norm(header));
...
function localDate(value,isEnd=false){
  ...
  if(typeof value==="number"){
    ...
    return parsePromotionDate(`${p.y}-${pad(p.m)}-${pad(p.d)}T${pad(p.H||0)}:${pad(p.M||0)}:${pad(p.S||0)}`);
  }
  const raw=text(value);
  let match=raw.match(/^(\d{1,2})[\/-](\d{1,2})[\/-](\d{4})(?:\s+(\d{1,2}):(\d{2})(?::(\d{2}))?)?$/);
  if(match){const pad=n=>String(n).padStart(2,"0"),hasTime=match[4]!==undefined;return parsePromotionDate(`${match[3]}-${pad(match[2])}-${pad(match[1])}T${pad(hasTime?match[4]:(isEnd?23:0))}:${pad(hasTime?match[5]:(isEnd?59:0))}:${pad(hasTime?(match[6]||0):(isEnd?59:0))}`)}
  match=raw.match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if(match)return parsePromotionDate(`${raw}T${isEnd?"23:59:59":"00:00:00"}`);
  const parsed=parsePromotionDate(raw);
...
async function productIndex(companyId){
  ...
  for(const row of rows){
    let product=products.get(row.id);if(!product){product={id:row.id,sku:row.sku||null,name:row.name,salePrice:Number(row.salePrice||0),vatRate:Number(row.vatRate||0)};products.set(row.id,product);pushMap(skus,norm(row.sku),product);pushMap(names,norm(row.name),product)}
    pushMap(barcodes,text(row.barcode),product);
  }
...
async function validateStores(companyId,storeIds){
  const ids=[...new Set(storeIds.map(v=>text(v)).filter(Boolean))];
  if(!ids.length){const e=new Error("Επίλεξε τουλάχιστον ένα κατάστημα για την προσφορά.");e.status=400;throw e}
  if(ids.length>200){const e=new Error("Υπερβολικά πολλά καταστήματα.");e.status=400;throw e}
  const stores=await prisma.store.findMany({where:{companyId,active:true,id:{in:ids}},select:{id:true,name:true},orderBy:{name:"asc"}});
  if(stores.length!==ids.length){const e=new Error("Ένα ή περισσότερα καταστήματα δεν ανήκουν στην εταιρεία ή δεν είναι ενεργά.");e.status=400;throw e}
  return stores;
}

async function existingOverlaps(companyId,productIds,storeIds){
  if(!productIds.length)return [];
  return prisma.$queryRaw`SELECT pr."id",pr."productId",pr."validFrom",pr."validUntil",ps."storeId" FROM "PriceCatalogPromotion" pr JOIN "PriceCatalogPromotionStore" ps ON ps."promotionId"=pr."id" AND ps."companyId"=pr."companyId" WHERE pr."companyId"=${companyId} AND pr."promotionType"='LEAFLET' AND pr."active"=true AND pr."productId"=ANY(${productIds}::text[]) AND ps."storeId"=ANY(${storeIds}::text[])`;
}
...
async function buildPreview(req,body){
  const companyId=req.user.companyId,{filename,buffer,fileHash}=decodeFile(body),stores=await validateStores(companyId,body.storeIds),rawRows=sheetRows(buffer),index=await productIndex(companyId);
  const staged=[];
  for(const entry of rawRows){
    ...
    let offerPrice=offerInput!==null?round2(offerInput):round2(realOriginal*(1-discountInput/100));
    if(offerPrice<0){staged.push({...row,product,matchedBy:match.matchedBy,status:"INVALID",error:"Η νέα τιμή δεν μπορεί να είναι αρνητική.",warnings});continue}
  ...
  for(const row of staged.filter(r=>r.status==="READY")){
    const hits=existing.filter(ex=>ex.productId===row.product.id&&intervalsOverlap(row.validFrom,row.validUntil,ex.validFrom,ex.validUntil));
    const hitStores=[...new Set(hits.map(h=>h.storeId))];
    if(hitStores.length){row.status="OVERLAP";row.error=`Υπάρχει ήδη ενεργή προσφορά που επικαλύπτεται σε ${hitStores.length} επιλεγμένο/α κατάστημα/τα.`;row.overlapStoreIds=hitStores}
  }
  ...
  const previewHash=crypto.createHash("sha256").update(`${companyId}:${fileHash}:${JSON.stringify([...storeIds].sort())}`).digest("hex");
...
router.post("/promotions/import/preview",async(req,res,next)=>{try{const body=requestSchema.parse(req.body||{});res.json(await buildPreview(req,body))}catch(error){if(error?.name==="ZodError")return res.status(400).json({error:"Επίλεξε αρχείο και τουλάχιστον ένα κατάστημα.",details:error.issues});next(error)}});
...
router.post("/promotions/import/commit",async(req,res,next)=>{
  try{
    const body=commitSchema.parse(req.body||{}),preview=await buildPreview(req,body);
    if(preview.previewHash!==body.previewHash)return res.status(409).json({error:"Το αρχείο ή τα επιλεγμένα καταστήματα άλλαξαν μετά την προεπισκόπηση. Κάνε νέα προεπισκόπηση."});
    if(!preview.counts.ready)return res.status(409).json({error:"Δεν υπάρχει καμία έγκυρη γραμμή για εισαγωγή."});
    if(preview.skipped>0&&!body.acceptSkipped)return res.status(409).json({error:`Υπάρχουν ${preview.skipped} γραμμές που θα παραλειφθούν. Επιβεβαίωσε ρητά ότι αποδέχεσαι την παράλειψή τους.`,code:"SKIPPED_ROWS_CONFIRMATION_REQUIRED",counts:preview.counts});
    const actor=req.user.fullName||req.user.email||"Χρήστης",created=[];
    await prisma.$transaction(async tx=>{
      for(const row of preview.rows.filter(r=>r.status==="READY")){
        const promoId=id();
        await tx.$executeRaw`INSERT INTO "PriceCatalogPromotion" ("id","companyId","productId","promotionType","originalPrice","offerPrice","discountPercent","saleQuantity","bonusQuantity","customerPoints","validFrom","validUntil","active","createdByUserId","createdByName") VALUES (${promoId},${req.user.companyId},${row.product.id},'LEAFLET',${row.originalPrice},${row.offerPrice},${row.discountPercent},1,0,${row.customerPoints},${new Date(row.validFrom)},${row.validUntil?new Date(row.validUntil):null},true,${req.user.id},${actor})`;
        for(const store of preview.stores)await tx.$executeRaw`INSERT INTO "PriceCatalogPromotionStore" ("promotionId","companyId","storeId") VALUES (${promoId},${req.user.companyId},${store.id}) ON CONFLICT ("promotionId","storeId") DO NOTHING`;
        created.push({promotionId:promoId,productId:row.product.id,productName:row.product.name});
      }
    });
