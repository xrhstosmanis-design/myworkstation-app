...
router.post("/ai-reader/jobs/:jobId/create-product",requireCompanyModule("AI_READER"),requireCompanyModule("INVENTORY"),async(req,res,next)=>{
  try{
    ...
    const product=await prisma.$transaction(async tx=>{
      ...
      await tx.$executeRaw`INSERT INTO "SupplierProductLink" ("id","companyId","supplierId","productId","supplierCode","active","source","updatedBy","updatedByName") VALUES (${id()},${req.user.companyId},${body.supplierId},${productId},${supplierItemCode||null},true,'INVOICE',${req.user.id},${req.user.fullName||req.user.email||'BackOffice'}) ON CONFLICT ("companyId","supplierId","productId") DO UPDATE SET "supplierCode"=COALESCE(NULLIF(EXCLUDED."supplierCode",''),"SupplierProductLink"."supplierCode"),"active"=true,"source"='INVOICE',"updatedBy"=EXCLUDED."updatedBy","updatedByName"=EXCLUDED."updatedByName","updatedAt"=CURRENT_TIMESTAMP`;
  ...
  try{
    ...
    const result=await prisma.$transaction(async tx=>{
      ...
      for(const line of lines){
        ...
        if(doc.supplierId)await tx.$executeRaw`INSERT INTO "SupplierProductLink" ("id","companyId","supplierId","productId","supplierCode","active","source","updatedBy","updatedByName") VALUES (${id()},${req.user.companyId},${doc.supplierId},${line.productId},${supplierItemCode||null},true,'INVOICE',${req.user.id},${req.user.fullName||req.user.email||'BackOffice'}) ON CONFLICT ("companyId","supplierId","productId") DO UPDATE SET "supplierCode"=COALESCE(NULLIF(EXCLUDED."supplierCode",''),"SupplierProductLink"."supplierCode"),"active"=true,"source"='INVOICE',"updatedBy"=EXCLUDED."updatedBy","updatedByName"=EXCLUDED."updatedByName","updatedAt"=CURRENT_TIMESTAMP`;
