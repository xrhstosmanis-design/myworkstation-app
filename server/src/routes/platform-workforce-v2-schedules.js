...
async function publishScheduleToStoreChat(tx,req,context,schedule){const lines=schedule.assignments.filter(a=>a.employee).map(a=>`${iso(a.date)} · ${a.shiftTemplate.name} ${a.shiftTemplate.startTime}–${a.shiftTemplate.endTime} · ${a.employee.fullName}`),body=[`📅 Νέο πρόγραμμα προσωπικού · ${context.store.name}`,`${iso(schedule.periodStart)} – ${iso(schedule.periodEnd)}`,...lines].join("\n").slice(0,5000),id=`chat-workforce-${schedule.id}-v${schedule.version}`;await tx.$executeRawUnsafe('CREATE TABLE IF NOT EXISTS "StoreChatMessage" ("id" TEXT PRIMARY KEY,"companyId" TEXT NOT NULL,"storeId" TEXT NOT NULL,"senderId" TEXT NOT NULL,"category" TEXT NOT NULL DEFAULT \'GENERAL\',"body" TEXT NOT NULL,"important" BOOLEAN NOT NULL DEFAULT false,"pinned" BOOLEAN NOT NULL DEFAULT false,"completed" BOOLEAN NOT NULL DEFAULT false,"createdAt" TIMESTAMP NOT NULL DEFAULT NOW(),"updatedAt" TIMESTAMP NOT NULL DEFAULT NOW())');await tx.$executeRaw`INSERT INTO "StoreChatMessage" ("id","companyId","storeId","senderId","category","body","important","createdAt","updatedAt") VALUES (${id},${context.company.id},${context.store.id},${req.user.id},'SHIFT',${body},true,NOW(),NOW()) ON CONFLICT ("id") DO NOTHING`;return id}
