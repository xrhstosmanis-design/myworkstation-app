...
router.post("/ai/interpret",async(req,res,next)=>{try{
 ...
 res.json({mode:"PREVIEW_ONLY",periodStart:body.periodStart,instruction:body.instruction,conversation:body.conversation,context:{store:{id:context.store.id,name:context.store.name},employees:permanentRules,shiftTemplates:templates.map(t=>({id:t.id,name:t.name,category:t.category,startTime:t.startTime,endTime:t.endTime,minimumPeople:t.minimumPeople,maximumPeople:t.maximumPeople}))},nextStep:"AI_GENERATION_NOT_CONNECTED",message:"Το ασφαλές context του AI Scheduler είναι έτοιμο. Δεν γράφτηκε καμία βάρδια. Η επόμενη έκδοση θα παράγει structured proposal και θα περνά από το υπάρχον schedule validation πριν από οποιαδήποτε αποθήκευση."});
