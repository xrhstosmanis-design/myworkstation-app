...
async function publishScheduleToStoreChat(tx,req,context,schedule){
 const storeEmployees=await tx.workforceEmployee.findMany({where:{companyId:context.company.id,active:true,OR:[{baseStoreId:context.store.id},{storeAccess:{some:{storeId:context.store.id,active:true}}}]},select:{id:true,fullName:true},orderBy:{fullName:"asc"}});
 const employeeMap=new Map(storeEmployees.map(employee=>[employee.id,employee]));
 for(const assignment of schedule.assignments)if(assignment.employee&&!employeeMap.has(assignment.employee.id))employeeMap.set(assignment.employee.id,assignment.employee);
 const dates=[];for(let cursor=new Date(schedule.periodStart);cursor<=new Date(schedule.periodEnd);cursor=new Date(cursor.getTime()+86400000))dates.push(iso(cursor));
 const dateLabel=value=>new Intl.DateTimeFormat("el-GR",{weekday:"short",day:"2-digit",month:"2-digit",timeZone:"UTC"}).format(new Date(`${value}T12:00:00.000Z`));
 const sections=[...employeeMap.values()].sort((a,b)=>a.fullName.localeCompare(b.fullName,"el")).map(employee=>{
   const employeeRows=schedule.assignments.filter(row=>row.employeeId===employee.id);
   const dayLines=dates.map(date=>{const rows=employeeRows.filter(row=>iso(row.date)===date);return rows.length?`${dateLabel(date)} · ${rows.map(row=>`${row.shiftTemplate.name} ${row.shiftTemplate.startTime}–${row.shiftTemplate.endTime}`).join(" / ")}`:`${dateLabel(date)} · ΡΕΠΟ`});
   return [`👤 ${employee.fullName}`,...dayLines].join("\n");
 });
 const body=[`📅 Νέο πρόγραμμα προσωπικού · ${context.store.name}`,`${iso(schedule.periodStart)} – ${iso(schedule.periodEnd)}`,"ΑΝΑ ΕΡΓΑΖΟΜΕΝΟ","",...sections].join("\n\n").slice(0,5000),id=`chat-workforce-${schedule.id}-v${schedule.version}`;
 await tx.$executeRawUnsafe('CREATE TABLE IF NOT EXISTS "StoreChatMessage" ("id" TEXT PRIMARY KEY,"companyId" TEXT NOT NULL,"storeId" TEXT NOT NULL,"senderId" TEXT NOT NULL,"category" TEXT NOT NULL DEFAULT \'GENERAL\',"body" TEXT NOT NULL,"important" BOOLEAN NOT NULL DEFAULT false,"pinned" BOOLEAN NOT NULL DEFAULT false,"completed" BOOLEAN NOT NULL DEFAULT false,"createdAt" TIMESTAMP NOT NULL DEFAULT NOW(),"updatedAt" TIMESTAMP NOT NULL DEFAULT NOW())');
 await tx.$executeRaw`INSERT INTO "StoreChatMessage" ("id","companyId","storeId","senderId","category","body","important","createdAt","updatedAt") VALUES (${id},${context.company.id},${context.store.id},${req.user.id},'SHIFT',${body},true,NOW(),NOW()) ON CONFLICT ("id") DO NOTHING`;
 return id
}
...
function aiNorm(value){return String(value||"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLocaleUpperCase("el-GR")}
function aiMention(text,name){const parts=aiNorm(name).split(/\s+/).filter(x=>x.length>2);return parts.some(x=>aiNorm(text).includes(x))}
function aiDays(text){const names=[["ΔΕΥΤΕΡ","MON"],["ΤΡΙΤ","TUE"],["ΤΕΤΑΡΤ","WED"],["ΠΕΜΠΤ","THU"],["ΠΑΡΑΣΚΕΥ","FRI"],["ΣΑΒΒΑΤ","SAT"],["ΚΥΡΙΑΚ","SUN"]];return names.filter(([x])=>aiNorm(text).includes(x)).map(([,x])=>x)}
function addDaysIso(start,index){const d=dayStart(start);d.setUTCDate(d.getUTCDate()+index);return iso(d)}
function buildAiProposal({instruction,periodStart,employees,templates}){
 const text=aiNorm(instruction),days=[0,1,2,3,4,5,6].map(i=>addDaysIso(periodStart,i)),dayCodes=["MON","TUE","WED","THU","FRI","SAT","SUN"],blocked=new Map(),preferences=[];
 for(const e of employees){const mentioned=aiMention(instruction,e.fullName);if(!mentioned)continue;const sentence=instruction.split(/[.\n]/).find(x=>aiMention(x,e.fullName))||instruction,ds=aiDays(sentence);if(aiNorm(sentence).includes("ΡΕΠΟ")||aiNorm(sentence).includes("ΑΔΕΙΑ"))for(const d of ds){const idx=dayCodes.indexOf(d);if(idx>=0)blocked.set(`${e.id}:${days[idx]}`,true)};if(aiNorm(sentence).includes("ΜΟΝΟ ΠΡΩ"))preferences.push({employeeId:e.id,category:"MORNING"});if(aiNorm(sentence).includes("ΜΟΝΟ ΑΠΟΓ"))preferences.push({employeeId:e.id,category:"AFTERNOON"});if(aiNorm(sentence).includes("ΜΟΝΟ ΝΥΧ")||aiNorm(sentence).includes("ΜΟΝΟ ΒΡΑ"))preferences.push({employeeId:e.id,category:"NIGHT"});}
 const counts=new Map(employees.map(e=>[e.id,0])),assignments=[],warnings=[];
 for(const date of days){for(const t of templates){const slots=Math.max(1,Number(t.minimumPeople||1));for(let slot=1;slot<=slots;slot++){const candidates=employees.filter(e=>!blocked.get(`${e.id}:${date}`)&&!e.leaveRequests?.some(l=>l.status==="APPROVED"&&iso(l.startDate)<=date&&iso(l.endDate)>=date)).filter(e=>t.category==="MORNING"?e.worksMorning:t.category==="AFTERNOON"?e.worksAfternoon:t.category==="NIGHT"?e.worksNight:true).filter(e=>!preferences.some(p=>p.employeeId===e.id&&p.category!==t.category)).sort((a,b)=>(counts.get(a.id)||0)-(counts.get(b.id)||0));const chosen=candidates[0];if(chosen){assignments.push({date,shiftTemplateId:t.id,shiftName:t.name,category:t.category,startTime:t.startTime,endTime:t.endTime,slot,employeeId:chosen.id,employeeName:chosen.fullName});counts.set(chosen.id,(counts.get(chosen.id)||0)+1)}else{assignments.push({date,shiftTemplateId:t.id,shiftName:t.name,category:t.category,startTime:t.startTime,endTime:t.endTime,slot,employeeId:null,employeeName:null});warnings.push({date,shiftTemplateId:t.id,message:`Ακάλυπτη θέση ${t.name} · θέση ${slot}`})}}}}
...
router.post("/ai/apply",async(req,res,next)=>{try{
 const context=await contextFor(req),body=z.object({periodStart:dateText,instruction:z.string().trim().min(5).max(8000),assignments:z.array(z.object({date:dateText,shiftTemplateId:z.string().min(1),slot:z.number().int().min(1),employeeId:z.string().nullable()})).min(1),confirmed:z.literal(true),reason:z.string().trim().min(3).max(500)}).parse(req.body||{});context.requestUser=req.user;
 const start=weekStart(body.periodStart),end=new Date(start.getTime()+6*86400000);
 for(const assignment of body.assignments){const date=dayStart(assignment.date);if(date<start||date>end)throw Object.assign(new Error("Η πρόταση AI περιέχει ημέρα εκτός της επιλεγμένης εβδομάδας. Δημιούργησε ξανά την προεπισκόπηση."),{status:409});}
 const [allowedEmployees,allowedTemplates]=await Promise.all([
  prisma.workforceEmployee.findMany({where:{companyId:context.company.id,active:true,OR:[{baseStoreId:context.store.id},{storeAccess:{some:{storeId:context.store.id,active:true,canSchedule:true}}}]},select:{id:true}}),
  prisma.workforceShiftTemplate.findMany({where:{companyId:context.company.id,storeId:context.store.id,active:true},select:{id:true}})
 ]),employeeIds=new Set(allowedEmployees.map(item=>item.id)),templateIds=new Set(allowedTemplates.map(item=>item.id));
 if(body.assignments.some(item=>item.employeeId&&!employeeIds.has(item.employeeId)))throw Object.assign(new Error("Η πρόταση AI περιέχει εργαζόμενο που δεν είναι πλέον διαθέσιμος στο κατάστημα. Δημιούργησε ξανά την προεπισκόπηση."),{status:409});
 if(body.assignments.some(item=>!templateIds.has(item.shiftTemplateId)))throw Object.assign(new Error("Η πρόταση AI περιέχει ανενεργή βάρδια. Δημιούργησε ξανά την προεπισκόπηση."),{status:409});
 const created=await prisma.$transaction(async tx=>{const latest=await tx.workforceSchedule.findFirst({where:{storeId:context.store.id,periodStart:start,periodEnd:end},select:{version:true},orderBy:{version:"desc"}}),version=(latest?.version||0)+1;const schedule=await tx.workforceSchedule.create({data:{companyId:context.company.id,storeId:context.store.id,periodType:"WEEK",periodStart:start,periodEnd:end,status:"DRAFT",version,createdByUserId:req.user.id}});for(const a of body.assignments){if(!a.employeeId)continue;await tx.workforceScheduleAssignment.create({data:{scheduleId:schedule.id,date:dayStart(a.date),employeeId:a.employeeId,shiftTemplateId:a.shiftTemplateId,slot:a.slot,note:"AI Scheduler draft"}})}await audit(tx,req,{companyId:context.company.id,storeId:context.store.id,action:"WORKFORCE_AI_SCHEDULE_DRAFT_CREATED",entityType:"WORKFORCE_SCHEDULE",entityId:schedule.id,after:{periodStart:iso(start),instruction:body.instruction,assignments:body.assignments.filter(a=>a.employeeId).length,version},reason:body.reason});return schedule});
...
router.post("/ai/interpret",async(req,res,next)=>{try{
 ...
 const normalizedPeriodStart=iso(weekStart(body.periodStart));
 const employees=await prisma.workforceEmployee.findMany({where:{companyId:context.company.id,active:true,OR:[{baseStoreId:context.store.id},{storeAccess:{some:{storeId:context.store.id,active:true,canSchedule:true}}}]},include:{rules:true,leaveRequests:{where:{status:"APPROVED",endDate:{gte:dayStart(normalizedPeriodStart)}}}}});
 const templates=await prisma.workforceShiftTemplate.findMany({where:{companyId:context.company.id,storeId:context.store.id,active:true},orderBy:{startTime:"asc"}});
 const permanentRules=employees.map(e=>({employeeId:e.id,name:e.fullName,maxDaysPerWeek:e.maxDaysPerWeek,maxHoursPerWeek:e.maxHoursPerWeek,minimumDaysOff:e.minimumDaysOff,availability:{morning:e.worksMorning,afternoon:e.worksAfternoon,night:e.worksNight,weekend:e.worksWeekend},rules:(e.rules||[]).filter(r=>r.active).map(r=>({type:r.ruleType,value:r.valueJson,relatedEmployeeId:r.relatedEmployeeId})),approvedLeave:(e.leaveRequests||[]).map(l=>({type:l.leaveType,start:iso(l.startDate),end:iso(l.endDate)}))}));
 const proposal=buildAiProposal({instruction:body.instruction,periodStart:normalizedPeriodStart,employees,templates});res.json({mode:"PREVIEW_ONLY",periodStart:normalizedPeriodStart,instruction:body.instruction,conversation:body.conversation,context:{store:{id:context.store.id,name:context.store.name},employees:permanentRules,shiftTemplates:templates.map(t=>({id:t.id,name:t.name,category:t.category,startTime:t.startTime,endTime:t.endTime,minimumPeople:t.minimumPeople,maximumPeople:t.maximumPeople}))},proposal,nextStep:"USER_REVIEW",message:"Δημιουργήθηκε structured preview από τα πραγματικά δεδομένα Workforce. Δεν γράφτηκε καμία βάρδια. Έλεγξε το πρόγραμμα και γράψε διορθώσεις πριν από έγκριση."});
}catch(error){next(error)}});
