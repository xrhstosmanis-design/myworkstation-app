...
const executeSchema=z.object({storeId:z.string().min(1),productId:z.string().min(1),payload:z.record(z.any()).default({}),confirmation:z.record(z.any()).optional(),requestId:z.string().min(6).max(100),paymentMethod:z.enum(["CASH","CARD","IRIS","MIXED"]).optional(),saleId:z.string().min(1).optional(),testRun:z.boolean().optional().default(false)});
router.post("/execute",async(req,res,next)=>{
  ...
  try{
    ...
    if(testMode&&!body.testRun)return res.status(400).json({error:"Στο Netlink test mode απαιτείται ρητό testRun.",code:"NETLINK_TEST_RUN_REQUIRED"});
    if(testMode&&String(body.confirmation?.phrase||"")!==`STAGING ${body.productId}`)return res.status(400).json({error:"Η επιβεβαίωση staging δεν είναι σωστή.",code:"NETLINK_STAGING_CONFIRMATION_REQUIRED"});
    if(!testMode&&!body.saleId)return res.status(400).json({error:"Απαιτείται η συνδεδεμένη πώληση POS.",code:"NETLINK_POS_SALE_REQUIRED"});
    ...
    const sales=testMode?[]:await prisma.$queryRaw`SELECT "id","total","status","fiscalStatus" FROM "Sale" WHERE "id"=${body.saleId} AND "storeId"=${body.storeId} AND "companyId"=${req.user.companyId} LIMIT 1`;const sale=sales[0]||null;
    if(!testMode&&!sale)return res.status(404).json({error:"Δεν βρέθηκε η συνδεδεμένη δοκιμαστική πώληση POS."});
    if(!testMode&&String(sale.status)!=="COMPLETED")return res.status(409).json({error:"Η Netlink δοκιμή επιτρέπεται μόνο αφού ολοκληρωθεί η κανονική πώληση POS.",code:"NETLINK_POS_SALE_NOT_COMPLETED"});
    const fiscalRows=testMode?[]:await prisma.$queryRaw`SELECT "id","provider","externalId","fiscalNumber","status","issuedAt","payloadHash" FROM "FiscalDocument" WHERE "saleId"=${body.saleId} LIMIT 1`;const fiscalDocument=fiscalRows[0]||null;
    if(!testMode&&!validFiscalReceipt(fiscalDocument)){const error=fiscalReceiptError();return res.status(error.status).json({error:error.message,code:error.code})}
    ...
    const flow=testMode?(existing?'TEST_PREPARE_EXECUTE':'TEST_EXECUTE_DIRECT'):'RECEIPT_THEN_EXECUTE';
    if(existing)await prisma.$executeRaw`UPDATE "NetlinkTransaction" SET "saleId"=${body.saleId||null},"flow"=${flow},"status"='EXECUTING',"paymentMethod"=${body.paymentMethod||null},"serviceFeeAmount"=${serviceFeeAmount},"fiscalDocumentId"=${fiscalDocument?.id||null},"fiscalNumber"=${fiscalDocument?.fiscalNumber||null},"fiscalIssuedAt"=${fiscalDocument?.issuedAt||null},"updatedAt"=NOW() WHERE "id"=${ledgerId}`;
    else await prisma.$executeRaw`INSERT INTO "NetlinkTransaction" ("id","companyId","storeId","saleId","requestId","productId","flow","status","paymentMethod","serviceFeeAmount","operatorId","operatorName","fiscalDocumentId","fiscalNumber","fiscalIssuedAt") VALUES (${ledgerId},${req.user.companyId},${body.storeId},${body.saleId||null},${body.requestId},${body.productId},${flow},'EXECUTING',${body.paymentMethod||null},${serviceFeeAmount},${req.user.id||null},${req.user.fullName||null},${fiscalDocument?.id||null},${fiscalDocument?.fiscalNumber||null},${fiscalDocument?.issuedAt||null})`;
    ...
    const amount=txAmount(result),providerTransactionId=txId(result),reference=txReference(result),commissionAmount=money(amount*commissionRate),customerTotal=money(amount+serviceFeeAmount),saleTotal=testMode?customerTotal:money(sale.total),amountNeedsReview=!testMode&&Math.abs(customerTotal-saleTotal)>0.01;
    ...
    res.json({testRun:testMode,requestId:body.requestId,transactionLedgerId:ledgerId,status:amountNeedsReview?"AMOUNT_REVIEW":"COMPLETED",saleId:body.saleId||null,fiscalReceipt:fiscalDocument?{id:fiscalDocument.id,number:fiscalDocument.fiscalNumber,issuedAt:fiscalDocument.issuedAt}:null,cardAmount:amount,serviceFeeAmount,customerTotal,saleTotal,commission:{rate:commissionRate,baseAmount:amount,amount:commissionAmount},result});
  }catch(error){if(ledgerId)await prisma.$executeRaw`UPDATE "NetlinkTransaction" SET "status"='FAILED',"errorCode"=${error.code||null},"errorMessage"=${String(error.message||"Netlink error").slice(0,500)},"updatedAt"=NOW() WHERE "id"=${ledgerId}`.catch(()=>{});next(error)}
});
