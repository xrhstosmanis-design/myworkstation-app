...
const dateRange=query=>({from:query.from||new Date(new Date().setHours(0,0,0,0)),to:query.to||new Date()});
...
router.get("/reports/weekly-settlement",async(req,res,next)=>{try{const result=await reportRows(req,reportQuery.parse(req.query));const completed=result.items.filter(x=>x.status==="COMPLETED");res.json({...result,settlement:{transactions:completed.length,grossAmount:money(completed.reduce((sum,x)=>sum+Number(x.amount||0),0)),serviceFees:money(completed.reduce((sum,x)=>sum+Number(x.serviceFeeAmount||0),0)),customerTotal:money(completed.reduce((sum,x)=>sum+Number(x.customerTotal||0),0)),commissionAmount:money(completed.reduce((sum,x)=>sum+Number(x.commissionAmount||0),0)),providerSettlementAmount:null,variance:null,providerSettlementStatus:"AWAITING_NETLINK_STATEMENT"}})}catch(error){next(error)}});
