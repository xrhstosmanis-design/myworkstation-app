...
router.get("/stores/:storeId/config",async(req,res,next)=>{
  try{
    ...
    const rows=await prisma.$queryRaw`SELECT c."storeId",c."saleProductId",c."active",c."notes",p."name" AS "saleProductName",p."sku" AS "saleProductSku",p."vatRate",p."trackStock",COALESCE(sp."salePrice",p."salePrice") AS "salePrice" FROM "NetlinkStoreConfig" c LEFT JOIN "Product" p ON p."id"=c."saleProductId" AND p."companyId"=${req.user.companyId} LEFT JOIN "StoreProduct" sp ON sp."productId"=p."id" AND sp."storeId"=${store.id} WHERE c."storeId"=${store.id} AND c."companyId"=${req.user.companyId} LIMIT 1`;
    const row=rows[0]||null;
    res.json({configured:Boolean(row?.saleProductId),storeId:store.id,active:row?.active!==false,saleProduct:row?.saleProductId?{id:row.saleProductId,name:row.saleProductName,sku:row.saleProductSku,vatRate:Number(row.vatRate||0),trackStock:Boolean(row.trackStock),salePrice:money(row.salePrice)}:null,notes:row?.notes||null});
...
router.put("/stores/:storeId/config",async(req,res,next)=>{
  try{
    ...
    const products=await prisma.$queryRaw`SELECT p."id",p."name",p."sku",p."vatRate",p."trackStock",COALESCE(sp."salePrice",p."salePrice") AS "salePrice" FROM "Product" p JOIN "StoreProduct" sp ON sp."productId"=p."id" AND sp."storeId"=${store.id} AND sp."active"=true WHERE p."id"=${body.saleProductId} AND p."companyId"=${req.user.companyId} AND p."active"=true LIMIT 1`;
    ...
    if(product.trackStock)return res.status(400).json({error:"Το προϊόν POS για Netlink πρέπει να είναι υπηρεσία χωρίς παρακολούθηση αποθέματος.",code:"NETLINK_SERVICE_PRODUCT_MUST_NOT_TRACK_STOCK"});
    ...
    res.json({configured:true,storeId:store.id,active:body.active,saleProduct:{id:product.id,name:product.name,sku:product.sku,vatRate:Number(product.vatRate||0),trackStock:false,salePrice:money(product.salePrice)},notes:body.notes||null});
  }catch(error){next(error)}
});
...
router.post("/execute",async(req,res,next)=>{
  ...
  try{
    ...
    if(existing)await prisma.$executeRaw`UPDATE "NetlinkTransaction" SET "saleId"=${body.saleId},"flow"='PREPARE_EXECUTE',"status"='EXECUTING',"paymentMethod"=${body.paymentMethod||null},"updatedAt"=NOW() WHERE "id"=${ledgerId}`;
