...
async function ensureMarketSchema(){
  ...
  await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "InternetProductSearch_company_created_idx" ON "InternetProductSearch" ("companyId","createdAt" DESC)`);
  await prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "InternetPriceProposal" (
  ...
  await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "InternetPriceProposal_company_status_idx" ON "InternetPriceProposal" ("companyId","status","createdAt" DESC)`);
  await prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "InternetPriceProposalAudit" (
  ...
  await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "InternetPriceProposalAudit_proposal_idx" ON "InternetPriceProposalAudit" ("proposalId","createdAt")`);
}
...
async function internetMarketSearch(query){
  ...
  if(!serperKey&&!googleKey)return {configured:false,rows:[],reason:"PROVIDER_NOT_CONFIGURED"};
