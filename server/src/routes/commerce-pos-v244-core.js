...
// Paid POS invoices must be visible in BackOffice before full OCR can fail.
router.post("/ai-reader/jobs/:jobId/pos-draft",requireCompanyModule("AI_READER"),requireCompanyModule("INVENTORY"),async(req,res,next)=>{try{
const body=z.object({supplierId:z.string().min(1),documentNumber:z.string().trim().min(1).max(80),documentDate:z.coerce.date(),totalGross:z.coerce.number().positive(),settlementMode:z.enum(["PAID","CREDIT"]),paymentTransactionId:z.string().optional().nullable(),note:z.string().max(500).optional().nullable()}).parse(req.body||{});
const result=await prisma.$transaction(async tx=>{const jobs=await tx.$queryRaw`SELECT "id","storeId","purchaseDocumentId" FROM "AiReaderJob" WHERE "id"=${req.params.jobId} AND "companyId"=${req.user.companyId} LIMIT 1 FOR UPDATE`;const job=jobs[0];if(!job)throw Object.assign(new Error("Δεν βρέθηκε η ανάγνωση."),{status:404});if(job.purchaseDocumentId)return {documentId:job.purchaseDocumentId,reused:true};
const suppliers=await tx.$queryRaw`SELECT "id","taxId" FROM "Supplier" WHERE "id"=${body.supplierId} AND "companyId"=${req.user.companyId} AND "active"=true LIMIT 1`;const supplier=suppliers[0];if(!supplier)throw Object.assign(new Error("Δεν βρέθηκε ο προμηθευτής."),{status:404});
const payment=body.settlementMode==="PAID"?await findInvoicePayment(tx,{companyId:req.user.companyId,supplierId:body.supplierId,supplierTaxId:String(supplier.taxId||"").replace(/\\D/g,""),documentNumber:body.documentNumber}):null;if(payment)assertReusableInvoicePayment(payment,{companyId:req.user.companyId,storeId:job.storeId,supplierId:body.supplierId,supplierTaxId:String(supplier.taxId||"").replace(/\\D/g,""),documentNumber:body.documentNumber,totalGross:body.totalGross});
const duplicate=await duplicateInvoice(tx,{companyId:req.user.companyId,supplierId:body.supplierId,documentNumber:body.documentNumber});if(duplicate)throw Object.assign(new Error("Το τιμολόγιο υπάρχει ήδη."),{status:409});const skeletonRows=skeletonDocumentId?await tx.$queryRaw`SELECT "id","purchaseOrderId" FROM "PurchaseDocument" WHERE "id"=${skeletonDocumentId} AND "companyId"=${req.user.companyId} AND "status"=\'DRAFT\' LIMIT 1 FOR UPDATE`:[];
...
if(skeletonRows[0])await tx.$executeRaw`UPDATE "PurchaseOrder" SET "description"=${body.note||`OCR V2.4.4 τιμολόγιο ${body.documentNumber} — έλεγχος πριν την οριστικοποίηση`},"updatedByName"=${actor},"updatedAt"=CURRENT_TIMESTAMP WHERE "id"=${orderId} AND "companyId"=${req.user.companyId}`;
      else await tx.$executeRaw`INSERT INTO "PurchaseOrder" ("id","companyId","storeId","supplierId","status","invoiceNumber","description","createdByUserId","createdByName","updatedByName","sourceType","sourceDocumentId") VALUES (${orderId},${req.user.companyId},${job.storeId},${body.supplierId},'NEW',${body.documentNumber},${body.note||"POS πρόχειρο — αναμονή πλήρους ανάγνωσης"},${createdByUserId},${actor},${actor},'POS_OCR_DRAFT',${documentId})`;
if(payment)await tx.$executeRaw`UPDATE "StoreTransaction" SET "attachmentMimeType"='application/vnd.myworkstation.purchase-document',"attachmentFilename"=${documentId},"invoiceDocumentNumber"=${body.documentNumber} WHERE "id"=${payment.id} AND "companyId"=${req.user.companyId}`;await tx.$executeRaw`UPDATE "AiReaderJob" SET "stage"='POS_DRAFT_READY',"status"='POS_DRAFT_READY',"purchaseDocumentId"=${documentId},"updatedAt"=CURRENT_TIMESTAMP WHERE "id"=${job.id} AND "companyId"=${req.user.companyId}`;return {documentId,orderId,reused:false};});res.status(result.reused?200:201).json({ok:true,status:"DRAFT",stockUpdated:false,awaitingApproval:true,...result});
