...
export function onlineProductAvailable(product,config,quantity=1){
  return isPreparedProduct(product)||!Boolean(config?.stockCheckEnabled)||!product?.trackStock||Number(product?.currentStock||0)>=Number(quantity||1);
}
...
router.get("/backoffice-managed/stores",auth,safe(async(req,res)=>{
  ...
  const rows=platform
    ?await prisma.$queryRaw`SELECT s."id",s."name",s."companyId",c."name" AS "companyName",(SELECT COUNT(*)::int FROM "OnlineOrder" o WHERE o."storeId"=s."id") AS "orderCount" FROM "Store" s JOIN "OnlineOrderingConfig" oc ON oc."storeId"=s."id" LEFT JOIN "Company" c ON c."id"=s."companyId" WHERE s."active"=TRUE AND oc."enabled"=TRUE ORDER BY s."createdAt"`
    :await prisma.$queryRaw`SELECT s."id",s."name",s."companyId",c."name" AS "companyName",(SELECT COUNT(*)::int FROM "OnlineOrder" o WHERE o."storeId"=s."id") AS "orderCount" FROM "Store" s JOIN "OnlineOrderingConfig" oc ON oc."storeId"=s."id" LEFT JOIN "Company" c ON c."id"=s."companyId" WHERE s."active"=TRUE AND oc."enabled"=TRUE AND s."companyId"=${req.user.companyId} ORDER BY s."createdAt"`;
...
router.get("/backoffice-managed/stores/:storeId/orders",auth,safe(async(req,res)=>{
  if(req.user?.tokenType==="STORE_OPERATOR")return res.status(403).json({error:"Η προβολή BackOffice δεν είναι διαθέσιμη από λογαριασμό POS."});
  const store=(await prisma.$queryRaw`SELECT s."id",s."name",s."companyId",s."active" FROM "Store" s JOIN "OnlineOrderingConfig" oc ON oc."storeId"=s."id" AND oc."enabled"=TRUE WHERE s."id"=${req.params.storeId} AND s."active"=TRUE LIMIT 1`)[0];
  ...
  await prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "PaymentDeviceRouteAttempt" ("id" TEXT PRIMARY KEY,"companyId" TEXT NOT NULL,"storeId" TEXT NOT NULL,"saleId" TEXT NOT NULL,"sessionId" TEXT NOT NULL,"terminalPos" TEXT NOT NULL,"channel" TEXT NOT NULL,"fiscalDeviceCode" TEXT NOT NULL,"eftposDeviceCode" TEXT NOT NULL,"role" TEXT NOT NULL,"status" TEXT NOT NULL DEFAULT 'PLANNED',"fallbackUsed" BOOLEAN NOT NULL DEFAULT FALSE,"amount" NUMERIC(14,2) NOT NULL,"idempotencyKey" TEXT NOT NULL,"createdAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),UNIQUE("storeId","idempotencyKey"))`);
  const rows=await prisma.$queryRaw`SELECT o.*,COALESCE((SELECT json_agg(json_build_object('id',l."id",'productId',l."productId",'productName',l."productName",'quantity',l."quantity",'onlineUnitPrice',l."onlineUnitPrice",'lineTotal',l."lineTotal",'modifiers',COALESCE(l."modifiersJson",'[]'::jsonb)) ORDER BY l."createdAt") FROM "OnlineOrderLine" l WHERE l."orderId"=o."id"),'[]') AS "items",CASE WHEN o."saleId" IS NULL THEN NULL ELSE (SELECT json_build_object('id',s."id",'total',s."total",'status',s."status",'source',s."source",'fiscalStatus',s."fiscalStatus",'createdAt',s."createdAt",'payments',COALESCE((SELECT json_agg(json_build_object('id',p."id",'method',p."method",'amount',p."amount")) FROM "Payment" p WHERE p."saleId"=s."id"),'[]'::json)) FROM "Sale" s WHERE s."id"=o."saleId" LIMIT 1) END AS "sale",(SELECT json_build_object('id',t."id",'sessionId',t."sessionId",'type',t."type",'amount',t."amount",'description',t."description",'actorName',t."actorName",'createdAt',t."createdAt") FROM "StoreTransaction" t WHERE t."storeId"=o."storeId" AND t."description" LIKE ${'ONLINE %'} || o."orderNumber" || '%' ORDER BY t."createdAt" DESC LIMIT 1) AS "shiftTransaction" FROM "OnlineOrder" o WHERE o."storeId"=${store.id} ORDER BY o."createdAt" DESC LIMIT ${limit}`;
  const saleIds=rows.map(row=>row.saleId).filter(Boolean),orderIds=rows.map(row=>row.id);
  const [attempts,movements,fiscalCounts,saleLinkCounts,movementDuplicates,terminalEvidence]=await Promise.all([
    saleIds.length?prisma.$queryRaw`SELECT DISTINCT ON ("saleId") "id","saleId","sessionId","terminalPos","status","eftposDeviceCode","fiscalDeviceCode","channel","role","amount","createdAt" FROM "PaymentDeviceRouteAttempt" WHERE "storeId"=${store.id} AND "saleId"=ANY(${saleIds}::text[]) ORDER BY "saleId","createdAt" DESC`:[],
    orderIds.length?prisma.$queryRaw`SELECT "id","sourceId","productId","movementType","quantity","sourceType","idempotencyKey","createdAt" FROM "StockMovement" WHERE "storeId"=${store.id} AND "sourceId"=ANY(${orderIds}::text[]) ORDER BY "createdAt"`:[],
    saleIds.length?prisma.$queryRaw`SELECT "saleId",COUNT(*)::int AS "count" FROM "FiscalDocument" WHERE "saleId"=ANY(${saleIds}::text[]) GROUP BY "saleId"`:[],
    saleIds.length?prisma.$queryRaw`SELECT "saleId",COUNT(*)::int AS "count" FROM "OnlineOrder" WHERE "storeId"=${store.id} AND "saleId"=ANY(${saleIds}::text[]) GROUP BY "saleId"`:[],
    orderIds.length?prisma.$queryRaw`SELECT "sourceId","idempotencyKey",COUNT(*)::int AS "count" FROM "StockMovement" WHERE "storeId"=${store.id} AND "sourceId"=ANY(${orderIds}::text[]) AND "idempotencyKey" IS NOT NULL GROUP BY "sourceId","idempotencyKey" HAVING COUNT(*)>1`:[],
    saleIds.length?prisma.$queryRaw`SELECT s."id" AS "saleId",st."sessionId",cs."terminalPos" FROM "Sale" s LEFT JOIN LATERAL (SELECT t."sessionId" FROM "StoreTransaction" t WHERE t."storeId"=s."storeId" AND COALESCE(t."description",'') LIKE ('%'||s."id"||'%') ORDER BY t."createdAt" ASC LIMIT 1) st ON TRUE LEFT JOIN "CashShiftSession" cs ON cs."id"=st."sessionId" WHERE s."storeId"=${store.id} AND s."id"=ANY(${saleIds}::text[])`:[]
  ]);
  const attemptBySale=new Map(attempts.map(row=>[row.saleId,row])),terminalBySale=new Map(terminalEvidence.map(row=>[row.saleId,row])),fiscalCountBySale=new Map(fiscalCounts.map(row=>[row.saleId,Number(row.count||0)])),saleLinkCountBySale=new Map(saleLinkCounts.map(row=>[row.saleId,Number(row.count||0)])),duplicateMovementOrders=new Set(movementDuplicates.map(row=>row.sourceId)),movementsByOrder=new Map();
  for(const movement of movements){const list=movementsByOrder.get(movement.sourceId)||[];list.push({...movement,quantity:Number(movement.quantity||0)});movementsByOrder.set(movement.sourceId,list)}
  const normalized=rows.map(row=>{const fiscalCount=row.saleId?fiscalCountBySale.get(row.saleId)||0:0,saleLinkCount=row.saleId?saleLinkCountBySale.get(row.saleId)||0:0,attempt=attemptBySale.get(row.saleId)||null,terminal=terminalBySale.get(row.saleId)||null,expectedRole=row.fulfillmentType==="DELIVERY"?"DELIVERY":"STORE",issues=[];if(fiscalCount>1)issues.push("DUPLICATE_FISCAL_DOCUMENT");if(saleLinkCount>1)issues.push("DUPLICATE_SALE_LINK");if(duplicateMovementOrders.has(row.id))issues.push("DUPLICATE_STOCK_MOVEMENT");if(attempt&&terminal?.terminalPos&&String(attempt.terminalPos)!==String(terminal.terminalPos))issues.push("SHIFT_TERMINAL_MISMATCH");if(attempt&&attempt.role!==expectedRole)issues.push("EFTPOS_ROLE_MISMATCH");if(attempt&&attempt.sessionId&&terminal?.sessionId&&attempt.sessionId!==terminal.sessionId)issues.push("SHIFT_SESSION_MISMATCH");return{...row,subtotal:money(row.subtotal),deliveryFee:money(row.deliveryFee),total:money(row.total),items:(row.items||[]).map(item=>({...item,quantity:Number(item.quantity||0),onlineUnitPrice:money(item.onlineUnitPrice),lineTotal:money(item.lineTotal)})),sale:row.sale?{...row.sale,total:money(row.sale.total),payments:(row.sale.payments||[]).map(payment=>({...payment,amount:money(payment.amount)}))}:null,shiftTransaction:row.shiftTransaction?{...row.shiftTransaction,amount:money(row.shiftTransaction.amount)}:null,terminalEvidence:terminal,eftposAttempt:attempt,stockMovements:movementsByOrder.get(row.id)||[],integrity:{fiscalCount,saleLinkCount,duplicateStockMovement:duplicateMovementOrders.has(row.id),issues,ok:issues.length===0}}});
