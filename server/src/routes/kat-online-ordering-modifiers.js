...
router.get("/backoffice-managed/stores/:storeId/orders",auth,safe(async(req,res)=>{
  ...
  const [attempts,movements]=await Promise.all([
    ...
    orderIds.length?prisma.$queryRaw`SELECT "id","sourceId","productId","movementType","quantity","sourceType","createdAt" FROM "StockMovement" WHERE "storeId"=${store.id} AND "sourceId"=ANY(${orderIds}::text[]) ORDER BY "createdAt"`:[]
  ]);
  const attemptBySale=new Map(attempts.map(row=>[row.saleId,row])),movementsByOrder=new Map();
