...
router.get("/backoffice-managed/stores/:storeId/orders",auth,safe(async(req,res)=>{
  ...
  const [attempts,movements,fiscalCounts,saleLinkCounts,movementDuplicates]=await Promise.all([
    saleIds.length?prisma.$queryRaw`SELECT DISTINCT ON ("saleId") "id","saleId","status","eftposDeviceCode","fiscalDeviceCode","channel","role","amount","createdAt" FROM "PaymentDeviceRouteAttempt" WHERE "storeId"=${store.id} AND "saleId"=ANY(${saleIds}::text[]) ORDER BY "saleId","createdAt" DESC`:[],
  ...
  const attemptBySale=new Map(attempts.map(row=>[row.saleId,row])),fiscalCountBySale=new Map(fiscalCounts.map(row=>[row.saleId,Number(row.count||0)])),saleLinkCountBySale=new Map(saleLinkCounts.map(row=>[row.saleId,Number(row.count||0)])),duplicateMovementOrders=new Set(movementDuplicates.map(row=>row.sourceId)),movementsByOrder=new Map();
  ...
  const normalized=rows.map(row=>{const fiscalCount=row.saleId?fiscalCountBySale.get(row.saleId)||0:0,saleLinkCount=row.saleId?saleLinkCountBySale.get(row.saleId)||0:0,issues=[];if(fiscalCount>1)issues.push("DUPLICATE_FISCAL_DOCUMENT");if(saleLinkCount>1)issues.push("DUPLICATE_SALE_LINK");if(duplicateMovementOrders.has(row.id))issues.push("DUPLICATE_STOCK_MOVEMENT");return{...row,subtotal:money(row.subtotal),deliveryFee:money(row.deliveryFee),total:money(row.total),items:(row.items||[]).map(item=>({...item,quantity:Number(item.quantity||0),onlineUnitPrice:money(item.onlineUnitPrice),lineTotal:money(item.lineTotal)})),sale:row.sale?{...row.sale,total:money(row.sale.total),payments:(row.sale.payments||[]).map(payment=>({...payment,amount:money(payment.amount)}))}:null,shiftTransaction:row.shiftTransaction?{...row.shiftTransaction,amount:money(row.shiftTransaction.amount)}:null,eftposAttempt:attemptBySale.get(row.saleId)||null,stockMovements:movementsByOrder.get(row.id)||[],integrity:{fiscalCount,saleLinkCount,duplicateStockMovement:duplicateMovementOrders.has(row.id),issues,ok:issues.length===0}}});
