...
async function learnConfirmedLineCorrection(tx,{companyId,supplierId,line,userId}){
  ...
  await tx.$executeRaw`
    INSERT INTO "SupplierProductMapping" (
      "id","companyId","supplierId","supplierItemCode","productId","lastDescription","unitsPerPackage","lastUnitCost","lastDiscount1","lastDiscount2","lastDiscount3","lastExcisePerInvoiceUnit","lastMarkupPercent","usageCount","confirmedByUserId","confirmedAt","lastSeenAt","createdAt","updatedAt"
    ) VALUES (
      ${id()},${companyId},${supplierId},${supplierCode},${productId},${line.description||null},${Math.max(1,Number(line.stockUnitsPerInvoiceUnit||1))},${n(line.unitCost)},${n(line.discount1)},${n(line.discount2)},${n(line.discount3)},${n(line.exciseTotal)/quantity},${n(line.markupPercent)},1,${userId||null},NOW(),NOW(),NOW(),NOW()
    )
...
const auditLineState=line=>({
  productId:line.productId||null,supplierCode:line.supplierCode||null,description:line.description||null,
  quantity:n(line.quantity),unitCost:n(line.unitCost),invoiceUnit:line.invoiceUnit||"PIECE",
  stockUnitsPerInvoiceUnit:Math.max(1,n(line.stockUnitsPerInvoiceUnit)||1),discount1:n(line.discount1),
  discount2:n(line.discount2),discount3:n(line.discount3),exciseTotal:n(line.exciseTotal),vatRate:n(line.vatRate),
  markupPercent:n(line.markupPercent),proposedSalePrice:n(line.proposedSalePrice),netAmount:n(line.netAmount),
  vatAmount:n(line.vatAmount),grossAmount:n(line.grossAmount)
});
async function writePurchaseAudit(tx,req,{storeId,eventType,details}){
  await tx.$executeRaw`INSERT INTO "StoreOperatorAudit" ("id","companyId","storeId","operatorId","actorId","eventType","details") VALUES (${id()},${req.user.companyId},${storeId},${req.user.operatorId||null},${req.user.id},${eventType},${JSON.stringify({...details,stockChanged:false})}::jsonb)`;
}
const lineSchema=z.object({productId:z.string().optional().nullable(),supplierCode:z.string().trim().max(100).optional().nullable(),description:z.string().trim().min(1).max(250),quantity:z.coerce.number().positive().max(1000000).optional(),unitCost:z.coerce.number().min(0).max(1000000).optional(),invoiceUnit:z.enum(["PIECE","PACKAGE"]).optional(),stockUnitsPerInvoiceUnit:z.coerce.number().min(1).max(100000).optional(),discount1:z.coerce.number().min(-100).max(100).optional(),discount2:z.coerce.number().min(-100).max(100).optional(),discount3:z.coerce.number().min(-100).max(100).optional(),exciseTotal:z.coerce.number().min(0).max(1000000).optional(),vatRate:z.coerce.number().min(0).max(100).optional(),gift:z.boolean().optional(),markupPercent:z.coerce.number().min(-100).max(10000).optional(),proposedSalePrice:z.coerce.number().min(0).max(1000000).optional(),calculateFrom:z.enum(["MARKUP","RETAIL","NONE"]).optional()});
...
router.get("/report",async(req,res,next)=>{try{
  ...
  const rows=await prisma.$queryRaw`SELECT o."id",o."status",o."invoiceNumber",o."description",o."createdAt",o."updatedAt",o."createdByName",o."updatedByName",s."name" AS "supplierName",st."name" AS "storeName",COUNT(l."id")::int AS "lineCount",COALESCE(SUM(ROUND(l."netAmount"+l."exciseTotal",2)),0) AS "totalNet",COALESCE(SUM(l."vatAmount"),0) AS "totalVat",COALESCE(SUM(l."grossAmount"),0) AS "totalGross",(SELECT jsonb_build_object('status',j."status",'stage',j."stage",'updatedAt',j."updatedAt",'error',LEFT(COALESCE(j."resultJson"->'posBackground'->>'error',''),700)) FROM "AiReaderJob" j WHERE j."companyId"=o."companyId" AND j."purchaseDocumentId"=o."sourceDocumentId" ORDER BY j."updatedAt" DESC LIMIT 1) AS "ocrJob" FROM "PurchaseOrder" o JOIN "Store" st ON st."id"=o."storeId" AND st."companyId"=o."companyId" LEFT JOIN "Supplier" s ON s."id"=o."supplierId" LEFT JOIN "PurchaseOrderLine" l ON l."orderId"=o."id" WHERE o."companyId"=${companyId} AND o."createdAt">=${from} AND o."createdAt"<=${to} AND (${storeId}::text IS NULL OR o."storeId"=${storeId}) AND (${supplierId}::text IS NULL OR o."supplierId"=${supplierId}) AND (${status}='ALL' OR o."status"=${status}) AND (${text}::text IS NULL OR COALESCE(o."invoiceNumber",'') ILIKE ${text} OR COALESCE(o."description",'') ILIKE ${text} OR COALESCE(s."name",'') ILIKE ${text}) GROUP BY o."id",o."companyId",o."sourceDocumentId",s."name",st."name" ORDER BY o."updatedAt" DESC LIMIT 800`;
  const orders=rows.map(r=>({...r,lineCount:n(r.lineCount),totalNet:n(r.totalNet),totalVat:n(r.totalVat),totalGross:n(r.totalGross)}));const summary=orders.reduce((a,r)=>{a.count++;a.net=money2(a.net+r.totalNet);a.gross=money2(a.gross+r.totalGross);a[r.status]=(a[r.status]||0)+1;return a},{count:0,net:0,gross:0,NEW:0,FINAL:0,INVOICED:0});res.json({from,to,stores,suppliers,orders,summary});
...
router.post("/",async(req,res,next)=>{try{
  ...
  await prisma.$transaction(async tx=>{await tx.$executeRaw`INSERT INTO "PurchaseOrder" ("id","companyId","storeId","supplierId","invoiceNumber","description","createdByUserId","createdByName","updatedByName") VALUES (${orderId},${companyId},${body.storeId},${body.supplierId||null},${body.invoiceNumber||null},${body.description||null},${req.user.id},${actor},${actor})`;await writePurchaseAudit(tx,req,{storeId:body.storeId,eventType:"PURCHASE_ORDER_DRAFT_CREATED",details:{orderId,invoiceNumber:body.invoiceNumber||null,supplierId:body.supplierId||null,sourceType:"MANUAL"}})});res.status(201).json({id:orderId,status:"NEW"});
}catch(error){next(error)}});
...
router.post("/:orderId/lines",async(req,res,next)=>{try{
  ...
  const created={...body,...c,id:lineId,description:body.description||p?.name||"Είδος",invoiceUnit:body.invoiceUnit||"PIECE",stockUnitsPerInvoiceUnit:body.invoiceUnit==="PACKAGE"?body.stockUnitsPerInvoiceUnit||1:1};
  await prisma.$transaction(async tx=>{await tx.$executeRaw`INSERT INTO "PurchaseOrderLine" ("id","orderId","productId","supplierCode","description","quantity","unitCost","invoiceUnit","stockUnitsPerInvoiceUnit","discount1","discount2","discount3","exciseTotal","vatRate","gift","initialUnitCost","markupPercent","proposedSalePrice","netAmount","vatAmount","grossAmount") VALUES (${lineId},${found.id},${body.productId||null},${body.supplierCode||null},${created.description},${c.quantity},${c.unitCost},${created.invoiceUnit},${created.stockUnitsPerInvoiceUnit},${c.discount1},${c.discount2},${c.discount3},${c.exciseTotal},${c.vatRate},${body.gift||false},${c.unitCost},${c.markupPercent},${c.proposedSalePrice},${c.netAmount},${c.vatAmount},${c.grossAmount})`;await writePurchaseAudit(tx,req,{storeId:found.storeId,eventType:"PURCHASE_ORDER_LINE_ADDED",details:{orderId:found.id,lineId,invoiceNumber:found.invoiceNumber,line:auditLineState(created)}})});res.status(201).json({id:lineId,...c});
}catch(error){next(error)}});
...
router.patch("/:orderId/lines/:lineId",async(req,res,next)=>{try{
  ...
  const invoiceUnit=body.invoiceUnit??current.invoiceUnit??"PIECE",stockUnitsPerInvoiceUnit=invoiceUnit==="PACKAGE"?Math.max(1,n(body.stockUnitsPerInvoiceUnit??current.stockUnitsPerInvoiceUnit??1)):1;
  const corrected={...current,...body,...c,invoiceUnit,stockUnitsPerInvoiceUnit,productId:body.productId??current.productId,supplierCode:body.supplierCode??current.supplierCode,description:body.description??current.description};
  let centralLearning={learned:false,scope:"COMPANY"};
  const mappingLearned=await prisma.$transaction(async tx=>{await tx.$executeRaw`UPDATE "PurchaseOrderLine" SET "productId"=COALESCE(${body.productId??null},"productId"),"supplierCode"=COALESCE(${body.supplierCode??null},"supplierCode"),"description"=COALESCE(${body.description??null},"description"),"quantity"=${c.quantity},"unitCost"=${c.unitCost},"invoiceUnit"=${invoiceUnit},"stockUnitsPerInvoiceUnit"=${stockUnitsPerInvoiceUnit},"discount1"=${c.discount1},"discount2"=${c.discount2},"discount3"=${c.discount3},"exciseTotal"=${c.exciseTotal},"vatRate"=${c.vatRate},"gift"=COALESCE(${body.gift??null},"gift"),"markupPercent"=${c.markupPercent},"proposedSalePrice"=${c.proposedSalePrice},"netAmount"=${c.netAmount},"vatAmount"=${c.vatAmount},"grossAmount"=${c.grossAmount},"resolutionStatus"='MATCHED',"ocrReviewReasons"=NULL,"updatedAt"=NOW() WHERE "id"=${current.id}`;const learned=await learnConfirmedLineCorrection(tx,{companyId,supplierId:found.supplierId,line:corrected,userId:req.user.id});centralLearning=await learnCentralInvoiceCorrection(tx,{actor:req.user,companyId,supplierId:found.supplierId,line:corrected});await tx.$executeRaw`INSERT INTO "StoreOperatorAudit" ("id","companyId","storeId","operatorId","actorId","eventType","details") VALUES (${id()},${companyId},${found.storeId},${req.user.operatorId||null},${req.user.id},'INVOICE_LINE_CORRECTED',${JSON.stringify({orderId:found.id,lineId:current.id,invoiceNumber:found.invoiceNumber,supplierId:found.supplierId,before:auditLineState(current),after:auditLineState(corrected),supplierMappingLearned:learned,centralLearning,stockChanged:false})}::jsonb)`;return learned});res.json({ok:true,...c,invoiceUnit,stockUnitsPerInvoiceUnit,stockQuantity:c.quantity*stockUnitsPerInvoiceUnit,mappingLearned,centralLearning});
}catch(error){next(error)}});

router.delete("/:orderId/lines/:lineId",async(req,res,next)=>{try{const companyId=req.user.companyId,found=await order(companyId,req.params.orderId),current=await line(companyId,req.params.lineId);if(!found||!current||current.orderId!==found.id)return res.status(404).json({error:"Δεν βρέθηκε η γραμμή."});editable(found);await prisma.$transaction(async tx=>{await tx.$executeRaw`DELETE FROM "PurchaseOrderLine" WHERE "id"=${current.id}`;await writePurchaseAudit(tx,req,{storeId:found.storeId,eventType:"PURCHASE_ORDER_LINE_DELETED",details:{orderId:found.id,lineId:current.id,invoiceNumber:found.invoiceNumber,line:auditLineState(current)}})});res.json({ok:true})}catch(error){next(error)}});
