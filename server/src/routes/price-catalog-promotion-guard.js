...
const promotionBody=z.object({
  productId:z.string().min(1).optional(),promotionType:z.enum(["LEAFLET","GIFT"]).optional(),offerPrice:z.coerce.number().min(0).nullable().optional(),discountPercent:z.coerce.number().min(0).max(100).default(0),saleQuantity:z.coerce.number().positive().max(9999).default(1),bonusQuantity:z.coerce.number().min(0).max(9999).default(0),customerPoints:z.coerce.number().min(0).max(999999).default(0),validFrom:z.union([z.string(),z.date()]),validUntil:z.union([z.string(),z.date()]).nullable().optional(),active:z.boolean().default(true),storeIds:storeIdsSchema
});
...
router.patch("/promotions/:promotionId/scoped",async(req,res,next)=>{try{
  ...
  await prisma.$transaction(async tx=>{await tx.$executeRaw`UPDATE "PriceCatalogPromotion" SET "offerPrice"=${offerPrice},"discountPercent"=${discount},"saleQuantity"=${b.saleQuantity===undefined?n(old.saleQuantity):b.saleQuantity},"bonusQuantity"=${b.bonusQuantity===undefined?n(old.bonusQuantity):b.bonusQuantity},"customerPoints"=${b.customerPoints===undefined?n(old.customerPoints):b.customerPoints},"validFrom"=${validFrom},"validUntil"=${validUntil},"active"=${active},"updatedAt"=NOW() WHERE "id"=${old.id} AND "companyId"=${companyId}`;await replaceStores(tx,companyId,old.id,stores)});
  res.json({ok:true,id:old.id,storeIds:stores.map(s=>s.id),posActive:active&&stores.length>0});
}catch(error){if(error?.name==="ZodError")return res.status(400).json({error:"Ελέγξτε τα στοιχεία και τα καταστήματα της προσφοράς.",details:error.issues});next(error)}});

router.put("/promotions/:promotionId/stores",async(req,res,next)=>{try{
  const companyId=req.user.companyId,promotion=await ownedPromotion(companyId,req.params.promotionId);if(!promotion)return res.status(404).json({error:"Δεν βρέθηκε η προσφορά."});const b=scopeBody.parse(req.body||{}),stores=await storesFor(companyId,b.storeIds),validFrom=new Date(promotion.validFrom),validUntil=promotion.validUntil?new Date(promotion.validUntil):null;const overlaps=promotion.active?await findOverlap(prisma,{companyId,productId:promotion.productId,promotionType:promotion.promotionType,validFrom,validUntil,storeIds:stores.map(s=>s.id),excludePromotionId:promotion.id}):[];if(overlaps.length)return overlapResponse(res,overlaps);await prisma.$transaction(tx=>replaceStores(tx,companyId,promotion.id,stores));res.json({ok:true,storeIds:stores.map(s=>s.id),posActive:promotion.active&&stores.length>0});
}catch(error){if(error?.name==="ZodError")return res.status(400).json({error:"Μη έγκυρη επιλογή καταστημάτων.",details:error.issues});next(error)}});
