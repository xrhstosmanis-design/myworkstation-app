...
router.post("/:productId/supplier-return",requireCompanyModule("INVENTORY"),async(req,res,next)=>{
  try{
    const company=companyId(req);if(!company)return res.status(403).json({error:"Δεν υπάρχει ενεργή εταιρεία."});
    const body=z.object({storeId:z.string().min(1),supplierId:z.string().min(1),quantity:z.coerce.number().positive().max(100000000),reason:z.string().trim().min(3).max(300),documentNumber:z.string().trim().min(1).max(80),destinationAddress:z.string().trim().min(3).max(300),dispatchedAt:z.coerce.date().optional().nullable()}).parse(req.body||{});
    const [rows,supplierRows]=await Promise.all([
      ...
      prisma.$queryRaw`SELECT "id","name","taxId" FROM "Supplier" WHERE "companyId"=${company} AND "id"=${body.supplierId} AND "active"=true LIMIT 1`
    ]);
    ...
    const current=Number(row.currentStock||0);if(current<0||body.quantity>current)return res.status(400).json({error:"Το δελτίο δεν μπορεί να περιλαμβάνει ποσότητα μεγαλύτερη από το διαθέσιμο stock."});
    const duplicate=await prisma.$queryRaw`SELECT "id" FROM "DispatchNote" WHERE "companyId"=${company} AND "documentNumber"=${body.documentNumber} LIMIT 1`;if(duplicate[0])return res.status(409).json({error:"Ο αριθμός δελτίου χρησιμοποιείται ήδη."});
    const dispatchNoteId=uid();
    await prisma.$transaction(async tx=>{
      await tx.$executeRaw`INSERT INTO "DispatchNote" ("id","companyId","storeId","documentNumber","direction","recipientName","recipientTaxId","destinationAddress","purpose","dispatchedAt","createdByUserId") VALUES (${dispatchNoteId},${company},${body.storeId},${body.documentNumber},'OUTBOUND',${supplier.name},${supplier.taxId||null},${body.destinationAddress},${`Επιστροφή σε προμηθευτή · ${body.reason}`},${body.dispatchedAt||null},${req.user.id})`;
      await tx.$executeRaw`INSERT INTO "DispatchNoteLine" ("id","dispatchNoteId","productId","description","quantity","unit") VALUES (${uid()},${dispatchNoteId},${req.params.productId},${row.name},${body.quantity},'PIECE')`;
    });
    res.status(201).json({ok:true,dispatchNoteId,status:"DRAFT",fiscalStatus:"NOT_SUBMITTED",stockChanged:false,currentStock:current,message:"Το δελτίο αποθηκεύτηκε ως πρόχειρο. Το stock δεν άλλαξε πριν από επιτυχημένη διαβίβαση myDATA."});
  }catch(error){next(error)}
});
