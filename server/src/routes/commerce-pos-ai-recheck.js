...
router.put("/ai-reader/jobs/:jobId/product-lines",requireCompanyModule("AI_READER"),async(req,res,next)=>{try{
  ...
  const backgroundMayFillLinkedDraft=Boolean(job.purchaseDocumentId&&body.source==="V2.4.4"&&["AI_COMPLETE","POS_PROCESSING"].includes(job.status)&&req.user?.tokenType==="POS_BACKGROUND"&&job.resultJson?.posHandoff&&job.documentSourceType==="POS_OCR_DRAFT"&&job.documentStatus==="DRAFT");
