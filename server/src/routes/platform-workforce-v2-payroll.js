...
import {overlapDays,paymentBalance,payrollClosingSummary,payrollGross,payrollableAttendanceWhere,payrollWorkDate,shiftDurationMinutes} from "../workforce-v2-payroll.js";
...
async function addPayrollEvidence(context,from,to,rows){
  ...
  const [assignments,absences,leaves]=await Promise.all([
    prisma.workforceScheduleAssignment.findMany({where:{employeeId:{in:employeeIds},date:{gte:from,lt:to},schedule:{companyId:context.company.id,storeId:context.store.id,status:{in:["APPROVED","PUBLISHED","LOCKED"]}}},include:{shiftTemplate:true}}),
    prisma.workforceAbsence.findMany({where:{companyId:context.company.id,employeeId:{in:employeeIds},startsAt:{lt:to},OR:[{endsAt:null},{endsAt:{gte:from}}],AND:[{OR:[{storeId:null},{storeId:context.store.id}]},{approvedByUserId:{not:null}}]}}),
    prisma.workforceLeaveRequest.findMany({where:{companyId:context.company.id,employeeId:{in:employeeIds},status:"APPROVED",startDate:{lt:to},endDate:{gte:from},OR:[{storeId:null},{storeId:context.store.id}]}})
  ]);
...
router.post("/periods",async(req,res,next)=>{
  try{
    ...
    const [sessions,employees]=await Promise.all([prisma.workforceAttendanceSession.findMany({where:{companyId:context.company.id,storeId:context.store.id,startedAt:{gte:from,lt:to},status:payrollableAttendanceWhere()},include:{employee:{include:{hourlyRates:{where:{validFrom:{lte:to},OR:[{validTo:null},{validTo:{gt:from}}]},orderBy:{validFrom:"desc"},take:1}}}}}),payrollEmployees(context,from,to)]);
    const grouped=seedPayrollRows(employees); for(const s of sessions){const e=s.employee,r=e.hourlyRates[0]?.hourlyRate??null,row=grouped.get(e.id)||{employeeId:e.id,paymentType:e.paymentType,actualMinutes:0,overtimeMinutes:0,hourlyRate:r,dailyRate:e.paymentType==="DAILY"?e.dailyRate:null,fixedAmount:e.paymentType==="FIXED_MONTHLY"?e.fixedMonthlyAmount:null,workDates:new Set()};row.actualMinutes+=s.workedMinutes;row.overtimeMinutes+=s.overtimeMinutes;row.workDates.add(payrollWorkDate(s.startedAt));grouped.set(e.id,row);}
    await addPayrollEvidence(context,from,to,grouped);
    const period=await prisma.workforcePayrollPeriod.create({data:{companyId:context.company.id,storeId:context.store.id,name,periodStart:from,periodEnd:to,createdByUserId:req.user?.id||null,lines:{create:[...grouped.values()].map(row=>{const workDays=row.workDates.size,grossAmount=payrollGross({...row,workDays});return {employeeId:row.employeeId,plannedMinutes:row.plannedMinutes,actualMinutes:row.actualMinutes,overtimeMinutes:row.overtimeMinutes,absenceMinutes:row.absenceMinutes,hourlyRate:row.hourlyRate,dailyRate:row.dailyRate,fixedAmount:row.fixedAmount,grossAmount:Number(grossAmount||0),balanceAmount:Number(grossAmount||0),calculationJson:{source:"WORKFORCE_ATTENDANCE",paymentType:row.paymentType,workDays,approvedLeaveDays:row.approvedLeaveDays,approvedLeaveTypes:row.approvedLeaveTypes}}})}}});
