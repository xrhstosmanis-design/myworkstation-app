...
router.post("/stores/:storeId/orders/:orderId/complete-from-pos",async(req,res,next)=>{
  try{
    ...
    const result=await prisma.$transaction(async tx=>{
      ...
      const routingSessionTerminal=normalizeTerminalPos(req.user?.terminalPos)===normalizeTerminalPos(configuredTerminalPos)?req.user?.terminalPos:"";
      const routing=resolveKatOnlineRouting({configuredTerminalPos,currentTerminalPos:routingSessionTerminal});
