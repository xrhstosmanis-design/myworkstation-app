...
import {learnCentralInvoiceCorrection} from "../lib/invoice-correction-learning.js";
...
const ean13=data=>{const digits=String(data).replace(/\D/g,"").slice(0,12).padEnd(12,"0"),sum=[...digits].reduce((total,digit,index)=>total+Number(digit)*(index%2===0?1:3),0);return `${digits}${(10-sum%10)%10}`};
async function generateInternalBarcode(tx,companyId){
  for(let attempt=0;attempt<20;attempt++){
    const data=`29${crypto.randomInt(0,10_000_000_000).toString().padStart(10,"0")}`;
    const barcode=ean13(data);
    const used=await tx.$queryRaw`SELECT 1 FROM "ProductBarcode" pb JOIN "Product" p ON p."id"=pb."productId" WHERE p."companyId"=${companyId} AND pb."barcode"=${barcode} LIMIT 1`;
    if(!used[0])return barcode;
  }
  throw new Error("Δεν ήταν δυνατή η δημιουργία μοναδικού εσωτερικού barcode.");
}
...
async function learnSupplierMapping(tx,{companyId,supplierId,supplierCode,productId,barcode,description,userId,unitCost,unitsPerPackage,discount1,discount2,discount3,excisePerInvoiceUnit,markupPercent}){
  ...
  await tx.$executeRaw`
    INSERT INTO "SupplierProductMapping" (
      ...
      ${id()},${companyId},${supplierId},${code},${productId},${barcode||null},${description||null},${Number(unitsPerPackage||0)>1?Number(unitsPerPackage):null},${Number(unitCost||0)},${Number(discount1||0)},${Number(discount2||0)},${Number(discount3||0)},${Number(excisePerInvoiceUnit||0)},${Number(markupPercent||0)},1,${userId||null},NOW(),NOW(),NOW(),NOW()
    )
...
router.get("/:orderId/ocr-lines",async(req,res,next)=>{
  try{
    ...
    const rows=await prisma.$queryRaw`
      SELECT l."id",l."productId",l."supplierCode",l."description",l."quantity",l."unitCost",l."discount1",l."discount2",l."discount3",l."exciseTotal",l."vatRate",l."netAmount",l."vatAmount",l."grossAmount",l."invoiceUnit",l."stockUnitsPerInvoiceUnit",l."ocrRawText",l."ocrConfidence",l."resolutionStatus",l."ocrReviewReasons",l."detectedBarcode",l."ocrSequence",l."ocrLineIndex",p."name" AS "productName",p."sku",p."salePrice",p."costPrice",COALESCE((SELECT json_agg(pb."barcode" ORDER BY pb."barcode") FROM "ProductBarcode" pb WHERE pb."productId"=p."id"),'[]') AS "barcodes" FROM "PurchaseOrderLine" l LEFT JOIN "Product" p ON p."id"=l."productId" AND p."companyId"=${req.user.companyId} WHERE l."orderId"=${order.id} ORDER BY COALESCE(l."ocrSequence",l."ocrLineIndex",2147483647),l."createdAt",l."id"`;
    const mapped=rows.map((r,index)=>{const quantity=Number(r.quantity||0),unitCost=Number(r.unitCost||0),grossAmount=Number(r.grossAmount||0);const economicProduct=Boolean(String(r.description||r.ocrRawText||"").trim())&&quantity>0&&(unitCost>0||grossAmount>0);return {...r,quantity,unitCost,discount1:Number(r.discount1||0),discount2:Number(r.discount2||0),discount3:Number(r.discount3||0),exciseTotal:Number(r.exciseTotal||0),vatRate:Number(r.vatRate||0),netAmount:Number(r.netAmount||0),vatAmount:Number(r.vatAmount||0),grossAmount,ocrConfidence:Number(r.ocrConfidence||0),ocrSequence:Number(r.ocrSequence||r.ocrLineIndex||index+1),ocrLineType:economicProduct?"PRODUCT":"INFO"}});
...
router.post("/:orderId/ocr-lines/:lineId/resolve-existing",async(req,res,next)=>{
  try{
    ...
    await prisma.$transaction(async tx=>{if(body.addBarcode&&barcode){const duplicate=await tx.$queryRaw`SELECT p."id",p."name" FROM "ProductBarcode" pb JOIN "Product" p ON p."id"=pb."productId" WHERE p."companyId"=${req.user.companyId} AND pb."barcode"=${barcode} LIMIT 1`;if(duplicate[0]&&duplicate[0].id!==product.id){const error=new Error(`Το barcode ${barcode} ανήκει ήδη στο προϊόν «${duplicate[0].name}».`);error.status=409;throw error}if(!duplicate[0])await tx.$executeRaw`INSERT INTO "ProductBarcode" ("id","productId","barcode","unitMultiplier") VALUES (${id()},${product.id},${barcode},1)`}await tx.$executeRaw`UPDATE "PurchaseOrderLine" SET "productId"=${product.id},"resolutionStatus"='MATCHED',"updatedAt"=NOW() WHERE "id"=${line.id}`;await learnSupplierMapping(tx,{companyId:req.user.companyId,supplierId:line.supplierId,supplierCode:line.supplierCode,productId:product.id,barcode,description:line.description,userId:req.user.id,unitCost:line.unitCost});await learnCentralInvoiceCorrection(tx,{actor:req.user,companyId:req.user.companyId,supplierId:line.supplierId,line})});
...
router.post("/:orderId/ocr-lines/:lineId/create-product",async(req,res,next)=>{
  try{
    const body=z.object({name:z.string().trim().min(2).max(250),supplierCode:z.string().trim().max(100).optional().nullable(),barcodeMode:z.enum(["PROVIDED","GENERATED","NONE"]).optional().default("NONE"),barcode:z.string().trim().max(80).optional().nullable(),quantity:z.coerce.number().positive().max(1000000).optional(),gift:z.boolean().optional(),unitCost:z.coerce.number().min(0).max(1000000).optional(),invoiceUnit:z.enum(["PIECE","PACKAGE"]).optional().default("PIECE"),unitsPerPackage:z.coerce.number().min(1).max(100000).optional().default(1),discount1:z.coerce.number().min(0).max(100).default(0),discount2:z.coerce.number().min(0).max(100).default(0),discount3:z.coerce.number().min(0).max(100).default(0),exciseTotal:z.coerce.number().min(0).max(1000000).default(0),vatRate:z.coerce.number().min(0).max(100).default(24),markupPercent:z.coerce.number().min(-100).max(10000).default(0),salePrice:z.coerce.number().min(0).default(0),categoryId:z.string().optional().nullable(),subcategoryId:z.string().optional().nullable()}).parse(req.body||{});
    const line=await ownedLine(req.user.companyId,req.params.orderId,req.params.lineId);if(!line)return res.status(404).json({error:"Δεν βρέθηκε η γραμμή."});if(line.status!=="NEW")return res.status(409).json({error:"Νέο προϊόν από τιμολόγιο δημιουργείται μόνο πριν την Οριστικοποίηση."});const barcode=body.barcodeMode!=="NONE"?String(body.barcode||"").trim():"";
    if(body.barcodeMode==="PROVIDED"&&!/^\d{6,18}$/.test(String(body.barcode||"").trim()))return res.status(400).json({error:"Βάλε έγκυρο barcode 6–18 ψηφίων ή επίλεξε εσωτερικό/χωρίς barcode."});
    if(body.barcodeMode==="GENERATED"&&barcode&&(!/^29\d{11}$/.test(barcode)||ean13(barcode.slice(0,12))!==barcode))return res.status(400).json({error:"Το εσωτερικό barcode δεν είναι έγκυρο. Πάτησε ξανά Generate Barcode."});
    ...
    const supplierCode=body.supplierCode!==undefined?body.supplierCode:line.supplierCode,unitCost=Math.max(0,Number(body.unitCost??line.unitCost??0)),packSize=body.invoiceUnit==="PACKAGE"?Math.max(1,Number(body.unitsPerPackage||1)):1,quantity=Math.max(0.0001,Number(body.quantity??line.quantity??1)),factor=(1-body.discount1/100)*(1-body.discount2/100)*(1-body.discount3/100),excise=Math.max(0,Number(body.exciseTotal||0)),net=quantity*unitCost*factor,pieceCost=(net+excise)/(quantity*packSize),vat=(net+excise)*body.vatRate/100;
    const productId=id();let savedBarcode="";await prisma.$transaction(async tx=>{savedBarcode=body.barcodeMode==="GENERATED"?(barcode||await generateInternalBarcode(tx,req.user.companyId)):body.barcodeMode==="PROVIDED"?barcode:"";const nextSkuRows=await tx.$queryRaw`SELECT COALESCE(MAX(CASE WHEN "sku" ~ '^[0-9]+$' THEN "sku"::bigint END),10000)+1 AS "next" FROM "Product" WHERE "companyId"=${req.user.companyId}`;const sku=String(nextSkuRows[0]?.next||10001);await tx.$executeRaw`INSERT INTO "Product" ("id","companyId","categoryId","subcategoryId","sku","name","unit","vatRate","vatVerified","salePrice","costPrice","trackStock","active") VALUES (${productId},${req.user.companyId},${body.categoryId||null},${body.subcategoryId||null},${sku},${body.name},'PIECE',${body.vatRate},true,${body.salePrice},${pieceCost},true,true)`;if(savedBarcode)await tx.$executeRaw`INSERT INTO "ProductBarcode" ("id","productId","barcode","unitMultiplier") VALUES (${id()},${productId},${savedBarcode},1)`;await tx.$executeRaw`INSERT INTO "StoreProduct" ("id","storeId","productId","salePrice","currentStock","active") VALUES (${id()},${line.storeId},${productId},${body.salePrice},0,true) ON CONFLICT ("storeId","productId") DO NOTHING`;await tx.$executeRaw`UPDATE "PurchaseOrderLine" SET "productId"=${productId},"supplierCode"=${supplierCode||null},"description"=${body.name},"quantity"=${quantity},"gift"=${body.gift??line.gift??false},"unitCost"=${unitCost},"invoiceUnit"=${body.invoiceUnit},"stockUnitsPerInvoiceUnit"=${packSize},"discount1"=${body.discount1},"discount2"=${body.discount2},"discount3"=${body.discount3},"exciseTotal"=${excise},"vatRate"=${body.vatRate},"markupPercent"=${body.markupPercent},"netAmount"=${net},"vatAmount"=${vat},"grossAmount"=${net+excise+vat},"proposedSalePrice"=${body.salePrice},"resolutionStatus"='MATCHED',"updatedAt"=NOW() WHERE "id"=${line.id}`;await learnSupplierMapping(tx,{companyId:req.user.companyId,supplierId:line.supplierId,supplierCode,productId,barcode:savedBarcode,description:body.name,userId:req.user.id,unitCost:pieceCost,unitsPerPackage:packSize,discount1:body.discount1,discount2:body.discount2,discount3:body.discount3,excisePerInvoiceUnit:excise/quantity,markupPercent:body.markupPercent});await learnCentralInvoiceCorrection(tx,{actor:req.user,companyId:req.user.companyId,supplierId:line.supplierId,line:{...line,supplierCode,description:body.name,quantity,gift:body.gift??line.gift??false,unitCost,invoiceUnit:body.invoiceUnit,stockUnitsPerInvoiceUnit:packSize,discount1:body.discount1,discount2:body.discount2,discount3:body.discount3,exciseTotal:excise,netAmount:net,proposedSalePrice:body.salePrice,barcode:savedBarcode}}) });
    res.status(201).json({ok:true,productId,barcode:savedBarcode,barcodeMode:body.barcodeMode,mappingLearned:Boolean(line.supplierId&&String(supplierCode||"").trim())});
  }catch(error){next(error)}
});
