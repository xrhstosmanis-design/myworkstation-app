...
router.get("/:orderId/ocr-lines",async(req,res,next)=>{
  try{
    ...
    const rows=await prisma.$queryRaw`
      SELECT l."id",l."productId",l."supplierCode",l."description",l."quantity",l."unitCost",l."discount1",l."discount2",l."discount3",l."exciseTotal",l."vatRate",l."netAmount",l."vatAmount",l."grossAmount",l."invoiceUnit",l."stockUnitsPerInvoiceUnit",l."ocrRawText",l."ocrConfidence",l."resolutionStatus",l."detectedBarcode",l."ocrSequence",l."ocrLineIndex",p."name" AS "productName",p."sku",p."salePrice",p."costPrice",COALESCE((SELECT json_agg(pb."barcode" ORDER BY pb."barcode") FROM "ProductBarcode" pb WHERE pb."productId"=p."id"),'[]') AS "barcodes" FROM "PurchaseOrderLine" l LEFT JOIN "Product" p ON p."id"=l."productId" AND p."companyId"=${req.user.companyId} WHERE l."orderId"=${order.id} ORDER BY COALESCE(l."ocrSequence",l."ocrLineIndex",2147483647),l."createdAt",l."id"`;
