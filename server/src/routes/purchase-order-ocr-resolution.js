...
router.post("/:orderId/ocr-lines/:lineId/create-product",async(req,res,next)=>{
  try{
    const body=z.object({name:z.string().trim().min(2).max(250),barcode:z.string().trim().max(80).optional().nullable(),vatRate:z.coerce.number().min(0).max(100).default(24),salePrice:z.coerce.number().min(0).default(0),categoryId:z.string().optional().nullable(),subcategoryId:z.string().optional().nullable()}).parse(req.body||{});
    ...
    const productId=id();await prisma.$transaction(async tx=>{const nextSkuRows=await tx.$queryRaw`SELECT COALESCE(MAX(CASE WHEN "sku" ~ '^[0-9]+$' THEN "sku"::bigint END),10000)+1 AS "next" FROM "Product" WHERE "companyId"=${req.user.companyId}`;const sku=String(nextSkuRows[0]?.next||10001);await tx.$executeRaw`INSERT INTO "Product" ("id","companyId","categoryId","subcategoryId","sku","name","unit","vatRate","vatVerified","salePrice","costPrice","trackStock","active") VALUES (${productId},${req.user.companyId},${body.categoryId||null},${body.subcategoryId||null},${sku},${body.name},'PIECE',${body.vatRate},true,${body.salePrice},${Number(line.unitCost||0)},true,true)`;if(barcode)await tx.$executeRaw`INSERT INTO "ProductBarcode" ("id","productId","barcode","unitMultiplier") VALUES (${id()},${productId},${barcode},1)`;await tx.$executeRaw`INSERT INTO "StoreProduct" ("id","storeId","productId","salePrice","currentStock","active") VALUES (${id()},${line.storeId},${productId},${body.salePrice},0,true) ON CONFLICT ("storeId","productId") DO NOTHING`;const quantity=Math.max(0.0001,Number(line.quantity||1)),unitCost=Math.max(0,Number(line.unitCost||0)),net=quantity*unitCost,vat=net*body.vatRate/100;await tx.$executeRaw`UPDATE "PurchaseOrderLine" SET "productId"=${productId},"description"=${body.name},"vatRate"=${body.vatRate},"netAmount"=${net},"vatAmount"=${vat},"grossAmount"=${net+vat},"proposedSalePrice"=${body.salePrice},"resolutionStatus"='MATCHED',"updatedAt"=NOW() WHERE "id"=${line.id}`;await learnSupplierMapping(tx,{companyId:req.user.companyId,supplierId:line.supplierId,supplierCode:line.supplierCode,productId,barcode,description:body.name,userId:req.user.id,unitCost:line.unitCost})});
