  ...
  try{
    ...
    const result=rows.map(row=>{
      ...
      const productSummary=lines.length
        ?lines.map(line=>`${Math.abs(Number(line.quantity||0))}× ${line.description}`).join(" · ")
        :"Χωρίς προϊόντα";
      return {
        ...row,
        total:money(row.total),subtotal:money(row.subtotal),discount:money(row.discount),payments,lines,
        paymentSummary,
        paymentMethod:`${productSummary} · ${paymentSummary}`,
        productSummary,
      };
    });
    res.json({store,rows:result});
