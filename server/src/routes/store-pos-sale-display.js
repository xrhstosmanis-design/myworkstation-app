...
function normalizeSale(row){
  ...
  return {...row,total:money(row.total),subtotal:money(row.subtotal),discount:money(row.discount),payments,lines,paymentSummary,paymentMethod:`${productSummary} · ${paymentSummary}`,productSummary};
}
  ...
  try{
    ...
    const rows=await prisma.$queryRaw`
      SELECT s."id",s."receiptNumber",s."total",s."subtotal",s."discount",s."occurredAt",s."createdAt",
             s."transactionMode",s."delayedReason",s."reversalState",s."fiscalStatus",s."source",c."name" AS "customerName",
      ...
      WHERE s."companyId"=${req.user.companyId} AND s."storeId"=${store.id}
        AND s."source" IN ('POS','EXCHANGE') AND s."status"='COMPLETED'
  ...
  try{
    ...
    const rows=await prisma.$queryRaw`
      SELECT s."id",s."receiptNumber",s."total",s."subtotal",s."discount",s."occurredAt",s."createdAt",s."status",
             s."transactionMode",s."delayedReason",s."reversalState",s."fiscalStatus",s."source",st."name" AS "storeName",c."name" AS "customerName",
