...
async function syncOperatorWorkforceAttendance(tx,req,storeId,eventType,cashSessionId,options={}){
  ...
  if(resolvedEventType==="OUT"&&now-new Date(open.startedAt)<60000){const error=new Error("Η προσέλευση μόλις καταχωρίστηκε. Περίμενε ένα λεπτό πριν από νέα καταχώρηση.");error.status=409;throw error}
  const method=options.method||"POS_SHIFT",operatorId=req.user.operatorId||req.user.id||null;
  if(resolvedEventType==="IN"){
    ...
    const note=method==="POS_CARD"?"Σάρωση κάρτας εργασίας από το POS":method==="POS_PIN"?"Προσωπικό PIN εργασίας από το POS":`Αυτόματη έναρξη από άνοιγμα POS ${cashSessionId}`;
    ...
    await tx.workforceAuditLog.create({data:{companyId:req.user.companyId,storeId,actorUserId:null,action:"WORKFORCE_CLOCK_IN",entityType:"WORKFORCE_ATTENDANCE_SESSION",entityId:session.id,afterJson:{employeeId:employee.id,date,eventType:"IN",cashSessionId:cashSessionId||null,method,scannedByOperatorId:method==="POS_CARD"?operatorId:null,verifiedByOperatorId:method==="POS_PIN"?operatorId:null},reason:method==="POS_CARD"?"Προσέλευση με σάρωση προσωπικής κάρτας στο POS":method==="POS_PIN"?"Προσέλευση με προσωπικό PIN στο POS":"Αυτόματη έναρξη παρουσίας με το άνοιγμα της βάρδιας POS",deviceName:req.user.terminalPos||null,userAgent:String(req.headers?.["user-agent"]||"").slice(0,500)||null}});
  ...
  const note=method==="POS_CARD"?"Σάρωση κάρτας εργασίας κατά την αποχώρηση από το POS":method==="POS_PIN"?"Προσωπικό PIN εργασίας κατά την αποχώρηση από το POS":`Αυτόματη λήξη από κλείσιμο POS ${cashSessionId}`;
  ...
  await tx.workforceAuditLog.create({data:{companyId:req.user.companyId,storeId,actorUserId:null,action:"WORKFORCE_CLOCK_OUT",entityType:"WORKFORCE_ATTENDANCE_SESSION",entityId:session.id,beforeJson:{status:"OPEN"},afterJson:{employeeId:employee.id,date,eventType:"OUT",cashSessionId:cashSessionId||null,method,scannedByOperatorId:method==="POS_CARD"?operatorId:null,verifiedByOperatorId:method==="POS_PIN"?operatorId:null,workedMinutes:worked,status},reason:method==="POS_CARD"?"Αποχώρηση με σάρωση προσωπικής κάρτας στο POS":method==="POS_PIN"?"Αποχώρηση με προσωπικό PIN στο POS":"Αυτόματη λήξη παρουσίας με το κλείσιμο της βάρδιας POS",deviceName:req.user.terminalPos||null,userAgent:String(req.headers?.["user-agent"]||"").slice(0,500)||null}});
...
router.post("/stores/:storeId/attendance-pin/submit",route(async(req,res)=>{
  assertStoreAccess(req,req.params.storeId);
  const store=await ownedStore(req.params.storeId,req.user.companyId),body=attendancePinSchema.parse(req.body||{}),operatorId=req.user.operatorId||req.user.id,subjectKey=`ATTENDANCE_PIN:${body.employeeId}:${operatorId}`;
  const guard=(await prisma.$queryRaw`SELECT "failedCount","windowStartedAt","lockedUntil" FROM "StoreOperatorLoginGuard" WHERE "storeId"=${store.id} AND "subjectKey"=${subjectKey} LIMIT 1`)[0];
  if(guard?.lockedUntil&&new Date(guard.lockedUntil)>new Date())return res.status(429).json({error:"Πολλές λανθασμένες προσπάθειες. Δοκίμασε ξανά σε 15 λεπτά."});
  const credential=(await prisma.$queryRaw`
    ...
    LIMIT 1
  `)[0];
  const valid=Boolean(credential?.pinHash&&await bcrypt.compare(body.pin,credential.pinHash));
  if(!valid){
    const resetWindow=!guard?.windowStartedAt||Date.now()-new Date(guard.windowStartedAt).getTime()>15*60*1000,nextCount=resetWindow?1:Number(guard.failedCount||0)+1,lockedUntil=nextCount>=5?new Date(Date.now()+15*60*1000):null;
    await prisma.$executeRaw`INSERT INTO "StoreOperatorLoginGuard" ("id","storeId","subjectKey","failedCount","windowStartedAt","lockedUntil","updatedAt") VALUES (${crypto.randomUUID()},${store.id},${subjectKey},${nextCount},NOW(),${lockedUntil},NOW()) ON CONFLICT ("storeId","subjectKey") DO UPDATE SET "failedCount"=${nextCount},"windowStartedAt"=CASE WHEN ${resetWindow} THEN NOW() ELSE "StoreOperatorLoginGuard"."windowStartedAt" END,"lockedUntil"=${lockedUntil},"updatedAt"=NOW()`;
    return res.status(401).json({error:"Ο εργαζόμενος ή το προσωπικό PIN δεν είναι σωστό."});
  }
  await prisma.$executeRaw`DELETE FROM "StoreOperatorLoginGuard" WHERE "storeId"=${store.id} AND "subjectKey"=${subjectKey}`;
...
router.post("/sessions/:sessionId/close",route(async(req,res)=>{
  ...
  const closeResult=await prisma.$transaction(async tx=>{
    ...
    if(variance < -0.009){
      ...
      await tx.$executeRaw`INSERT INTO "StoreOperatorAudit" ("id","companyId","storeId","operatorId","actorId","eventType","details","createdAt") VALUES (${crypto.randomUUID()},${req.user.companyId},${session.storeId},${req.user.operatorId||req.user.id},${req.user.id},${eventType},${details}::jsonb,${attemptedAt})`;
      if(!body.forceCloseWithShortage)return {recountRequired:true,sessionId:session.id,storeId:session.storeId,expectedOperational:expected,actualOperational:actual,variance,shortage};
    }
