...
async function ensureEfoodLabIntegrationScope(integration){
  const scope=(await prisma.$queryRaw`SELECT c."name" AS "companyName",s."name" AS "storeName" FROM "Store" s JOIN "Company" c ON c."id"=s."companyId" WHERE s."id"=${integration.storeId} AND s."companyId"=${integration.companyId} LIMIT 1`)[0];
...
export async function recordEfoodWebhookEvent({integration,payload,mode="LOCAL_MOCK",authenticated=true}){
  await schemas();
  await ensureEfoodLabIntegrationScope(integration);
...
router.post("/pelican/:webhookKey",async(req,res,next)=>{try{
  await schemas();
  const integration=(await prisma.$queryRaw`SELECT i."id",i."companyId",i."storeId",i."kind",i."environment",i."credentialsEnc",i."metadataJson",i."enabled",i."externalCallsEnabled",i."sandboxValidatedAt",s."name" AS "storeName",c."name" AS "companyName" FROM "StoreIntegrationCredential" i JOIN "Store" s ON s."id"=i."storeId" AND s."companyId"=i."companyId" JOIN "Company" c ON c."id"=i."companyId" WHERE i."kind"='EFOOD' AND i."webhookKey"=${req.params.webhookKey} LIMIT 1`)[0];
  if(!integration)return res.status(404).json({error:"Δεν βρέθηκε ενεργή διασύνδεση efood."});
  assertEfoodLabContext({companyName:integration.companyName,storeName:integration.storeName});
  ...
  if(integration.environment!=="SANDBOX"||integration.enabled!==true||integration.externalCallsEnabled!==true||!integration.sandboxValidatedAt){
    return res.status(503).json({error:"Η διασύνδεση efood είναι προετοιμασμένη αποκλειστικά στο LAB αλλά δεν έχει ενεργοποιηθεί μετά από test vendor και LAB PASS.",code:"EFOOD_FAIL_CLOSED"});
  }
  const result=await recordEfoodWebhookEvent({integration,payload:req.body,mode:"LIVE_WEBHOOK",authenticated:true});
  res.status(result.idempotent?200:202).json({ok:true,idempotent:result.idempotent,eventId:result.id,status:result.parsed.status,processingStatus:result.processingStatus,labOnly:true,orderCreated:false,saleCreated:false,stockChanged:false,paymentPosted:false,fiscalExecution:false});
