...
function candidateOcrRows(resultJson){
  ...
  for(const entry of source){
    ...
    // Product invoice lines normally contain a description and at least one numeric field.
    ...
    rows.push({text,description,barcode,unitCost,quantity,confidence:Number(entry?.confidence||0)});
  }
...
router.post("/ai-reader/jobs/:jobId/pos-intake",requireCompanyModule("AI_READER"),requireCompanyModule("INVENTORY"),async(req,res,next)=>{
  try{
    ...
    const jobs=await prisma.$queryRaw`
      SELECT "id","storeId","status","purchaseDocumentId","resultJson"
      FROM "AiReaderJob"
    ...
    const result=await prisma.$transaction(async tx=>{
      ...
      const documentId=id(),orderId=id(),actor=req.user.fullName||"Χειριστής";
      const createdByUserId=req.user?.tokenType==="STORE_OPERATOR"?null:req.user.id;
      await tx.$executeRaw`
        INSERT INTO "PurchaseDocument" (
          "id","companyId","storeId","supplierId","documentType","documentNumber","documentDate",
          "totalNet","totalVat","totalGross","sourceType","status","createdByUserId","settlementMode","purchaseOrderId"
        ) VALUES (
      ...
      await tx.$executeRaw`
        INSERT INTO "PurchaseOrder" ("id","companyId","storeId","supplierId","status","invoiceNumber","description","createdByUserId","createdByName","updatedByName","sourceType","sourceDocumentId")
        VALUES (${orderId},${req.user.companyId},${job.storeId},${body.supplierId},'NEW',${body.documentNumber},${body.note||`OCR τιμολόγιο ${body.documentNumber} — έλεγχος πριν την οριστικοποίηση`},${createdByUserId},${actor},${actor},'POS_OCR_DRAFT',${documentId})`;

      const ocrRows=await matchProducts(tx,req.user.companyId,candidateOcrRows(job.resultJson||{}));
      for(const row of ocrRows){
        const product=row.product;
        const vatRate=product?Number(product.vatRate||0):0;
        const quantity=Math.max(0.0001,Number(row.quantity||1));
        ...
        await tx.$executeRaw`
          INSERT INTO "PurchaseOrderLine" (
            "id","orderId","productId","description","quantity","unitCost","discount1","discount2","discount3","exciseTotal","vatRate","gift","initialUnitCost","markupPercent","proposedSalePrice","netAmount","vatAmount","grossAmount","ocrRawText","ocrConfidence","resolutionStatus","detectedBarcode"
          ) VALUES (
            ${id()},${orderId},${product?.id||null},${row.description},${quantity},${unitCost},0,0,0,0,${vatRate},false,${unitCost},0,${Number(product?.salePrice||0)},${netAmount},${vatAmount},${grossAmount},${row.text},${row.confidence},${product?'MATCHED':'UNRESOLVED'},${row.barcode||null}
          )`;
      }
      ...
      if(body.settlementMode==="PAID"){
        ...
        await tx.$executeRaw`
          INSERT INTO "StoreTransaction" (
            ...
            true,${req.user.id},${actor},NULL,'application/vnd.myworkstation.purchase-document',${documentId},
    ...
    res.status(201).json({
      ...
      unresolvedLines:result.unresolved,
      message:body.settlementMode==="PAID"
        ?`Το τιμολόγιο καταχωρίστηκε ως πληρωμένο με μετρητά, αφαιρέθηκε από την ενεργή βάρδια και δημιουργήθηκε Νέα αγορά με ${result.lineCount} γραμμές (${result.unresolved} για επίλυση). Η αποθήκη δεν ενημερώθηκε ακόμη.`
        :`Το τιμολόγιο καταχωρίστηκε με πίστωση και δημιουργήθηκε Νέα αγορά με ${result.lineCount} γραμμές (${result.unresolved} για επίλυση). Δεν αφαιρέθηκε ποσό από τη βάρδια και η αποθήκη δεν ενημερώθηκε ακόμη.`
    });
