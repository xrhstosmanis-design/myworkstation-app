...
async function onlineStoreContext(companyId,storeId){
  await ensureKatOnlineOrderingSchema();
...
const onlineSettingsSchema=z.object({
  ...
  doctorDiscountPercent:z.coerce.number().min(0).max(100),nurseDiscountPercent:z.coerce.number().min(0).max(100),
...
router.get("/companies/:companyId/stores/:storeId/online-store",async(req,res,next)=>{
  try{
    ...
    const configs=await prisma.$queryRaw`SELECT "enabled","publicSlug","surchargeType","surchargeValue","stockCheckEnabled","pickupEnabled","deliveryEnabled","deliveryFee","minimumOrderRetail","doctorDiscountPercent","nurseDiscountPercent","cashEnabled","cardOnDeliveryEnabled","timezone","weeklyHours","brandName","brandTagline","brandLogoUrl","brandPrimaryColor","brandSecondaryColor","brandWelcomeMessage","estimatedMinutes" FROM "OnlineOrderingConfig" WHERE "companyId"=${req.params.companyId} AND "storeId"=${store.id} LIMIT 1`;
    const config=configs[0]||{enabled:false,publicSlug:"",surchargeType:"FIXED",surchargeValue:0,stockCheckEnabled:false,pickupEnabled:true,deliveryEnabled:true,deliveryFee:0,minimumOrderRetail:0,doctorDiscountPercent:0,nurseDiscountPercent:0,cashEnabled:true,cardOnDeliveryEnabled:true,timezone:"Europe/Athens",weeklyHours:{},brandName:store.name,brandTagline:"Online Παραγγελίες",brandLogoUrl:"",brandPrimaryColor:"#7b1216",brandSecondaryColor:"#5d0c0f",brandWelcomeMessage:"Γρήγορα, εύκολα, όποτε θέλεις!",estimatedMinutes:25};
    ...
    res.json({store:{id:store.id,name:store.name},settings:{...config,surchargeValue:Number(config.surchargeValue||0),deliveryFee:Number(config.deliveryFee||0),minimumOrderRetail:Number(config.minimumOrderRetail||0),doctorDiscountPercent:Number(config.doctorDiscountPercent||0),nurseDiscountPercent:Number(config.nurseDiscountPercent||0)},products:products.map(row=>({...row,storePrice:Number(row.storePrice||0),onlinePrice:onlineUnitPrice(row.storePrice,config)}))});
  }catch(error){next(error)}
});
...
const onlineMemberSchema=z.object({fullName:z.string().trim().min(3).max(120),phone:z.string().trim().min(6).max(40),memberType:z.enum(["DOCTOR","NURSE"]),pin:z.string().regex(/^\d{6}$/,{message:"Ο προσωπικός PIN πρέπει να έχει 6 ψηφία."})});
const normalizePhone=value=>String(value||"").replace(/\D/g,"");
router.get("/companies/:companyId/stores/:storeId/online-store/members",async(req,res,next)=>{
  try{const store=await onlineStoreContext(req.params.companyId,req.params.storeId);await ensureKatOnlineOrderingSchema();const rows=await prisma.$queryRaw`SELECT "id","fullName","phoneNormalized","memberType","active","createdAt","updatedAt" FROM "OnlineStoreMember" WHERE "companyId"=${req.params.companyId} AND "storeId"=${store.id} ORDER BY "memberType","fullName"`;res.json({members:rows})}catch(error){next(error)}
});
router.post("/companies/:companyId/stores/:storeId/online-store/members",async(req,res,next)=>{
  try{const store=await onlineStoreContext(req.params.companyId,req.params.storeId),body=onlineMemberSchema.parse(req.body||{}),phoneNormalized=normalizePhone(body.phone);if(phoneNormalized.length<8)return res.status(400).json({error:"Βάλε έγκυρο κινητό ή τηλέφωνο εργασίας."});await ensureKatOnlineOrderingSchema();const id=crypto.randomUUID(),pinHash=await bcrypt.hash(body.pin,12);await prisma.$executeRaw`INSERT INTO "OnlineStoreMember" ("id","companyId","storeId","fullName","phoneNormalized","pinHash","memberType","active") VALUES (${id},${req.params.companyId},${store.id},${body.fullName},${phoneNormalized},${pinHash},${body.memberType},TRUE) ON CONFLICT ("storeId","phoneNormalized") DO UPDATE SET "fullName"=EXCLUDED."fullName","pinHash"=EXCLUDED."pinHash","memberType"=EXCLUDED."memberType","active"=TRUE,"updatedAt"=CURRENT_TIMESTAMP`;await prisma.authAudit.create({data:{userId:req.user.id,email:req.user.email||"super-admin",event:"ONLINE_STORE_MEMBER_UPSERT",success:true,deviceName:`${store.name} · ${body.memberType}`,userAgent:req.headers["user-agent"]||null,ipAddress:req.ip||null}});res.status(201).json({ok:true})}catch(error){next(error)}
});
...
router.get("/companies/:companyId/stores/:storeId/video-connection",async(req,res,next)=>{
  try{const store=await videoStoreContext(req.params.companyId,req.params.storeId),rows=await prisma.$queryRaw`SELECT "id","provider","protocol","endpoint","username","active","timeOffsetSeconds","retentionDays","privacyMode","audioEnabled",("privacyNoticeAcknowledgedAt" IS NOT NULL) AS "privacyNoticeAcknowledged","connectionStatus","lastTestedAt","timeSyncStatus","timeCheckSource","lastSystemTime","lastNvrTime","measuredOffsetSeconds","timeDeviationSeconds","lastTimeCheckedAt",("passwordEnc" IS NOT NULL) AS "passwordConfigured" FROM "StoreVideoConnection" WHERE "companyId"=${req.params.companyId} AND "storeId"=${store.id} LIMIT 1`,cameras=await prisma.$queryRaw`SELECT "cameraKey","displayName","zone","streamReference","active","sortOrder" FROM "StoreVideoCamera" WHERE "companyId"=${req.params.companyId} AND "storeId"=${store.id} AND "active"=true ORDER BY "sortOrder","displayName"`,connector=await videoConnectorStatus(req.params.companyId,store.id);res.json({store:{id:store.id,name:store.name},connection:rows[0]||{provider:"GENERIC",protocol:"ONVIF",endpoint:"",username:"",active:false,timeOffsetSeconds:0,retentionDays:30,privacyMode:"EVENT_ONLY",audioEnabled:false,privacyNoticeAcknowledged:false,connectionStatus:"NOT_TESTED",lastTestedAt:null,timeSyncStatus:"NOT_CHECKED",lastTimeCheckedAt:null,passwordConfigured:false},cameras,connector,configurationOnly:!connector?.online})}catch(error){next(error)}
});

router.post("/companies/:companyId/stores/:storeId/video-connection/test",async(req,res,next)=>{
  try{const store=await videoStoreContext(req.params.companyId,req.params.storeId),connector=await videoConnectorStatus(req.params.companyId,store.id);if(!connector?.online)return res.status(409).json({error:"Ο τοπικός Video Connector δεν είναι online."});const command=await enqueueVideoCommand({companyId:req.params.companyId,storeId:store.id,commandType:"HEALTH",payload:{requestedBy:req.user.id}});res.status(202).json({ok:true,command})}catch(error){next(error)}
});

router.post("/companies/:companyId/stores/:storeId/video-connection/snapshot",async(req,res,next)=>{
  try{const store=await videoStoreContext(req.params.companyId,req.params.storeId),body=z.object({cameraKey:z.string().trim().min(1).max(80)}).parse(req.body||{}),camera=(await prisma.$queryRaw`SELECT "cameraKey","streamReference" FROM "StoreVideoCamera" WHERE "companyId"=${req.params.companyId} AND "storeId"=${store.id} AND "cameraKey"=${body.cameraKey} AND "active"=true LIMIT 1`)[0];if(!camera)return res.status(404).json({error:"Δεν βρέθηκε η ενεργή κάμερα."});const connector=await videoConnectorStatus(req.params.companyId,store.id);if(!connector?.online)return res.status(409).json({error:"Ο τοπικός Video Connector δεν είναι online."});const command=await enqueueVideoCommand({companyId:req.params.companyId,storeId:store.id,commandType:"SNAPSHOT",cameraKey:camera.cameraKey,payload:{streamReference:camera.streamReference||null},ttlSeconds:120});res.status(202).json({ok:true,command})}catch(error){next(error)}
});

router.get("/companies/:companyId/stores/:storeId/video-connection/commands/:commandId",async(req,res,next)=>{
  try{const store=await videoStoreContext(req.params.companyId,req.params.storeId),command=await videoCommandStatus({companyId:req.params.companyId,storeId:store.id,commandId:req.params.commandId});if(!command)return res.status(404).json({error:"Δεν βρέθηκε η εντολή Video Connector."});res.json({...command,mediaUrl:command.artifactStatus==="READY"?`/api/platform/companies/${encodeURIComponent(req.params.companyId)}/stores/${encodeURIComponent(store.id)}/video-media/${encodeURIComponent(command.artifactId)}`:null})}catch(error){next(error)}
});

router.get("/companies/:companyId/stores/:storeId/video-media/:artifactId",async(req,res,next)=>{
  try{const store=await videoStoreContext(req.params.companyId,req.params.storeId),artifact=await readyVideoArtifact({companyId:req.params.companyId,storeId:store.id,artifactId:req.params.artifactId});if(!artifact)return res.status(404).json({error:"Το προσωρινό αρχείο video δεν βρέθηκε ή έληξε."});res.setHeader("Content-Type",artifact.mimeType);res.setHeader("Content-Length",String(artifact.bytes.length));res.setHeader("Content-Disposition",`inline; filename="${String(artifact.filename).replace(/[\r\n\"]/g,"_")}"`);res.setHeader("Cache-Control","private, no-store");res.send(artifact.bytes)}catch(error){next(error)}
});
