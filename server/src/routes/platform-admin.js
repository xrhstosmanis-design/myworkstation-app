...
router.post("/super-admin-analytics/execute",async(req,res,next)=>{try{const body=z.object({companyId:z.string().optional(),storeId:z.string().optional(),from:z.string().optional(),to:z.string().optional(),pack:z.string().optional(),category:z.string().optional()}).parse(req.body||{});const rows=await prisma.$queryRaw\`SELECT s."storeId",s."companyId",COUNT(*)::int AS "shifts",COALESCE(SUM(s."variance"),0)::float AS "cashVariance",COALESCE(SUM(s."cardVariance"),0)::float AS "cardVariance" FROM "CashShiftSession" s WHERE (\${body.companyId||null}::text IS NULL OR s."companyId"=\${body.companyId||null}) AND (\${body.storeId||null}::text IS NULL OR s."storeId"=\${body.storeId||null}) GROUP BY s."storeId",s."companyId"\`;const findings=rows.filter(r=>Math.abs(Number(r.cashVariance||0))>.009||Math.abs(Number(r.cardVariance||0))>.02).map(r=>({storeId:r.storeId,status:"Χρειάζεται έλεγχος",cashVariance:Number(r.cashVariance||0),cardVariance:Number(r.cardVariance||0)}));res.json({ok:true,pack:body.pack||"BASIC ΕΛΕΓΧΟΣ",category:body.category||"ALL",rows,findings,status:findings.length?"Χρειάζεται έλεγχος":"ΟΚ",automaticEmployeeAccusation:false})}catch(error){next(error)}});
