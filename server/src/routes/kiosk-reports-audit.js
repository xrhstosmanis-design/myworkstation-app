...
router.get("/audit-events",requireManagement,async(req,res,next)=>{
  try{
    ...
    const actionRows=await prisma.$queryRaw`
      ...
      LEFT JOIN "Store" s ON s."id"=a."storeId" AND s."companyId"=a."companyId"
      WHERE (${companyId}::text IS NULL OR a."companyId"=${companyId}) AND a."actionType" IN ('RETURN','CANCEL','RETURN_ITEMS','SELF_CONSUMPTION','PRODUCT_DESTRUCTION','CART_ITEM_ADD','CART_QTY_CHANGE','CART_ITEM_REMOVE','CART_CANCEL','PRICE_CHANGE','AUDIENCE_DISCOUNT_SELECTED','ITEM_CHANGE_REQUEST','ITEM_EXCHANGE_COMPLETED','HOLD_RESTORE','HOLD_SAVE','WASTE')
        AND a."createdAt">=${from} AND a."createdAt"<${to}
        AND (${storeId}::text IS NULL OR a."storeId"=${storeId})
        AND (${text}::text IS NULL
    ...
    const operatorRows=await prisma.$queryRaw`
      ...
      ORDER BY a."createdAt" DESC LIMIT 10000`;
    const transactionItems=transactionRows.map(r=>({...r,amount:n(r.amount),financialDetails:{amount:n(r.amount),previousTotal:null,newTotal:null,difference:null},sourceType:"StoreTransaction",paymentSource:r.subtractFromShift?"CASH_SHIFT":"EXTERNAL"}));
    const actionItems=actionRows.map(r=>{
      const details=r.details&&typeof r.details==="object"?r.details:{};
      const isReturn=r.actionType==="RETURN",isPartialReturn=r.actionType==="RETURN_ITEMS",isSelfConsumption=r.actionType==="SELF_CONSUMPTION",isDestruction=r.actionType==="PRODUCT_DESTRUCTION",isCartAdd=r.actionType==="CART_ITEM_ADD",isQtyChange=r.actionType==="CART_QTY_CHANGE",isCartRemoval=r.actionType==="CART_ITEM_REMOVE",isCartCancel=r.actionType==="CART_CANCEL",isPriceChange=r.actionType==="PRICE_CHANGE";
      ...
      const eventType=isPriceChange?"PRICE_CHANGE":isCartAdd?"CART_ITEM_ADD":isQtyChange?"CART_QTY_CHANGE":isCartRemoval?"CART_ITEM_REMOVE":isCartCancel?"CART_CANCEL":isPartialReturn?"POS_RETURN_ITEMS":isSelfConsumption?"POS_SELF_CONSUMPTION":isDestruction?"POS_PRODUCT_DESTRUCTION":isReturn||r.actionType==="CANCEL"?baseAction.eventType:r.actionType;
        ...
        :isReturn
          ...
          :isSelfConsumption
            ...
            :isDestruction
              ...
              :isCartAdd||isQtyChange
                ?`${greekAuditEventLabel(r.actionType)} · ${details.productName||"Άγνωστο προϊόν"} · ποσότητα ${n(details.previousQuantity??details.oldQuantity)} → ${n(details.newQuantity)} · τιμή ${n(details.unitPrice).toFixed(2)} € → ${n(details.effectiveUnitPrice??details.unitPrice).toFixed(2)} € · αξία ${n(details.previousTotal).toFixed(2)} € → ${n(details.newTotal).toFixed(2)} €`
      ...
      return {
        ...
        reversedAt:null,reversedByName:null,reversalReason:r.reason||null,financialDetails:auditFinancialDetails(details),storeName:r.storeName,storeId:r.storeId,terminalPos:r.terminalPos,sourceType:"PosSaleActionAudit",paymentSource:"AUDIT_EVENT"
      };
    });
    const operatorItems=operatorRows.map(r=>{
      ...
      return {id:r.id,createdAt:r.createdAt,eventType:r.eventType,amount:eventAmount,description,supplierId:details.supplierId||null,supplierName:details.supplierName||null,shiftId:details.sessionId||null,actorId:r.actorId,actorName:details.actorName||r.actorName||r.actorId,subtractFromShift:false,reversedAt:null,reversedByName:null,reversalReason:null,storeName:r.storeName,storeId:r.storeId,terminalPos:details.terminalPos||"BACKOFFICE",financialDetails:auditFinancialDetails(details),sourceType:"StoreOperatorAudit",paymentSource:"AUDIT_EVENT"};
    });
