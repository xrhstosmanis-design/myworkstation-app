...
async function ownedStore(companyId,storeId){return prisma.store.findFirst({where:{id:storeId,companyId,active:true},select:{id:true,name:true}})}
...
router.get("/stores/:storeId/fiscal-bridge/dry-runs",async(req,res,next)=>{try{
  const store=await ownedStore(req.user.companyId,req.params.storeId);if(!store)return res.status(404).json({error:"Δεν βρέθηκε το κατάστημα."});
  await ensureSchema();
  const rows=await prisma.$queryRaw`SELECT "id","saleId","terminalPos","fiscalDeviceCode","schemaVersion","idempotencyKey","payloadHash","validationJson","status","externalExecution","createdAt" FROM "FiscalBridgeDryRun" WHERE "companyId"=${req.user.companyId} AND "storeId"=${store.id} ORDER BY "createdAt" DESC LIMIT 100`;
...
router.post("/stores/:storeId/fiscal-bridge/dry-runs",async(req,res,next)=>{try{
  ...
  const [sales,lines,payments,routes]=await Promise.all([
    prisma.$queryRaw`SELECT "id","source","status","fiscalStatus","subtotal","discount","total","occurredAt","createdAt" FROM "Sale" WHERE "id"=${body.saleId} AND "companyId"=${req.user.companyId} AND "storeId"=${store.id} LIMIT 1`,
    ...
    prisma.$queryRaw`SELECT "terminalPos","channel","fiscalDeviceCode","eftposDeviceCode","role" FROM "PaymentDeviceRouteAttempt" WHERE "companyId"=${req.user.companyId} AND "storeId"=${store.id} AND "saleId"=${body.saleId} ORDER BY "createdAt" DESC LIMIT 1`
  ]);
  ...
  const inserted=await prisma.$executeRaw`INSERT INTO "FiscalBridgeDryRun" ("id","companyId","storeId","saleId","terminalPos","fiscalDeviceCode","schemaVersion","idempotencyKey","payloadHash","payloadJson","validationJson","createdByUserId") VALUES (${id},${req.user.companyId},${store.id},${sale.id},${route.terminalPos},${route.fiscalDeviceCode},${envelope.schemaVersion},${envelope.idempotencyKey},${payloadHash},${payload}::jsonb,${JSON.stringify(validation)}::jsonb,${req.user.id}) ON CONFLICT ("companyId","storeId","saleId","schemaVersion") DO NOTHING`;
  const rows=await prisma.$queryRaw`SELECT "id","saleId","terminalPos","fiscalDeviceCode","schemaVersion","idempotencyKey","payloadHash","validationJson","status","externalExecution","createdAt" FROM "FiscalBridgeDryRun" WHERE "companyId"=${req.user.companyId} AND "storeId"=${store.id} AND "saleId"=${sale.id} AND "schemaVersion"=${envelope.schemaVersion} LIMIT 1`;
