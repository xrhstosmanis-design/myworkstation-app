...
async function premiumVarianceControls(body){
  ...
  const auditedSaleIds=[...new Set([...reversalAudits,...safetyAudits].flatMap(row=>[row.saleId,row.relatedSaleId]).filter(Boolean))];
  const auditSales=auditedSaleIds.length?await prisma.$queryRaw`
    SELECT s."id" AS "saleId",s."receiptNumber",s."total"::float AS "total",s."occurredAt",s."createdAt",
      COALESCE(lines."basketLines",'') AS "basketLines",COALESCE(payments."paymentMethods",'') AS "paymentMethods"
    FROM "Sale" s
    LEFT JOIN LATERAL (SELECT string_agg(COALESCE(l."description",l."productId",'—') || ' × ' || l."quantity"::text || ' = ' || l."lineTotal"::text || ' €',' · ' ORDER BY l."createdAt",l."id") AS "basketLines" FROM "SaleLine" l WHERE l."saleId"=s."id") lines ON TRUE
    LEFT JOIN LATERAL (SELECT string_agg(p."method" || ':' || p."amount"::text,'|' ORDER BY p."method",p."amount") AS "paymentMethods" FROM "Payment" p WHERE p."saleId"=s."id") payments ON TRUE
    WHERE s."companyId"=${body.companyId} AND s."storeId"=${body.storeId} AND s."id"=ANY(${auditedSaleIds}::text[])`:[];
  const saleEvidenceById=new Map([...sales,...auditSales].map(sale=>[sale.saleId,sale]));
  const evidenceTransaction=saleId=>{const sale=saleEvidenceById.get(saleId);return sale?{saleId:sale.saleId,receiptNumber:sale.receiptNumber||null,occurredAt:sale.occurredAt||sale.createdAt,total:number(sale.total),paymentMethods:sale.paymentMethods||"—",items:sale.basketLines||"—"}:null};
  ...
  for(const [key,group] of reversalGroups)if(group.length>1){const saleId=key.split(":").slice(1).join(":");const transaction=evidenceTransaction(saleId);findings.push({id:`reversal-repeat:${key}`,code:"POTENTIAL_REPEATED_REVERSAL",title:"Πιθανές επαναλαμβανόμενες ακυρώσεις / επιστροφές",sessionId:group[0].sessionId,shiftLabel:group[0].shiftLabel,terminalPos:group[0].terminalPos,saleIds:[saleId],occurredAt:group[0].createdAt,operatorName:group[0].actorName||"—",transactionCount:group.length,transactions:transaction?[transaction]:[],missingSaleIds:transaction?[]:[saleId],possibleExplanation:"Περισσότερα από ένα audit ακύρωσης/επιστροφής για την ίδια αρχική πώληση μέσα στην ίδια βάρδια. Χρειάζεται αντιπαραβολή με τις αντίστροφες εγγραφές."})}
  for(const row of safetyAudits){const saleIds=[row.saleId,row.relatedSaleId].filter(Boolean),transactions=saleIds.map(evidenceTransaction).filter(Boolean);findings.push({id:`safety:${row.id}`,code:row.eventType,title:row.eventType==="DUPLICATE_CONFIRMED"?"Επιβεβαιωμένη διπλή καταχώριση POS":"Συμβάν ασφάλειας διπλής/επανάληψης POS",sessionId:row.sessionId,shiftLabel:row.shiftLabel,terminalPos:row.terminalPos,saleIds,occurredAt:row.createdAt,operatorName:row.actorName||"—",transactions,missingSaleIds:saleIds.filter(saleId=>!saleEvidenceById.has(saleId)),possibleExplanation:"Το POS κατέγραψε συμβάν προστασίας διπλής συναλλαγής. Είναι ένδειξη για έλεγχο, όχι αυτόματη απόδοση ευθύνης."});}
  if(operationalTables[0]?.exists){
    const operationalEvents=await prisma.$queryRaw`
      SELECT e."id",e."sessionId",e."operatorName",e."type",e."itemsJson",e."detailsJson",e."total"::float AS "total",e."createdAt",sh."shiftLabel",sh."terminalPos"
  ...
  const shiftResults=sessions.map(session=>{const evidence=byShift.get(session.sessionId)||[],cashVariance=number(session.cashVariance),cardVariance=number(session.cardVariance),handoverDelta=session.nextOpenedAt?number(session.nextOpeningOperational)-number(session.closingOperational):null;const automaticOffset=handoverDelta!==null&&Math.abs(cashVariance+handoverDelta)<=cashTolerance;const handoverMatched=handoverDelta===null||Math.abs(handoverDelta)<=cashTolerance||automaticOffset;if(!handoverMatched)evidence.push({id:`handover:${session.sessionId}`,code:"SHIFT_HANDOVER_DIFFERENCE",title:"Απόκλιση παράδοσης προς επόμενη βάρδια",handover:{closingSessionId:session.sessionId,closingShiftLabel:session.shiftLabel||null,closingTerminalPos:session.terminalPos||null,closingAt:session.closedAt||null,closingOperational:number(session.closingOperational),nextSessionId:session.nextSessionId||null,nextShiftLabel:session.nextShiftLabel||null,nextTerminalPos:session.nextTerminalPos||null,nextOpenedByName:session.nextOpenedByName||null,nextOpenedAt:session.nextOpenedAt,nextOpeningOperational:number(session.nextOpeningOperational)},possibleExplanation:automaticOffset?`Το κλείσιμο ${number(session.closingOperational).toFixed(2)} € συμψηφίστηκε αυτόματα με το επόμενο άνοιγμα ${number(session.nextOpeningOperational).toFixed(2)} €, επειδή η διαφορά εξηγείται από τη διαφορά μετρητών της προηγούμενης βάρδιας.`:`Το κλείσιμο ${number(session.closingOperational).toFixed(2)} € δεν συμφωνεί με το επόμενο άνοιγμα ${number(session.nextOpeningOperational).toFixed(2)} €. Απαιτείται έλεγχος· δεν γίνεται αυτόματος συμψηφισμός.`});return{...session,cashVariance,cardVariance,handoverDelta,automaticOffset,handoverMatched,evidence,finalStatus:evidence.length?"FINAL_WITH_TRACED_EVIDENCE":Math.abs(cashVariance)>cashTolerance||Math.abs(cardVariance)>cardTolerance?"FINAL_UNEXPLAINED_VARIANCE":"FINAL_RECONCILED",finalLabel:evidence.length?"Τελικό αποτέλεσμα με τεκμηριωμένες κινήσεις":Math.abs(cashVariance)>cashTolerance||Math.abs(cardVariance)>cardTolerance?"Τελικό ανεξήγητο υπόλοιπο":"Τελική συμφωνία",approvalRequired:true}});
