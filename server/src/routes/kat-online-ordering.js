...
async function consumePreparationRecipe(tx,{store,line,enforceStock,order,user}){
  ...
  for(const ingredient of recipe){
    ...
    const movementKey=`online:${order.id}:line:${line.orderLineId}:recipe:${stockProductId}`;
    await tx.$executeRaw`INSERT INTO "StockMovement" ("id","storeId","productId","movementType","quantity","unitCost","sourceType","sourceId","note","createdByUserId","idempotencyKey") VALUES (${crypto.randomUUID()},${store.id},${stockProductId},'RECIPE_CONSUMPTION',${-qty},${null},'ONLINE_ORDER_RECIPE',${order.id},${`ONLINE ΠΑΡΑΓΓΕΛΙΑ · ${order.orderNumber} · Κατανάλωση συνταγής ${line.productName}`},${movementUserId},${movementKey}) ON CONFLICT ("storeId","idempotencyKey") WHERE "idempotencyKey" IS NOT NULL DO NOTHING`;
  }
  ...
  if(milkSku&&milkQty>0){
    ...
    if(milk?.storeProductId){
      ...
      if(changed){const movementKey=`online:${order.id}:line:${line.orderLineId}:milk:${milk.id}`;await tx.$executeRaw`INSERT INTO "StockMovement" ("id","storeId","productId","movementType","quantity","unitCost","sourceType","sourceId","note","createdByUserId","idempotencyKey") VALUES (${crypto.randomUUID()},${store.id},${milk.id},'RECIPE_CONSUMPTION',${-milkQty},${null},'ONLINE_ORDER_RECIPE',${order.id},${`ONLINE ΠΑΡΑΓΓΕΛΙΑ · ${order.orderNumber} · Γάλα modifier ${line.productName}`},${movementUserId},${movementKey}) ON CONFLICT ("storeId","idempotencyKey") WHERE "idempotencyKey" IS NOT NULL DO NOTHING`}
    }
  }
  return true;
}
...
async function postCommercialSale(tx,{order,store,user,config}){
  ...
  for(const line of lines){
    ...
    if(line.trackStock){
      ...
      const movementUserId=user?.tokenType==="STORE_OPERATOR"?null:(user?.id||null);
      const movementKey=`online:${order.id}:line:${line.orderLineId}:sale:${line.productId}`;
      await tx.$executeRaw`INSERT INTO "StockMovement" ("id","storeId","productId","movementType","quantity","unitCost","sourceType","sourceId","note","createdByUserId","idempotencyKey") VALUES (${crypto.randomUUID()},${store.id},${line.productId},'SALE',${-qty},${null},'ONLINE_ORDER_SALE',${order.id},${`ONLINE ΠΑΡΑΓΓΕΛΙΑ · ${order.orderNumber} · ${line.productName}`},${movementUserId},${movementKey}) ON CONFLICT ("storeId","idempotencyKey") WHERE "idempotencyKey" IS NOT NULL DO NOTHING`;
    }
  }
