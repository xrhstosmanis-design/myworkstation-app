...
const schema={
  type:"object",additionalProperties:false,required:["assistantMessage","corrections","printedLines","printedTotal","expectedPageCount","visiblePageNumbers","singlePageComplete"],
  properties:{
    assistantMessage:{type:"string"},printedTotal:{type:"string"},expectedPageCount:{type:"integer"},visiblePageNumbers:{type:"array",items:{type:"integer"}},singlePageComplete:{type:"boolean"},
...
router.post("/purchase-orders/:orderId/invoice-assistant/preview",requireCompanyModule("AI_READER"),manager,async(req,res,next)=>{
  try{
    ...
    const reviewReady=assessInvoicePages({...normalizedAssistantPages({...parsed,sourcePageCount:result.pages.length}),sourcePageCount:result.pages.length,printedLines,printedTotal});
