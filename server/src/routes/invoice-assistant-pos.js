...
import {assessInvoicePages} from "./invoice-assistant-review.js";
...
const schema={
  type:"object",additionalProperties:false,required:["assistantMessage","corrections","printedLines","printedTotal","expectedPageCount","visiblePageNumbers"],
  properties:{
    assistantMessage:{type:"string"},printedTotal:{type:"string"},expectedPageCount:{type:"integer"},visiblePageNumbers:{type:"array",items:{type:"integer"}},
...
router.post("/purchase-orders/:orderId/invoice-assistant/preview",requireCompanyModule("AI_READER"),manager,async(req,res,next)=>{
  try{
    ...
    const response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,"Content-Type":"application/json"},signal:AbortSignal.timeout(120000),body:JSON.stringify({model:process.env.OPENAI_INVOICE_ASSISTANT_MODEL||process.env.OPENAI_INVOICE_MODEL||process.env.OPENAI_INVOICE_FULL_MODEL||process.env.OPENAI_INVOICE_FAST_MODEL||"gpt-5",reasoning:{effort:"medium"},input:[{role:"user",content:[{type:"input_text",text:instruction},...files]}],text:{format:{type:"json_schema",name:"invoice_assistant_preview",strict:true,schema}}})});
    ...
    const used=new Set();const printedLines=(Array.isArray(parsed.printedLines)?parsed.printedLines:[]).slice(0,150).map((line,index)=>{const matchingLineId=ids.has(line.matchingLineId)&&!used.has(line.matchingLineId)?line.matchingLineId:"";if(matchingLineId)used.add(matchingLineId);return {...line,sequence:String(index+1),matchingLineId}});
    ...
    const reviewReady=assessInvoicePages({expectedPageCount:parsed.expectedPageCount,visiblePageNumbers:parsed.visiblePageNumbers,sourcePageCount:result.pages.length,printedLines,printedTotal});
    res.json({assistantMessage:String(parsed.assistantMessage||"").slice(0,5000),corrections:reviewReady?corrections.filter(change=>matchedIds.has(change.lineId)):[],printedLines,printedTotal,reviewOnly:true,pagesComplete:reviewReady,pageWarning:reviewReady?"":"Δεν επιβεβαιώθηκαν όλες οι φυσικές σελίδες και το άθροισμα των γραμμών με το τυπωμένο πληρωτέο. Οι προτάσεις δεν εφαρμόζονται."});
  }catch(error){next(error)}
});
