...
const schema={
  type:"object",additionalProperties:false,required:["assistantMessage","corrections","printedLines","printedTotal","printedQuantityTotal","printedNetTotal","expectedPageCount","visiblePageNumbers","singlePageComplete","exciseColumnAbsent"],
  properties:{
    assistantMessage:{type:"string"},printedTotal:{type:"string"},printedQuantityTotal:{type:"string"},printedNetTotal:{type:"string"},expectedPageCount:{type:"integer"},visiblePageNumbers:{type:"array",items:{type:"integer"}},singlePageComplete:{type:"boolean"},exciseColumnAbsent:{type:"boolean"},
    ...
    printedLines:{type:"array",items:{type:"object",additionalProperties:false,required:["sequence","supplierCode","description","quantity","invoiceUnit","stockUnitsPerInvoiceUnit","unitCost","unitDiscountAmount","lineDiscountAmount","discount1","discount2","discount3","exciseTotal","vatRate","netAmount","grossAmount","matchingLineId","confidence","reviewReason"],properties:Object.fromEntries(["sequence","supplierCode","description","quantity","invoiceUnit","stockUnitsPerInvoiceUnit","unitCost","unitDiscountAmount","lineDiscountAmount","discount1","discount2","discount3","exciseTotal","vatRate","netAmount","grossAmount","matchingLineId","confidence","reviewReason"].map(key=>[key,{type:"string"}]))}}
  }
};
...
router.post("/purchase-orders/:orderId/invoice-assistant/preview",requireCompanyModule("AI_READER"),manager,async(req,res,next)=>{
  try{
    ...
    const used=new Set();const mappedLines=(Array.isArray(parsed.printedLines)?parsed.printedLines:[]).slice(0,150).map((line,index)=>{const matchingLineId=ids.has(line.matchingLineId)&&!used.has(line.matchingLineId)?line.matchingLineId:"";if(matchingLineId)used.add(matchingLineId);const unitCost=number(line.unitCost),quantity=number(line.quantity),basket=number(line.unitDiscountAmount),lineDiscount=number(line.lineDiscountAmount);
      const printedPercentages=[line.discount1,line.discount2,line.discount3].map(number);
