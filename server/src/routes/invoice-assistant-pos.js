...
const schema={
  type:"object",additionalProperties:false,required:["assistantMessage","corrections","printedLines","printedTotal"],
  properties:{
    assistantMessage:{type:"string"},printedTotal:{type:"string"},
...
router.post("/purchase-orders/:orderId/invoice-assistant/preview",requireCompanyModule("AI_READER"),manager,async(req,res,next)=>{
  try{
    ...
    res.json({assistantMessage:String(parsed.assistantMessage||"").slice(0,5000),corrections,printedLines,printedTotal:/^\d+(?:[.,]\d{1,2})?$/.test(String(parsed.printedTotal||""))?number(parsed.printedTotal):null,reviewOnly:true});
  }catch(error){next(error)}
});
