...
router.post("/purchase-orders/:orderId/invoice-assistant/preview",requireCompanyModule("AI_READER"),manager,async(req,res,next)=>{
  try{
    ...
    const used=new Set();const mappedLines=(Array.isArray(parsed.printedLines)?parsed.printedLines:[]).slice(0,150).map((line,index)=>{const matchingLineId=ids.has(line.matchingLineId)&&!used.has(line.matchingLineId)?line.matchingLineId:"";if(matchingLineId)used.add(matchingLineId);const unitCost=number(line.unitCost),quantity=number(line.quantity),basket=number(line.unitDiscountAmount),lineDiscount=number(line.lineDiscountAmount);
      ...
      return {...line,discount1:String(discount1),discount2:String(discount2),discount3:String(discount3),confidence:lineDiscount>0&&!hasLineDiscount?"uncertain":line.confidence,sequence:String(index+1),matchingLineId}});
