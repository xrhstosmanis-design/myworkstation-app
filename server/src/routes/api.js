...
router.post("/lab/seed-drakou-workforce",async(req,res,next)=>{try{if(!isSuperAdmin(req.user))return res.status(403).json({error:"Μόνο Super Admin."});const body=z.object({storeId:z.string(),confirmed:z.literal(true)}).parse(req.body||{}),store=await prisma.store.findFirst({where:{id:body.storeId,companyId:req.user.companyId},include:{shifts:{where:{active:true}}}});if(!store||!plain(store.name).includes("ΕΡΓΑΣΤΗΡΙΟ"))return res.status(400).json({error:"Η δοκιμή επιτρέπεται μόνο στο ΕΡΓΑΣΤΗΡΙΟ ΔΟΚΙΜΩΝ."});const byCode=new Map(store.shifts.map(s=>[s.code,s]));for(const code of ["MORNING","AFTERNOON","NIGHT"])if(!byCode.has(code))return res.status(400).json({error:`Λείπει η βάρδια ${code} από το LAB.`});const specs=[["Νωπή",["MORNING"]],["Άγγελος",["NIGHT"]],["Γιώργος",["AFTERNOON","NIGHT"]],["Σπύρος",["AFTERNOON","NIGHT"]],["Κωνσταντίνα",["AFTERNOON"]]];const result=[];await prisma.$transaction(async tx=>{for(const [name,codes] of specs){let e=await tx.employee.findFirst({where:{storeId:store.id,fullName:name}});if(!e)e=await tx.employee.create({data:{fullName:name,position:"Εργαζόμενος",type:"PERMANENT",storeId:store.id,active:true,maxDaysPerWeek:5,maxHoursPerWeek:40,allowSixthDay:false}});else e=await tx.employee.update({where:{id:e.id},data:{active:true,maxDaysPerWeek:5,maxHoursPerWeek:40,allowSixthDay:false}});await tx.employeeRule.deleteMany({where:{employeeId:e.id}});await tx.employeeRule.createMany({data:codes.map(code=>({employeeId:e.id,shiftTypeId:byCode.get(code).id,allowed:true,targetPerWeek:null,priority:0,note:"LAB Drakou scenario",fixedWeekdays:[]}))});result.push({id:e.id,name,codes})}});res.status(201).json({store:store.name,employees:result,message:"Το σενάριο Δράκου δημιουργήθηκε μόνο στο LAB."})}catch(e){next(e)}});
...
let employeeNaturalRuleSchemaPromise;
async function ensureEmployeeNaturalRuleSchema(){if(!employeeNaturalRuleSchemaPromise)employeeNaturalRuleSchemaPromise=prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "EmployeeNaturalRule" ("id" TEXT PRIMARY KEY,"employeeId" TEXT NOT NULL,"instruction" TEXT NOT NULL,"interpretation" JSONB NOT NULL DEFAULT '{}'::jsonb,"effectiveFrom" DATE NOT NULL,"effectiveTo" DATE,"createdBy" TEXT,"createdAt" TIMESTAMPTZ NOT NULL DEFAULT NOW())`).catch(error=>{employeeNaturalRuleSchemaPromise=undefined;throw error});return employeeNaturalRuleSchemaPromise}
router.get("/employees/:id/natural-rules",async(req,res,next)=>{try{await ensureEmployeeNaturalRuleSchema();const employee=await prisma.employee.findFirst({where:{id:req.params.id,store:{companyId:req.user.companyId}}});if(!employee)return res.status(404).json({error:"Δεν βρέθηκε εργαζόμενος."});const rows=await prisma.$queryRaw`SELECT * FROM "EmployeeNaturalRule" WHERE "employeeId"=${employee.id} ORDER BY "effectiveFrom" DESC,"createdAt" DESC`;res.json({items:rows})}catch(e){next(e)}});
router.post("/employees/:id/natural-rules/interpret",async(req,res,next)=>{try{const body=z.object({instruction:z.string().trim().min(3).max(4000),effectiveFrom:z.string()}).parse(req.body||{}),employee=await prisma.employee.findFirst({where:{id:req.params.id,store:{companyId:req.user.companyId}},include:{store:{include:{shifts:{where:{active:true}}}}}});if(!employee)return res.status(404).json({error:"Δεν βρέθηκε εργαζόμενος."});const text=plain(body.instruction),allowedShiftCodes=employee.store.shifts.filter(s=>{const n=plain(s.name+" "+s.code);if(text.includes("ΜΟΝΟ ΠΡΩ"))return n.includes("ΠΡΩ")||s.code==="MORNING";if(text.includes("ΜΟΝΟ ΑΠΟΓ"))return n.includes("ΑΠΟΓ")||s.code==="AFTERNOON";if(text.includes("ΜΟΝΟ ΒΡΑ")||text.includes("ΜΟΝΟ ΝΥΧ"))return n.includes("ΒΡΑ")||n.includes("ΝΥΧ")||s.code==="NIGHT";return false}).map(s=>s.code),daysMatch=text.match(/(\d+)\s*(ΗΜΕΡ|ΜΕΡ)/),maxDaysPerWeek=daysMatch?Math.min(6,Math.max(1,Number(daysMatch[1]))):null,continuousDaysOff=text.includes("ΣΥΝΕΧ")&&text.includes("ΡΕΠΟ");const understood=[];if(allowedShiftCodes.length)understood.push(`Επιτρεπόμενες βάρδιες: ${allowedShiftCodes.join(", ")}`);if(maxDaysPerWeek)understood.push(`Έως ${maxDaysPerWeek} ημέρες εργασίας την εβδομάδα`);if(continuousDaysOff)understood.push("Προτίμηση για συνεχόμενα ρεπό");if(!understood.length)understood.push("Ο κανόνας θα αποθηκευτεί ως φυσική οδηγία και θα εμφανίζεται στο ιστορικό.");res.json({interpretation:{allowedShiftCodes,maxDaysPerWeek,continuousDaysOff,understood},effectiveFrom:body.effectiveFrom})}catch(e){next(e)}});
router.post("/employees/:id/natural-rules",async(req,res,next)=>{try{await ensureEmployeeNaturalRuleSchema();const body=z.object({instruction:z.string().trim().min(3).max(4000),effectiveFrom:z.string(),interpretation:z.object({allowedShiftCodes:z.array(z.string()).default([]),maxDaysPerWeek:z.number().int().min(1).max(6).nullable().default(null),continuousDaysOff:z.boolean().default(false),understood:z.array(z.string()).default([])}),confirmed:z.literal(true)}).parse(req.body||{}),employee=await prisma.employee.findFirst({where:{id:req.params.id,store:{companyId:req.user.companyId}},include:{store:{include:{shifts:{where:{active:true}}}}}});if(!employee)return res.status(404).json({error:"Δεν βρέθηκε εργαζόμενος."});const id=crypto.randomUUID();await prisma.$transaction(async tx=>{await tx.$executeRaw`INSERT INTO "EmployeeNaturalRule" ("id","employeeId","instruction","interpretation","effectiveFrom","createdBy") VALUES (${id},${employee.id},${body.instruction},${JSON.stringify(body.interpretation)}::jsonb,${new Date(body.effectiveFrom)},${req.user.id||req.user.email||null})`;if(body.interpretation.maxDaysPerWeek)await tx.employee.update({where:{id:employee.id},data:{maxDaysPerWeek:body.interpretation.maxDaysPerWeek,maxHoursPerWeek:body.interpretation.maxDaysPerWeek*8}});if(body.interpretation.allowedShiftCodes.length){const shiftIds=employee.store.shifts.filter(s=>body.interpretation.allowedShiftCodes.includes(s.code)).map(s=>s.id);await tx.employeeRule.deleteMany({where:{employeeId:employee.id}});if(shiftIds.length)await tx.employeeRule.createMany({data:shiftIds.map(shiftTypeId=>({employeeId:employee.id,shiftTypeId,allowed:true,targetPerWeek:null,priority:0,note:`Natural rule effective ${body.effectiveFrom}`,fixedWeekdays:[]}))})}});res.status(201).json({id})}catch(e){next(e)}});
