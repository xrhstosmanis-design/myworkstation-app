...
const scheduleChatSchema=z.object({storeId:z.string(),dateFrom:z.string(),dateTo:z.string(),instructions:z.string().max(12000).default(""),shiftOverrides:z.record(z.string(),z.object({startTime:z.string().optional(),endTime:z.string().optional(),requiredCount:z.number().optional()})).default({}),messages:z.array(z.object({role:z.enum(["user","assistant"]),text:z.string().min(1).max(4000)})).min(1).max(8)});
const scheduleChatOutputSchema={type:"object",additionalProperties:false,properties:{assistantText:{type:"string"},scheduleRows:{type:"array",items:{type:"object",additionalProperties:false,properties:{date:{type:"string"},shift:{type:"string"},hours:{type:"string"},employees:{type:"array",items:{type:"string"}},note:{type:"string"}},required:["date","shift","hours","employees","note"]}},suggestedInstructions:{type:"string"},warnings:{type:"array",items:{type:"string"}}},required:["assistantText","scheduleRows","suggestedInstructions","warnings"]};
...
router.post("/schedules/chat",async(req,res,next)=>{try{
  ...
  const store=await prisma.store.findFirst({where:storeScope(req,body.storeId),include:{shifts:{where:{active:true}},employees:{where:{active:true},include:{rules:true,leaveRequests:{where:{status:"APPROVED"}}}}}});if(!store)return res.status(404).json({error:"Δεν βρέθηκε κατάστημα."});
  const periodStart=new Date(body.dateFrom),periodEnd=new Date(`${body.dateTo}T23:59:59.999Z`);
  const latest=await prisma.schedule.findFirst({where:{storeId:store.id,weekStart:{lte:periodEnd}},orderBy:{weekStart:"desc"},include:{assignments:{where:{date:{gte:periodStart,lte:periodEnd}},include:{employee:true,shiftType:true},orderBy:[{date:"asc"},{slot:"asc"}]}}});
  const overlapsPeriod=leave=>leave.startDate<=periodEnd&&leave.endDate>=periodStart;
  const context={store:store.name,period:{dateFrom:body.dateFrom,dateTo:body.dateTo},employeeNames:store.employees.map(row=>row.fullName),approvedLeaves:store.employees.flatMap(row=>row.leaveRequests.filter(overlapsPeriod).map(leave=>({employee:row.fullName,from:dateKey(leave.startDate),to:dateKey(leave.endDate),type:leave.type}))),shiftTable:store.shifts.map(row=>{const override=body.shiftOverrides[row.id]||{};return {shift:row.name,code:row.code,startTime:override.startTime??row.startTime,endTime:override.endTime??row.endTime,requiredPeople:override.requiredCount??row.requiredCount}}),currentInstructions:body.instructions,currentSchedule:(latest?.assignments||[]).map(row=>({date:dateKey(row.date),shift:row.shiftType.name,hours:`${row.shiftType.startTime}-${row.shiftType.endTime}`,employee:row.employee?.fullName||"ΑΚΑΛΥΠΤΟ"})),conversation:body.messages.slice(-6)};
  const prompt=`Είσαι γρήγορος βοηθός προγράμματος εργαζομένων του MyWorkStation. Απάντησε σύντομα στα ελληνικά. Πηγή αλήθειας είναι ΜΟΝΟ ο εμφανιζόμενος πίνακας της επιλεγμένης περιόδου: shiftTable και currentSchedule. Για προσωπικό χρησιμοποίησε ΜΟΝΟ τα ονόματα employeeNames, χωρίς θέσεις, τηλέφωνα, email ή άλλα στοιχεία. Έλεγξε υποχρεωτικά όλες τις approvedLeaves και ποτέ μην αναθέσεις εργαζόμενο σε ημέρα άδειας. Μην εφευρίσκεις όνομα, άδεια, βάρδια ή ώρα. Όταν η ερώτηση αφορά πρόγραμμα ή αλλαγές, συμπλήρωσε scheduleRows για να εμφανιστεί απάντηση σε πίνακα ίδιο με το πρόγραμμα· μία γραμμή ανά ημερομηνία και βάρδια, με όλα τα ονόματα στο employees. Για απλή ερώτηση άφησε scheduleRows κενό. Αν ζητείται αλλαγή, δώσε και σύντομο suggestedInstructions. Δεν αποθηκεύεις αλλαγές αυτόματα. Context: ${JSON.stringify(context)}`;
  const response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({model:process.env.OPENAI_SCHEDULE_CHAT_MODEL||"gpt-5-mini",input:prompt,reasoning:{effort:"minimal"},text:{verbosity:"low",format:{type:"json_schema",name:"staff_schedule_chat",strict:true,schema:scheduleChatOutputSchema}}})});
  ...
  res.json({assistantText:String(result.assistantText||""),scheduleRows:Array.isArray(result.scheduleRows)?result.scheduleRows:[],suggestedInstructions:String(result.suggestedInstructions||""),warnings:Array.isArray(result.warnings)?result.warnings.map(String):[],model:process.env.OPENAI_SCHEDULE_CHAT_MODEL||"gpt-5-mini"});
}catch(e){next(e)}});
