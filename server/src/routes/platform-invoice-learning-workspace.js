...
// Central learning creates a reusable Master item first. Publishing is explicit:
// selected stores receive a zero-stock Product/StoreProduct record; no invoice stock
// or selling price is invented by the Learning Lab.
...
router.post("/invoice-learning/master-products",async(req,res,next)=>{try{
  const body=req.body&&typeof req.body==="object"?req.body:{};
  const name=String(body.description||body.name||"").trim().replace(/\s+/g," ");
  const supplierName=String(body.supplierName||"").trim().replace(/\s+/g," ");
  const supplierCode=String(body.supplierItemCode||"").trim();
  const barcode=String(body.barcode||"").replace(/\D/g,"");
  const categoryName=String(body.categoryName||"").trim()||null,subcategoryName=String(body.subcategoryName||"").trim()||null;
  const vatRate=asNumber(body.vatRate,{min:0,max:100}),cost=asNumber(body.defaultCostPrice??body.unitCost,{min:0}),retail=asNumber(body.defaultRetailPrice,{min:0});
  const storeIds=[...new Set(Array.isArray(body.storeIds)?body.storeIds.map(String).filter(Boolean):[])];
  ...
  const stores=storeIds.length?await prisma.store.findMany({where:{id:{in:storeIds},active:true},select:{id:true,name:true,companyId:true}}):[];
  if(stores.length!==storeIds.length)return res.status(400).json({error:"Κάποιο επιλεγμένο κατάστημα δεν είναι ενεργό."});
  const sourceBase=`IL-${cleanTaxId(body.supplierTaxId)||normName(supplierName).slice(0,20)||"SUPPLIER"}-${supplierCode||barcode||normName(name).slice(0,60)||"ITEM"}`.replace(/[^A-ZΑ-Ω0-9_-]/g,"").slice(0,120);
  const existing=await prisma.$queryRawUnsafe(`SELECT mp."id",mp."sourceCode",mp."name" FROM "MasterProduct" mp WHERE (CASE WHEN $1<>'' THEN mp."sourceCode"=$1 ELSE FALSE END) OR (CASE WHEN $2<>'' THEN EXISTS(SELECT 1 FROM "MasterProductBarcode" b WHERE b."masterProductId"=mp."id" AND b."barcode"=$2) ELSE FALSE END) LIMIT 1`,sourceBase,barcode);
  let master=existing?.[0]||null,created=false,activatedMappings=0,createdProducts=0;
  await prisma.$transaction(async tx=>{
    if(!master){
      const sourceCode=sourceBase||`IL-${Date.now()}`,id=uid();
      await tx.$executeRaw`INSERT INTO "MasterProduct" ("id","sourceCode","name","categoryName","subcategoryName","supplierName","defaultRetailPrice","defaultCostPrice","vatRate","vatVerified","active","reviewStatus","importVersion") VALUES (${id},${sourceCode},${name},${categoryName},${subcategoryName},${supplierName||null},${retail},${cost},${vatRate},${vatRate!==null},true,'LEARNING_CONFIRMED','INVOICE_LEARNING')`;
      if(barcode)await tx.$executeRaw`INSERT INTO "MasterProductBarcode" ("id","masterProductId","barcode","scanEnabled","duplicateBarcode") VALUES (${uid()},${id},${barcode},true,false)`;
      master={id,sourceCode,name};created=true;
    }
    ...
    for(const [companyId,companyStores] of byCompany){
      let product=(await tx.$queryRaw`SELECT "id" FROM "Product" WHERE "companyId"=${companyId} AND "masterProductId"=${master.id} LIMIT 1`)[0];
      if(!product){
        const productId=uid(),sku=String(master.sourceCode||`IL-${Date.now()}`);
        await tx.$executeRaw`INSERT INTO "Product" ("id","companyId","masterProductId","sku","name","unit","vatRate","vatVerified","salePrice","costPrice","trackStock","active") VALUES (${productId},${companyId},${master.id},${sku},${master.name},'PIECE',${vatRate||0},${vatRate!==null},${retail||0},${cost||0},true,true)`;
        if(barcode)await tx.$executeRaw`INSERT INTO "ProductBarcode" ("id","productId","barcode","unitMultiplier") VALUES (${uid()},${productId},${barcode},1) ON CONFLICT DO NOTHING`;
        product={id:productId};createdProducts++;
      }
      for(const store of companyStores){
        const before=await tx.$queryRaw`SELECT "id" FROM "StoreProduct" WHERE "storeId"=${store.id} AND "productId"=${product.id} LIMIT 1`;
        await tx.$executeRaw`INSERT INTO "StoreProduct" ("id","storeId","productId","salePrice","currentStock","active") VALUES (${uid()},${store.id},${product.id},${retail||0},0,true) ON CONFLICT ("storeId","productId") DO UPDATE SET "active"=true,"updatedAt"=CURRENT_TIMESTAMP`;
