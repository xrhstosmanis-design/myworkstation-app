...
router.put("/invoice-learning/supplier-profile/column-map",async(req,res,next)=>{try{
  const body=req.body&&typeof req.body==="object"?req.body:{},supplierTaxId=cleanTaxId(body.supplierTaxId),supplierName=String(body.supplierName||"").trim(),commercialFamily=String(body.commercialFamily||"").trim(),distributorName=String(body.distributorName||"").trim(),columns=body.columns&&typeof body.columns==="object"&&!Array.isArray(body.columns)?body.columns:null;
  ...
  await prisma.$transaction(async tx=>{
    ...
    const savedCommercialFamily=commercialFamily||existing.commercialFamily||previous.commercialFamily||"",savedDistributorName=distributorName||existing.distributorName||previous.distributorName||"";
    const profile={...previous,supplierName:supplierName||previous.supplierName||"",supplierTaxId:supplierTaxId||previous.supplierTaxId||"",commercialFamily:savedCommercialFamily,distributorName:savedDistributorName,central:true,profileVersion,ruleKey:"DECLARED_COLUMNS",readingRule,columnMap:{...(previous.columnMap||{}),columns:normalizedColumns},ruleDescription:"Χειροκίνητος χάρτης στηλών με μαθηματικό έλεγχο της τρέχουσας γραμμής."};
    await tx.$executeRawUnsafe(`INSERT INTO "InvoiceSupplierReadingProfile" ("supplierKey","supplierTaxId","supplierName","commercialFamily","distributorName","normalizedName","ruleKey","profileVersion","profile","isActive","updatedByUserId","updatedAt") VALUES ($1,$2,$3,$4,$5,$6,'DECLARED_COLUMNS',$7,$8::jsonb,TRUE,$9,CURRENT_TIMESTAMP) ON CONFLICT ("supplierKey") DO UPDATE SET "supplierTaxId"=EXCLUDED."supplierTaxId","supplierName"=EXCLUDED."supplierName","commercialFamily"=EXCLUDED."commercialFamily","distributorName"=EXCLUDED."distributorName","normalizedName"=EXCLUDED."normalizedName","ruleKey"=EXCLUDED."ruleKey","profileVersion"=EXCLUDED."profileVersion","profile"=EXCLUDED."profile","isActive"=TRUE,"updatedByUserId"=EXCLUDED."updatedByUserId","updatedAt"=CURRENT_TIMESTAMP`,supplierKey,supplierTaxId||null,supplierName||previous.supplierName||null,savedCommercialFamily||null,savedDistributorName||null,normalizedName||normName(previous.supplierName||""),profileVersion,JSON.stringify(profile),userId);
    saved={supplierKey,profileVersion};
  });
  res.json({ok:true,...saved,onlyTargetSupplierUpdated:true,existingLearningPreserved:true,scope:"ALL_STORES",consumers:["POS","BACKOFFICE"]});
}catch(error){next(error)}});
