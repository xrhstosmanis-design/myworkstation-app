...
const schema={type:"object",additionalProperties:false,properties:{
  ...
  headerConfidence:{type:"number",minimum:0,maximum:100},
  ...
  documentNumber:{type:"string"},documentNumberConfidence:{type:"number",minimum:0,maximum:100},
  documentDate:{type:"string"},documentDateConfidence:{type:"number",minimum:0,maximum:100},
  totalNet:{type:"number",minimum:0},totalVat:{type:"number",minimum:0},totalGross:{type:"number",minimum:0},
...
router.post("/invoice-learning/ai-recheck",async(req,res,next)=>{try{
  ...
  const prompt=`Είσαι ειδικός ελεγκτής ελληνικών τιμολογίων προμηθευτών για το MyWorkStation Invoice Learning Lab. Διάβασε ΠΡΩΤΑ το πρωτότυπο PDF/εικόνα και χρησιμοποίησε το OCR μόνο ως βοήθημα.\n\nΠΡΩΤΗ ΠΡΟΤΕΡΑΙΟΤΗΤΑ — HEADER: πριν διαβάσεις προϊόντα, εντόπισε με ξεχωριστό έλεγχο τον ΕΚΔΟΤΗ/ΠΡΟΜΗΘΕΥΤΗ του τιμολογίου και ΟΧΙ τον πελάτη/παραλήπτη. Βρες υποχρεωτικά όπου είναι ορατά: supplier.name (επωνυμία εκδότη), supplier.taxId (ΑΦΜ εκδότη, συνήθως 9 ψηφία), documentNumber (Αρ. Τιμολογίου/Παραστατικού) και documentDate. Μην μπερδεύεις ΑΦΜ πελάτη με ΑΦΜ προμηθευτή. Χρησιμοποίησε ενδείξεις όπως ΕΚΔΟΤΗΣ, ΠΡΟΜΗΘΕΥΤΗΣ, ΠΩΛΗΤΗΣ, ΕΠΩΝΥΜΙΑ, ΑΦΜ, ΤΙΜΟΛΟΓΙΟ, ΑΡ. ΠΑΡΑΣΤΑΤΙΚΟΥ, ΣΕΙΡΑ, ΗΜΕΡΟΜΗΝΙΑ. Αν υπάρχουν πολλαπλά ΑΦΜ, διάλεξε του εκδότη. Αν κάποιο header δεν είναι πραγματικά αναγνώσιμο, άφησέ το κενό και δώσε χαμηλό confidence — μην το εφεύρεις.\n\nΔΕΥΤΕΡΗ ΠΡΟΤΕΡΑΙΟΤΗΤΑ — PRODUCT LINES: κράτησε ΜΟΝΟ τις πραγματικές γραμμές προϊόντων. ΜΗΝ επιστρέψεις headers, στοιχεία εταιρείας, ΑΦΜ, διευθύνσεις, IBAN, τρόπους πληρωμής, υποσύνολα, σύνολα ή footer ως προϊόντα. Μην εφευρίσκεις τίποτα.\n\nΓια κάθε προϊόν βρες: supplierItemCode, ακριβή περιγραφή, quantity, unit, unitsPerPackage/pack, αρχική unitPrice πριν από εκπτώσεις, έως τρεις διαδοχικές εκπτώσεις discount1/2/3, netUnitCost μετά τις εκπτώσεις, netAmount γραμμής, vatRate, grossAmount και barcode ΜΟΝΟ αν εμφανίζεται πραγματικά στο παραστατικό. Αν barcode δεν υπάρχει άφησέ το κενό — θα βρεθεί αργότερα από Master Catalog/online.\n\nΠΡΟΣΟΧΗ ΣΤΙΣ ΕΚΠΤΩΣΕΙΣ: αν βλέπεις διαδοχικές εκπτώσεις π.χ. 10% + 5%, μην τις αθροίζεις. Το netUnitCost πρέπει να αντιστοιχεί στη διαδοχική εφαρμογή τους. Αν το παραστατικό δίνει ήδη καθαρή τιμή/καθαρή αξία, προτίμησε τα ορατά δεδομένα και χρησιμοποίησε υπολογισμό μόνο για διασταύρωση.\n\nΠΡΟΣΟΧΗ ΣΤΙΣ ΣΥΣΚΕΥΑΣΙΕΣ: 6x500ml, 24τμχ, κιβώτιο κ.λπ. δεν είναι αυτόματα quantity. Ξεχώρισε quantity παραστατικού από unitsPerPackage.\n\nΠΡΙΝ απαντήσεις: ξανακοίτα οπτικά την πάνω περιοχή της πρώτης σελίδας ειδικά για επωνυμία/ΑΦΜ/αριθμό/ημερομηνία. Μετά μέτρησε τις πραγματικές σειρές προϊόντων, σύγκρινε άθροισμα net/gross γραμμών με τα σύνολα και ξανακοίτα τυχόν γραμμές που παρέλειψες. documentDate σε YYYY-MM-DD αν είναι σαφές.\n\nΠρόχειρο OCR confidence ${Number(ocrConfidence||0)}%:\n${ocrText||"(χωρίς χρήσιμο OCR κείμενο)"}`;
