...
export async function ensureVideoEventsSchema(){
  ...
  await prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "VideoConnectorCommand" (
    "id" TEXT PRIMARY KEY,"companyId" TEXT NOT NULL,"storeId" TEXT NOT NULL,"connectorDeviceId" TEXT,
    "commandType" TEXT NOT NULL,"cameraKey" TEXT,"videoEventId" TEXT,"payload" JSONB NOT NULL DEFAULT '{}'::jsonb,
    "status" TEXT NOT NULL DEFAULT 'PENDING',"result" JSONB,"errorCode" TEXT,
    "createdAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),"claimedAt" TIMESTAMPTZ,"completedAt" TIMESTAMPTZ,
    "expiresAt" TIMESTAMPTZ NOT NULL DEFAULT (NOW()+INTERVAL '5 minutes'),
    CONSTRAINT "VideoConnectorCommand_company_fkey" FOREIGN KEY ("companyId") REFERENCES "Company"("id") ON DELETE CASCADE,
    CONSTRAINT "VideoConnectorCommand_store_fkey" FOREIGN KEY ("storeId") REFERENCES "Store"("id") ON DELETE CASCADE,
    CONSTRAINT "VideoConnectorCommand_type_check" CHECK ("commandType" IN ('HEALTH','SNAPSHOT','CLIP')),
    CONSTRAINT "VideoConnectorCommand_status_check" CHECK ("status" IN ('PENDING','CLAIMED','COMPLETED','FAILED','EXPIRED'))
  )`);
  await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "VideoConnectorCommand_store_status_idx" ON "VideoConnectorCommand"("storeId","status","createdAt")`);
  await prisma.$executeRawUnsafe(`CREATE UNIQUE INDEX IF NOT EXISTS "VideoConnectorCommand_event_active_uq" ON "VideoConnectorCommand"("videoEventId","commandType") WHERE "videoEventId" IS NOT NULL AND "status" IN ('PENDING','CLAIMED')`);
  await prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "VideoMediaArtifact" (
    "id" TEXT PRIMARY KEY,"companyId" TEXT NOT NULL,"storeId" TEXT NOT NULL,"commandId" TEXT NOT NULL UNIQUE,
    "videoEventId" TEXT,"cameraKey" TEXT,"kind" TEXT NOT NULL,"mimeType" TEXT NOT NULL,"filename" TEXT NOT NULL,
    "byteLength" BIGINT NOT NULL DEFAULT 0,"sha256" TEXT,"status" TEXT NOT NULL DEFAULT 'UPLOADING',
    "createdAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),"completedAt" TIMESTAMPTZ,"expiresAt" TIMESTAMPTZ NOT NULL,
    CONSTRAINT "VideoMediaArtifact_command_fkey" FOREIGN KEY ("commandId") REFERENCES "VideoConnectorCommand"("id") ON DELETE CASCADE,
    CONSTRAINT "VideoMediaArtifact_kind_check" CHECK ("kind" IN ('SNAPSHOT','CLIP')),
    CONSTRAINT "VideoMediaArtifact_status_check" CHECK ("status" IN ('UPLOADING','READY','FAILED'))
  )`);
  await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "VideoMediaArtifact_event_idx" ON "VideoMediaArtifact"("videoEventId","kind","createdAt" DESC)`);
  await prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "VideoMediaArtifactChunk" (
    "artifactId" TEXT NOT NULL,"chunkIndex" INTEGER NOT NULL,"bytes" BYTEA NOT NULL,"createdAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    PRIMARY KEY ("artifactId","chunkIndex"),
    CONSTRAINT "VideoMediaArtifactChunk_artifact_fkey" FOREIGN KEY ("artifactId") REFERENCES "VideoMediaArtifact"("id") ON DELETE CASCADE
  )`);
  await prisma.$executeRawUnsafe(`DELETE FROM "VideoMediaArtifact" WHERE "expiresAt"<NOW()`);
  await prisma.$executeRawUnsafe(`UPDATE "VideoConnectorCommand" SET "status"='EXPIRED' WHERE "status" IN ('PENDING','CLAIMED') AND "expiresAt"<NOW()`);
