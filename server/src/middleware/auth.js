...
function storeRuntimePermissions(profile){
  ...
  if(rights.cash)permissions.push("CASH_CONTROL","POS_CASH");
...
function enforceStorePosPermissions(req,res,permissions){
  ...
  if(req.method==="GET"&&/\/customers(?:\/|$)/.test(path)&&!permissions.includes("CUSTOMERS_POS")){
    return deny("Δεν έχεις δικαίωμα «Πελάτες (PoS)» από το BackOffice.");
  }
  return true;
}
