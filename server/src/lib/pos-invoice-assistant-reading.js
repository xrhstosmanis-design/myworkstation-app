...
import {assessInvoicePages} from "../routes/invoice-assistant-review.js";
...
const rowSchema={type:"object",additionalProperties:false,required:["page","rawText","code","description","quantity","unit","unitsPerPackage","unitCost","discount1","discount2","discount3","netAmount","exciseTotal","vatRate","grossAmount","confidence"],properties:Object.fromEntries(["page","rawText","code","description","quantity","unit","unitsPerPackage","unitCost","discount1","discount2","discount3","netAmount","exciseTotal","vatRate","grossAmount","confidence"].map(key=>[key,{type:"string"}]))};
const schema={type:"object",additionalProperties:false,required:["expectedPageCount","visiblePageNumbers","printedTotal","lines"],properties:{expectedPageCount:{type:"integer"},visiblePageNumbers:{type:"array",items:{type:"integer"}},printedTotal:{type:"string"},lines:{type:"array",items:rowSchema}}};
...
export function assistantRowsToProductLines(reading,{pageCount,totalGross}){
  ...
  if(!Number.isFinite(printedTotal)||printedTotal<=0||Math.abs(printedTotal-Number(totalGross))>0.05||!assessInvoicePages({expectedPageCount:reading.expectedPageCount,visiblePageNumbers:reading.visiblePageNumbers,sourcePageCount:pageCount,printedLines:reviewLines,printedTotal}))throw new Error("Ο βοηθός δεν επιβεβαίωσε όλες τις σελίδες, τις γραμμές και το τυπωμένο πληρωτέο. Το πρόχειρο διατηρήθηκε για έλεγχο χωρίς αυτόματες γραμμές.");
  ...
  const mapped=lines.map((line,index)=>{
    ...
    const discounts=[line.discount1,line.discount2,line.discount3].map(number),unitsPerPackage=number(line.unitsPerPackage);
    const description=String(line.description||"").trim(),unit=String(line.unit||"").trim().toUpperCase();
    if(!Number.isInteger(page)||page<1||page>pageCount||!description||description.length>500||!(quantity>0)||!(unitCost>0)||![0,6,13,24].includes(vatRate)||![quantity,unitCost,netAmount,exciseTotal,grossAmount,unitsPerPackage,...discounts].every(Number.isFinite)||[netAmount,exciseTotal,grossAmount].some(value=>value<0)||discounts.some(value=>value<0||value>=100)||!(unitsPerPackage>=1)||!(["PIECE","PACKAGE"].includes(unit)))throw new Error(`Η γραμμή ${index+1} του βοηθού χρειάζεται έλεγχο.`);
...
export async function readPosInvoiceWithAssistant({companyId,storeId,pageJobIds,totalGross}){
  ...
  const instruction=`Διάβασε το πλήρες τιμολόγιο από ΟΛΕΣ τις ${pages.length} συνημμένες σελίδες, με μία εγγραφή ανά φυσική γραμμή προϊόντος, συμπεριλαμβανομένων ίδιων επαναλαμβανόμενων προϊόντων. Μην χρησιμοποιήσεις προηγούμενο OCR ή αποθηκευμένες γραμμές. Επέστρεψε φυσικό αριθμό σελίδας ανά γραμμή και expectedPageCount/visiblePageNumbers από τις τυπωμένες ενδείξεις. Αν οι σελίδες δεν αποδεικνύονται πλήρεις, expectedPageCount=0, visiblePageNumbers=[]. Το rawText είναι αυτολεξεί η ορατή φυσική σειρά, μαζί με κωδικό και ποσότητα, χωρίς αναδιατύπωση. Δώσε τυπωμένο πληρωτέο και ξεχώρισε αρχική τιμή, τρεις εκπτώσεις %, καθαρό, ΕΦΚ, ΦΠΑ και μικτό ανά γραμμή. quantity είναι η ΤΥΠΩΜΕΝΗ ποσότητα· 48 πακέτα × 100 τεμάχια δίνει quantity=48, unit=PACKAGE, unitsPerPackage=100. Για τεμάχια unit=PIECE, unitsPerPackage=1. Όλοι οι αριθμοί είναι δεκαδικά strings χωρίς νόμισμα. confidence είναι certain μόνο όταν η φυσική γραμμή διαβάζεται ολόκληρη, διαφορετικά uncertain και κενό στα άγνωστα πεδία. Μην συμπληρώνεις αριθμούς από το ποσό κεφαλίδας. Το ποσό που επιβεβαίωσε ο χειριστής POS ήταν ${Number(totalGross).toFixed(2)} €, αλλά δεν είναι απόδειξη ορθότητας των γραμμών.`;
  const content=[{type:"input_text",text:instruction},...pages.map((page,index)=>page.mimeType==="application/pdf"?{type:"input_file",filename:page.filename||`page-${index+1}.pdf`,file_data:String(page.contentData).split(",").pop()}:{type:"input_image",image_url:page.contentData,detail:"high"})];
  const response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,"Content-Type":"application/json"},signal:AbortSignal.timeout(120000),body:JSON.stringify({model:process.env.OPENAI_INVOICE_ASSISTANT_MODEL||process.env.OPENAI_INVOICE_MODEL||"gpt-5",reasoning:{effort:"medium"},input:[{role:"user",content}],text:{format:{type:"json_schema",name:"pos_invoice_assistant_reading",strict:true,schema}}})});
