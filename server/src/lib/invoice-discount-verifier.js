...
function validatePrintedEconomics(candidate,{footerOnlyVat=false}={}){
  ...
  const excise=Math.max(0,Number(candidate?.exciseTotal||0)),vatRate=Math.max(0,Number(candidate?.vatRate||0));
  const taxable=footerOnlyVat?money4(net+excise):safeAmount(candidate?.taxableAmount);
  const vatAmount=footerOnlyVat?Math.round((taxable*vatRate/100+Number.EPSILON)*100)/100:Math.max(0,Number(candidate?.vatAmount||0));
  const gross=footerOnlyVat?money4(taxable+vatAmount):safeAmount(candidate?.grossAmount);
...
export function buildCompletePrintedTableCandidate(candidates,expectedGrossTotal,vatSummary=[]){
  ...
  // Thermal invoices often print only the VAT rate per product. Infer the
  // omitted per-row tax amounts exclusively from the printed net and rate,
  // and accept them only when the independent footer and header agree.
  const footerOnlyVat=summaries.length>0&&ordered.every(row=>Number(row?.taxableAmount||0)===0&&Number(row?.vatAmount||0)===0&&Number(row?.grossAmount||0)===0);
  const rebuilt=[];
  for(const candidate of ordered){
    const code=String(candidate?.supplierCode||'').trim(),description=String(candidate?.description||'').replace(/\s+/g,' ').trim();
    const printed=validatePrintedEconomics(candidate,{footerOnlyVat}),confidence=Number(candidate?.confidence||0);
