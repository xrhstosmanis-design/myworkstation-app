...
function recoverIfantisLine(line){
  ...
  const tokens=numberTokens(line?.azureRawRow||line?.rawText);if(tokens.length<3)return line;
...
// A supplier profile can state that OCR must prefer the numeric token which
// reconciles the printed line total. This covers layouts where an adjacent
// numeric column is mistaken for quantity (for example a pack/reference 4
// followed by the actual quantity 30). The rule changes no line unless the
// invoice arithmetic proves a unique, realistic candidate.
function recoverQuantityFromLineTotal(line){
  ...
  const candidates=numberTokens(line?.azureRawRow||line?.rawText).map(x=>x.value).filter(q=>q>0&&q<=100000&&close(q*price,net,Math.max(.03,net*.012)));
  const unique=[...new Set(candidates.map(q=>Math.round(q*10000)/10000))];
  if(unique.length!==1)return line;
  const quantity=unique[0];
  if(close(quantity,Number(line?.quantity||0),.0001))return line;
...
const wordsOf=line=>String(line?.azureRawRow||line?.rawText||"").trim().split(/\s+/).filter(Boolean);
