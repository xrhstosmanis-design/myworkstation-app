...
async function main(){
  ...
  const overEight=await request(`${base}/attendance?date=${periodStart}`,{token});assert.equal(overEight.response.status,200,JSON.stringify(overEight.payload));assert.equal(overEight.payload.summary.scheduledMinutes,480);assert.equal(overEight.payload.summary.varianceMinutes,30);assert.equal(overEight.payload.summary.workedAmount,51);assert.ok(overEight.payload.items.some(item=>item.id===clockIn.payload.item.id&&item.status==="NEEDS_APPROVAL"&&item.scheduledMinutes===480&&item.varianceMinutes===30&&item.hourlyRate===6&&item.workedAmount===51));
  const corrected=await request(`${base}/attendance/${clockIn.payload.item.id}/correct-hours`,{method:"POST",token,body:{confirmed:true,reason:"E2E διόρθωση ξεχασμένου κλεισίματος",workedMinutes:900}});assert.equal(corrected.response.status,200,JSON.stringify(corrected.payload));assert.equal(corrected.payload.item.workedMinutes,900);assert.equal(corrected.payload.item.status,"NEEDS_APPROVAL");
  const correctedRead=await request(`${base}/attendance?date=${periodStart}`,{token});assert.equal(correctedRead.response.status,200,JSON.stringify(correctedRead.payload));assert.ok(correctedRead.payload.items.some(item=>item.id===clockIn.payload.item.id&&item.workedMinutes===900&&item.status==="NEEDS_APPROVAL"));
  ...
  const reviewIn=await request(`${base}/attendance/clock`,{method:"POST",token,body:{employeeId:employeeB,eventType:"IN",confirmed:true,reason:"E2E review έναρξη"}});assert.equal(reviewIn.response.status,201,JSON.stringify(reviewIn.payload));
  const reviewOut=await request(`${base}/attendance/clock`,{method:"POST",token,body:{employeeId:employeeB,eventType:"OUT",confirmed:true,reason:"E2E review λήξη"}});assert.equal(reviewOut.response.status,201,JSON.stringify(reviewOut.payload));
  await prisma.workforceAttendanceSession.update({where:{id:reviewIn.payload.item.id},data:{status:"NEEDS_REVIEW",issueJson:{issues:[{code:"LATE_ARRIVAL",message:"E2E ανασκόπηση παρουσίας."}]}}});
  const reviewCorrection=await request(`${base}/attendance/${reviewIn.payload.item.id}/correct-hours`,{method:"POST",token,body:{confirmed:true,reason:"E2E διόρθωση μικρής παρουσίας",workedMinutes:240}});assert.equal(reviewCorrection.response.status,200,JSON.stringify(reviewCorrection.payload));assert.equal(reviewCorrection.payload.item.status,"NEEDS_REVIEW");
  const reviewApproval=await request(`${base}/attendance/${reviewIn.payload.item.id}/approve-review`,{method:"POST",token,body:{confirmed:true,reason:"E2E έγκριση διορθωμένων ωρών"}});assert.equal(reviewApproval.response.status,200,JSON.stringify(reviewApproval.payload));assert.equal(reviewApproval.payload.item.status,"APPROVED");
  const attendanceAudit=await request(`${base}/audit`,{token});assert.equal(attendanceAudit.response.status,200,JSON.stringify(attendanceAudit.payload));assert.ok(attendanceAudit.payload.items.some(item=>item.action==="WORKFORCE_CLOCK_IN"&&item.reason));assert.ok(attendanceAudit.payload.items.some(item=>item.action==="WORKFORCE_CLOCK_OUT"&&item.reason));assert.ok(attendanceAudit.payload.items.some(item=>item.action==="WORKFORCE_ATTENDANCE_HOURS_CORRECTED"&&item.reason));assert.ok(attendanceAudit.payload.items.some(item=>item.action==="WORKFORCE_ATTENDANCE_REVIEW_APPROVED"&&item.reason));assert.ok(attendanceAudit.payload.items.some(item=>item.action==="WORKFORCE_OVER_8_HOURS_APPROVED"&&item.reason));
