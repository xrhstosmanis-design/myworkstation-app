...
async function main(){
  ...
  assert.equal(shortageAttempt.response.status,409,JSON.stringify(shortageAttempt.payload));
  assert.equal(shortageAttempt.payload.code,"SHIFT_RECOUNT_REQUIRED");
  assert.equal(shortageAttempt.payload.shortage,3);
  const attemptAudit=await prisma.$queryRawUnsafe(`SELECT "details" FROM "StoreOperatorAudit" WHERE "eventType"='SHIFT_CLOSE_SHORTAGE_ATTEMPT' AND "details"->>'sessionId'=$1 ORDER BY "createdAt" DESC LIMIT 1`,sessionId);
  assert.equal(attemptAudit.length,1,"Shortage close attempt was not recorded in Events");
  assert.equal(Number(attemptAudit[0].details?.declaredOperational),47);

  const stillOpen=await request(`/api/cash/stores/${storeId}/overview`,{token:operatorToken});
  assert.equal(stillOpen.payload.openSession?.id,sessionId,"Shift closed before recount");
  const closed=await request(`/api/cash/sessions/${sessionId}/close`,{
    method:"POST",token:operatorToken,
    body:{cashSales:999,cardSales:999,eftposTotal:0,expenses:999,drawer:50,custody:0,coins:0,safe:20,note:"Σωστή επανακαταμέτρηση μετά την προειδοποίηση"}
  });
  ...
  assert.equal(closed.payload.actualOperational,50);
  assert.equal(closed.payload.variance,0);
  assert.equal(closed.payload.nextOpeningTotal,50);

  const duplicateClose=await request(`/api/cash/sessions/${sessionId}/close`,{
    method:"POST",token:operatorToken,
    body:{cashSales:0,cardSales:0,eftposTotal:0,expenses:0,drawer:50,custody:0,coins:0,safe:20,note:"duplicate close must fail"}
  });
  ...
  assert.equal(overview.payload.suggestedOpening?.drawer,50);
  assert.equal(overview.payload.suggestedOpening?.operational,50);
  ...
  assert.equal(row.actualOperational,50);
  ...
  const reopened=await request(`/api/cash/stores/${storeId}/sessions/open`,{
    method:"POST",token:operatorToken,
    body:{shiftLabel:"E2E επόμενη βάρδια",drawer:50,custody:0,coins:0,safe:20,note:"carry forward actual counted cash"}
  });
  assert.equal(reopened.response.status,201,JSON.stringify(reopened.payload));
  assert.equal(reopened.payload.expectedOpeningOperational,50);
  assert.equal(reopened.payload.openingOperational,50);
  ...
  const closeSecond=await request(`/api/cash/sessions/${reopened.payload.id}/close`,{
    method:"POST",token:operatorToken,
    body:{cashSales:0,cardSales:0,eftposTotal:0,expenses:0,drawer:50,custody:0,coins:0,safe:20,note:"clean up second E2E shift"}
  });
