...
function Get-Channel([object]$Command){$value=$Command.cameraKey;if($Command.payload.streamReference -match '^\d+$'){$value=$Command.payload.streamReference};$number=0;if([int]::TryParse([string]$value,[ref]$number)){return [Math]::Max(0,$number-1)};return 0}
