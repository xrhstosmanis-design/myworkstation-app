...
function Invoke-Onvif([string]$Path,[string]$Body){$content=New-Object Net.Http.StringContent((New-OnvifEnvelope $Body),[Text.Encoding]::UTF8,"application/soap+xml");try{$response=$script:NvrClient.PostAsync(($script:Config.nvrEndpoint.TrimEnd("/")+$Path),$content).GetAwaiter().GetResult();$xml=$response.Content.ReadAsStringAsync().GetAwaiter().GetResult();if(!$response.IsSuccessStatusCode){throw "ONVIF_$([int]$response.StatusCode)"};return $xml}finally{$content.Dispose()}}
function Onvif-Value([string]$Xml,[string]$Name){try{[xml]$doc=$Xml;$nodes=$doc.SelectNodes("//*[local-name()='$Name']");if($nodes -and $nodes.Count -gt 0){return [Net.WebUtility]::HtmlDecode($nodes[0].InnerText.Trim())}}catch{};return $null}
function Invoke-NvrBytes([string]$Path){
  ...
  if(!$response.IsSuccessStatusCode){$stage="NVR_REQUEST";if($Path.Contains("action=findFile")){$stage="FIND_FILE"}elseif($Path.Contains("action=findNextFile")){$stage="FIND_NEXT"}elseif($Path.Contains("loadfile.cgi")){$stage="LOAD_FILE"}elseif($Path.Contains("factory.create")){$stage="CREATE_SEARCH"};throw ("NVR_HTTP_{0}_{1}" -f [int]$response.StatusCode,$stage)}
