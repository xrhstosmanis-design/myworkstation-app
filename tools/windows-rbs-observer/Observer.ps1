...
function Send-ObservedFile([string]$Path){
  ...
  Write-SafeLog ("EVENT metadata sent bytes={0} hash={1}" -f $meta.length,$meta.hash.Substring(0,12))
}
...
Invoke-ObserverApi "/api/cloud/v1/device/observer/register" @{version=$Version;sources=@("CAPDRIVER","RBS","KIOSK_MANAGER")}|Out-Null
