param([string]$PairingCode,[string]$ApiBase="https://myworkstation-app.onrender.com",[string]$WatchPath="C:\capture")
...
$pairBody=@{code=$PairingCode;deviceName=("KAT-PC RBS Observer - "+$env:COMPUTERNAME);deviceKey=("rbs-observer:"+$env:COMPUTERNAME.ToLowerInvariant());platform="Windows Read-Only Observer";metadata=@{observerMode="READ_ONLY";commandsEnabled=$false}}|ConvertTo-Json -Depth 4 -Compress
...
$protectedToken=[Convert]::ToBase64String([System.Security.Cryptography.ProtectedData]::Protect($tokenBytes,$null,[System.Security.Cryptography.DataProtectionScope]::LocalMachine))
...
$shortcut.TargetPath="powershell.exe";$shortcut.Arguments=('-NoProfile -ExecutionPolicy Bypass -File "{0}"' -f (Join-Path $root "Status-Observer.ps1"));$shortcut.WorkingDirectory=$root;$shortcut.Save()
