...
### Zones & Multi-user access — συγχωνεύτηκε

- PR #320, CI #939 πράσινο, merge commit `4b4714a195ae07847fea47319baee23fac458ad7`.
- Ζώνες και προσωρινή QR/PIN πρόσβαση πολλών καταμετρητών βρίσκονται πλέον στο `main`.

### Ενεργό checkpoint — Finalization Summary
...
### Ενεργό checkpoint — ΚΑΤ POS πρότυπο σε LAB

- Προστέθηκε Super Admin API για ανάγνωση δημοσιευμένης διάταξης καταστήματος και ασφαλή αντιγραφή της σε επιλεγμένα καταστήματα.
- Αντιγράφεται μόνο το `StorePosLayout` (διάταξη/κουμπιά/κατηγορίες). Δεν αντιγράφονται προϊόντα, χρήστες/PIN, τερματικά, ταμεία, αποθήκη, βάρδιες, συναλλαγές ή fiscal/EFTPOS δεδομένα.
- Πρώτη δοκιμή παραμένει αποκλειστικά στο `MYWORKSTATION LAB` / `ΕΡΓΑΣΤΗΡΙΟ ΔΟΚΙΜΩΝ`, με LAB-POS-01 στο Surface και LAB-POS-02 στο PC.
- Επόμενο: UI επιλογής «ΚΑΤ ως πρότυπο», targeted tests, CI και δοκιμή ανοίγματος βάρδιας στο LAB.
